<?
class CPL_Admin_Lib_ArrayMaster extends CP_Common_Lib_ArrayMaster
{
     ...
     *
     */
    function setWidgetsArrayAccess() {
        $db    = Zend_Registry::get('db');
        $cpCfg = Zend_Registry::get('cpCfg');
        $tv    = Zend_Registry::get('tv');
        ...
        $result  = $db->sql_query($SQL);
        $numRows = $db->sql_numrows($result);
        $widgetsArrAccess = array();
        
        while ($row = $db->sql_fetchrow($result)) {
            $arr2 = &$widgetsArrAccess[$row['room_name']];
            $arr2['list'] = $row['list'];
        ...
        //------------------------------//
        /*$widgetRoomsArray = $cpCfg['cp.availableWidgets'];
        $widgetRoomsArrAccess = array();
        foreach($widgetArray as $widget => $arrTemp) {
            $widgetRoomsArrAccess[$widget]['hasAccess'] = false;
            if ($arrTemp['hasAccess'] == 1) {
                if (in_array($widget, $widgetRoomsArray)) {
                    $widgetRoomsArrAccess[$widget]['hasAccess'] = true;
                    break;
                }
            }
        }*/

        Zend_Registry::set('widgetsArrAccess', $widgetsArrAccess);
    }
}