<?
class CPL_Admin_Modules_EnggCrm_Reports_View extends CP_Admin_Modules_EnggCrm_Reports_View
{
    /**
     *
     */
    function getReportsDropdown() {
        $listObj          = Zend_Registry::get('listObj');
        $cpUtil           = Zend_Registry::get('cpUtil');
        $cpCfg            = Zend_Registry::get('cpCfg');
        $widgetsArrAccess = Zend_Registry::get('widgetsArrAccess');
        $widgetsArr       = Zend_Registry::get('widgetsArr');

        $text = "";
        $repArrSrc = $this->model->reportsArray;

        $text = "
        <table class='search'>
            <tr>
                <td>
                    <select name='report' class='report'>
                        <option value=''>Please Choose the Report</option>
        ";

        foreach($repArrSrc as $key => $value) {
            $groupTitle = $repArrSrc[$key]['title'];
            $reportsArr = $repArrSrc[$key]['reports'];

            $i = 0;
            $reports = "";
            foreach($reportsArr as $key => $value) {
                if (array_key_exists($value['name'], $widgetsArr)) {
                    if ($cpCfg['cp.hasAccessModule']) {
                        if (!$widgetsArrAccess[$value['name']]['hasAccess']) {
                            continue;
                        }
                    }

                    $i++;
                    
                    $reports .= "
                    <option value='{$key}'>{$value['title']}</option>
                    ";
                }
            }

            if($groupTitle != "") {
                if($i > 0) {
                    $text .= "
                    <optgroup label='{$groupTitle}'>
                        {$reports}
                    </optgroup>
                    ";
                }
            } else {
                $text .= "{$reports}";
            }
        }

        $text .= "
                    </select>
                </td>
            </tr>
        </table>
        ";

        return $text;
    }

    /**
     *
     */
    function getReportsDropdown1() {
        $listObj = Zend_Registry::get('listObj');
        $cpUtil  = Zend_Registry::get('cpUtil');
        $cpCfg  = Zend_Registry::get('cpCfg');

        $rowCounter = 0;
        $rows = "";
        $incomeExpenses = '';
        $reports = '';

        $repArrSrc = $this->model->reportsArray;

        $repArr = array();
        foreach($repArrSrc AS $key => $val){
            $repArr[$key] = $val['title'];
        }
        
        $text = "
        <table class='search'> 
        <tr>
            <td>
                <select name='report' class='report'>
                    <option value=''>Please Choose the Report</option>
                    <optgroup label='Standard Reports'>
                        <!--<option value='expenseSummaryReport'>Expense Summary Report</option>-->
                        <option value='overallSalesSummary'>Overall Sales Summary</option>
                        <option value='projectReport'>Project Report</option>
                    </optgroup>
                    <optgroup label='Financial Reports'>
                        <option value='invoiceByMonthReports'>Invoice by Month</option>
                        <option value='invoiceByYearReports'>Invoice by Year</option>
                        <option value='statementofAccountsReport'>Statement of Accounts Report</option>
                        <option value='ageingReport'>Ageing Report</option>
                        <!--<option value='profitLossReport'>Profit & Loss Report</option>-->
                    </optgroup>
                    <optgroup label='Payroll Reports'>
                        <option value='employeePayslipGeneratedReport'>Payslip Generated Report</option>
                        <option value='employeeSalaryReport'>Employee Salary Report</option>
                        <option value='employeeTrainingExpiryReport'>Employee Training Expiry Report</option>
                        <option value='cPFSummaryReport'>CPF Summary Report</option>
                        <option value='ir8a'>IR8A Report</option>
                    </optgroup>
                </select>
            </td>
        </tr>
        </table>
        ";

        return $text;
    }
    
    /**
     *
     */
    function getSearch() {
        $formObj          = Zend_Registry::get('formObj');
        $fn               = Zend_Registry::get('fn');
        $tv               = Zend_Registry::get('tv');
        $dbUtil           = Zend_Registry::get('dbUtil');
        $cpUtil           = Zend_Registry::get('cpUtil');
        $pager            = Zend_Registry::get('pager');
        $db               = Zend_Registry::get('db');
        $cpCfg            = Zend_Registry::get('cpCfg');
        $ln               = Zend_Registry::get('ln');
        $widgetsArrAccess = Zend_Registry::get('widgetsArrAccess');
        $widgetsArr       = Zend_Registry::get('widgetsArr');

        $url = "";
        $report          = $fn->getReqParam('report');
        $start_date      = $fn->getReqParam('start_date');
        $end_date        = $fn->getReqParam('end_date');
        $category        = $fn->getReqParam('category');
        $status          = $fn->getReqParam('status');
        $record_type     = $fn->getReqParam('record_type');
        $month           = $fn->getReqParam('month');
        $year            = $fn->getReqParam('year');
        $client_type     = $fn->getReqParam('client_type');
        $dormitory_id    = $fn->getReqParam('dormitory_id');
        $employee_status = $fn->getReqParam('employee_status');

        $reportsArray = $this->model->reportsArray;

        $repArr = array();
        foreach($reportsArray as $key => $value) {
            $groupTitle = $reportsArray[$key]['title'];
            $reportsArr = $reportsArray[$key]['reports'];

            $i = 0;
            $reports = "";
            foreach($reportsArr as $key => $value) {
                if (array_key_exists($value['name'], $widgetsArr)) {
                    if ($cpCfg['cp.hasAccessModule']) {
                        if (!$widgetsArrAccess[$value['name']]['hasAccess']) {
                            continue;
                        }
                    }

                    $i++;
                    
                    $repArr[$key] = $value;
                }
            }
        }

        $searchFldsArr = $repArr[$report]['searchFlds'];

        $employeeStatusArr = array(
              "Current"
             ,"Archive"
             ,"Cancel"
        );

        if ($year == '') {
            $year = date('Y');
        }

        $arr = array (
                '01' => 'January'
               ,'02' => 'February'
               ,'03' => 'March'
               ,'04' => 'April'
               ,'05' => 'May'
               ,'06' => 'June'
               ,'07' => 'July'
               ,'08' => 'August'
               ,'09' => 'September'
               ,'10' => 'October'
               ,'11' => 'November'
               ,'12' => 'December'
               );

        if ($month == '') {
            $month = date('m');
        }

        $current_date = date('Y-m-d');

        /* Statement of Accounts Report & Ageing Report */
        $company_id = $fn->getReqParam('company_id');
        $company_id = '';
        $rows = '';
        foreach($searchFldsArr AS $searchFld){

            if ($report == 'dormitoryReport') {
                $sqlDorm = "SELECT dormitory_id, name FROM dormitory";
                $rows .= "
                <td>
                    <select name='dormitory_id' class='dormitoryFilter'>
                        {$dbUtil->getDropDownFromSQLCols2($db, $sqlDorm, $dormitory_id)}
                    </select>
                </td>
                ";
            }

            if($report == 'expenseSummaryReport') {
                $sqlCompany ="
                SELECT c.supplier_id
                      ,c.company_name
                FROM supplier c
                WHERE company_name != ''
                ORDER BY company_name ASC
                ";

                $gstArray = array(
                    "Yes"
                    ,"No"
                );
                $statusArr = array('Due', 'Paid', 'On-hold', 'Cancelled');

                $rows .= "
                <td class='dateRange'>
                    <strong>Date Range</strong>
                    <input type='text' allowEdit='1' name='start_date' class='fld_date ml10'
                    id='fld_start_date' value='{$start_date}' maxDate='{$current_date}' />
                    <input type='text' allowEdit='1' name='end_date' class='fld_date ml10'
                    id='fld_end_date' value='{$end_date}' maxDate='{$current_date}' />
                </td>
                <td>
                    <select name='company_id' class='invoiceByClientFilter ml10'>
                        <option value=''>Select Company Name</option>
                        {$dbUtil->getDropDownFromSQLCols2($db, $sqlCompany, $company_id)}
                    </select>
                </td>
                <td>
                    <select name='status' class='statusFilter ml10'>
                        <option value=''>Select Status</option>
                        {$cpUtil->getDropDown1($statusArr, $status)}
                   </select>
                </td>
                ";
            }

            if($report == 'operationalFinancialReport') {
                $rows .= "
                <td class='dateRange'>
                    Date Range:
                    <input type='text' allowEdit='1' name='start_date' class='fld_date'
                    id='fld_start_date' value='{$start_date}' />
                    <input type='text' allowEdit='1' name='end_date' class='fld_date'
                    id='fld_end_date' value='{$end_date}' />
                </td>
                ";
            }
            
            if($report == 'overallSalesSummary') {

                $sqlCompany = "
                SELECT c.company_id
                      ,c.company_name
                FROM company c
                WHERE c.company_name != ''
                  AND c.category = 'Client'
                ORDER BY c.company_name ASC
                ";

                $rows .= "
                <td class='dateRange'>
                    <strong>Date Range</strong>
                    <input type='text' allowEdit='1' name='start_date' class='fld_date ml10'
                    id='fld_start_date' value='{$start_date}' maxDate='{$current_date}' />
                    <input type='text' allowEdit='1' name='end_date' class='fld_date ml10'
                    id='fld_end_date' value='{$end_date}' maxDate='{$current_date}' />
                </td>
                <td>
                    <select name='company_id' class='invoiceByClientFilter ml10'>
                        <option value=''>Select Company Name</option>
                        {$dbUtil->getDropDownFromSQLCols2($db, $sqlCompany, $company_id)}
                    </select>
                </td>
                ";
            }

            // Payslip filter starts
            if ($report == 'employeePayslipGeneratedReport' || $report == 'cPFSummaryReport'){
                if ($month == '' || $month == date('m')) {
                    $month = date('m') - 1;
                    if ($month <= 9 && $month > 0) {
                        $month = 0 . $month;
                    } else if ($month == 0) {
                        $month = 12;
                    } else {
                        $month = $month;
                    }
                }

                $rows .= "
                <td>
                    <select name='month' class='ml10 mr10 month'>
                        {$cpUtil->getDropDownFromArr($arr, $month)}
                    </select>
                </td>
                ";

                $sqlYear = "SELECT DISTINCT payroll_year FROM payroll_management";
                $rows .= "
                <td>
                    <select name='year' class='attendanceFilter'>
                        {$dbUtil->getDropDownFromSQLCols1($db, $sqlYear, $year)}
                    </select>
                </td>
                ";
            }

            if ($report == 'employeeSalaryReport'){
                $rows .= "
                <td>
                    <select name='employee_status' class='ml10 mr10 employeeStatus'>
                        {$cpUtil->getDropDown1($employeeStatusArr, $employee_status)}
                    </select>
                </td>
                ";
            }

            if ($report == 'profitLossReport') {
                $sqlYear = "
                SELECT DISTINCT payroll_year AS Year
                FROM payroll_management
                ";

                $rows .= "
                <td>
                    <select name='year' class='leadStaffYearFilter'>
                        {$dbUtil->getDropDownFromSQLCols1($db, $sqlYear, $year)}
                    </select>
                </td>
                ";

                $rows .= "
                <td>
                    <select name='month' class='ml10 mr10 month'>
                        {$cpUtil->getDropDownFromArr($arr, $month)}
                    </select>
                </td>
                ";

                $rows .= "
                <td class='dateRange'>
                    From Date:
                    <input type='text' allowEdit='1' name='start_date' class='fld_date'
                    id='fld_start_date' value='{$start_date}' maxDate='{$current_date}' />
                    To Date:
                    <input type='text' allowEdit='1' name='end_date' class='fld_date'
                    id='fld_end_date' value='{$end_date}' maxDate='{$current_date}' />
                </td>
                ";
            }
            // Payslip filter ends
            if ($report == 'ageingReport'){
                $rows .= "";
            }
            
            if ($report == 'statementofAccountsReport'){
                $sqlCompany = "
                SELECT DISTINCT c.company_id
                               ,c.company_name
                FROM `order` o
                LEFT JOIN (company c) ON (o.company_id = c.company_id)
                WHERE c.company_id != ''
                ORDER BY company_name
                ";

                $clientTypeArr = array(
                    'Client'
                   ,'Supplier'
                );
        
                $rows .= "
                <td class='dateRange'>
                    <strong>Date Range</strong>
                    <input type='text' allowEdit='1' name='start_date' class='fld_date'
                    id='fld_start_date' value='{$start_date}' maxDate='{$current_date}' />
                    <input type='text' allowEdit='1' name='end_date' class='fld_date'
                    id='fld_end_date' value='{$end_date}' maxDate='{$current_date}' />
                </td>
                <td class='statementofAccountsReport'>
                    <select name='client_type' class='statusFilter ml10'>
                        <option value=''>Select Type</option>
                        {$cpUtil->getDropDown1($clientTypeArr, $client_type)}
                    </select>
                </td>
                <td class='statementofAccountsReport clientSupplierDropdown'>
                    <select name='company_id' class='invoiceByClientFilter ml10'>
                        <option value=''>Please Select</option>
                    </select>
                </td>
                <td>&nbsp;&nbsp;&nbsp;</td>
                ";                                
            }

            if ($report == 'opportunityReport' || $report == 'opportunityQuotation' 
             || $report == 'vacationReport') {                
                $rows .= "
                <td class='dateRange'>
                    <strong>Date Range</strong>
                    <input type='text' allowEdit='1' name='start_date' class='fld_date'
                    id='fld_start_date' value='{$start_date}' maxDate='{$current_date}' />
                    <input type='text' allowEdit='1' name='end_date' class='fld_date'
                    id='fld_end_date' value='{$end_date}' maxDate='{$current_date}' />
                </td>
                ";
            }
            
            if ($report == 'opportunityReport' || $report == 'opportunityQuotation') {
                $sqlCat    = $fn->getValueListSQL('projectCategory');
                $SQLStatus = $fn->getValueListSQL('opportunityStatus');
                $rows .= "
                <td>
                    <select name='category'>
                        <option value=''>Category</option>
                        {$dbUtil->getDropDownFromSQLCols1($db, $sqlCat, $category)}
                    </select>
                </td>
                <td>
                    <select name='status'>
                        <option value=''>Status</option>
                        {$dbUtil->getDropDownFromSQLCols1($db, $SQLStatus, $status)}
                    </select>
                </td>
                ";
			}

            if ($report == 'employeeReport') {
                $SQLStatus = $fn->getValueListSQL('employeeStatus');
                $rows .= "
                <td>
                    <select name='status'>
                        <option value=''>Status</option>
                        {$dbUtil->getDropDownFromSQLCols1($db, $SQLStatus, $status)}
                    </select>
                </td>
                ";
            }

            if ($report == 'projectReport'){
                $SQLStatus = $fn->getValueListSQL('projectStatus');
                $sqlCat    = $fn->getValueListSQL('projectCategory');
                
                $rows .= "
                <td class='dateRange'>
                    <strong>Date Range</strong>
                    <input type='text' allowEdit='1' name='start_date' class='fld_date ml10'
                    id='fld_start_date' value='{$start_date}' maxDate='{$current_date}' />
                    <input type='text' allowEdit='1' name='end_date' class='fld_date ml10'
                    id='fld_end_date' value='{$end_date}' maxDate='{$current_date}' />
                </td>
                <td>
                    <select name='category' class='ml10 projectCategoryFilter'>
                        <option value=''>Select Category</option>
                        {$dbUtil->getDropDownFromSQLCols1($db, $sqlCat, $category)}
                    </select>
                </td>
                <td>
                    <select name='status' class='ml10 projectStatusFilter'>
                        <option value=''>Select Status</option>
                        {$dbUtil->getDropDownFromSQLCols1($db, $SQLStatus, $status)}
                    </select>
                </td>
                ";
            }

            if ($report == 'contractReport'){
                $sqlCompany = "
                SELECT c.company_id
                      ,c.company_name
                FROM company c
                WHERE c.company_name != ''
                ORDER BY c.company_name ASC
                ";

                $rows .= "
                <td class='dateRange'>
                    <strong>Date Range</strong>
                    <input type='text' allowEdit='1' name='start_date' class='fld_date ml10'
                    id='fld_start_date' value='{$start_date}' maxDate='{$current_date}' />
                    <input type='text' allowEdit='1' name='end_date' class='fld_date ml10'
                    id='fld_end_date' value='{$end_date}' maxDate='{$current_date}' />
                </td>
                <td>
                    <select name='company_id' class='invoiceByClientFilter ml10'>
                        <option value=''>Select Company Name</option>
                        {$dbUtil->getDropDownFromSQLCols2($db, $sqlCompany, $company_id)}
                    </select>
                </td>
                ";
            }

            if($report == 'invoiceByMonthReports' || $report == 'invoiceByYearReports') {
                $sqlCat  = $fn->getValueListSQL('projectCategory');

                $rows .= "
                <td>
                    <select name='record_type' class='projectCategoryFilter'>
                        {$dbUtil->getDropDownFromSQLCols1($db, $sqlCat, $record_type)}
                    </select>
                </td>
                ";
            }

            if($report == 'invoiceSummary') {
                $sqlCat  = $fn->getValueListSQL('projectCategory');
                $sqlCompany ="
                SELECT DISTINCT c.company_id
                               ,c.company_name
                FROM `order` o
                LEFT JOIN (company c) ON (o.company_id = c.company_id)
                WHERE c.company_id != ''
                ORDER BY company_name
                ";
        
                $rows .= "
                <td class='dateRange'>
                    <strong>Date Range</strong>
                    <input type='text' allowEdit='1' name='start_date' class='fld_date ml10'
                    id='fld_start_date' value='{$start_date}' maxDate='{$current_date}' />
                    <input type='text' allowEdit='1' name='end_date' class='fld_date ml10'
                    id='fld_end_date' value='{$end_date}' maxDate='{$current_date}' />
                </td>
                <td>
                    <select name='record_type' class='projectCategoryFilter ml10'>
                    <option value=''>Select Category</option>
                        {$dbUtil->getDropDownFromSQLCols1($db, $sqlCat, $record_type)}
                    </select>
                </td>
                <td>
                    <select name='company_id' class='invoiceByClientFilter ml10'>
                    <option value=''>Select Company Name</option>
                        {$dbUtil->getDropDownFromSQLCols2($db, $sqlCompany, $company_id)}
                    </select>
                </td>
                ";
            }

            if($report == 'detailSummaryByClient') {

                $sqlCompany ="
                SELECT c.company_id
                      ,c.company_name
                FROM company c
                WHERE category = 'Client'
                AND company_name != ''
                ORDER BY company_name ASC
                ";

                $rows .= "
                <td>
                    <select name='company_id' class='invoiceByClientFilter'>
                        <option value=''>Select Client</option>
                        {$dbUtil->getDropDownFromSQLCols2($db, $sqlCompany, $company_id)}
                    </select>
                </td>
                ";
            }

            if ($report == 'taskHoursByStaffReport'){

                $SQLStatus = $fn->getValueListSQL('taskHoursByStaffStatus');
                $rows .= "
                
                <td class='dateRange'>
                    <strong>Date Range</strong>
                    <input type='text' allowEdit='1' name='start_date' class='fld_date'
                    id='fld_start_date' value='{$start_date}' maxDate='{$current_date}' />
                    <input type='text' allowEdit='1' name='end_date' class='fld_date'
                    id='fld_end_date' value='{$end_date}' maxDate='{$current_date}' />
                </td>
                ";

            }
            if ($report == 'detailTaskSummaryReport'){

                /*$sqlmonth = "
                SELECT DISTINCT DATE_FORMAT(start_date, '%Y-%m') AS yearMonthStart
                ,DATE_FORMAT(start_date, '%b - %Y') AS monthYear
                FROM project
                WHERE DATE_FORMAT( start_date, '%b - %Y') IS NOT NULL
                ORDER BY yearMonthStart DESC
                ";
                */
                $sqlproject = "
                SELECT p.project_id as project_id,p.title as name
                FROM project p 
                ORDER BY name
                ";

                $sqlcompany = "
                SELECT c.company_id as company_id,c.company_name as company
                FROM company c 
                ORDER BY c.company_name
                ";

                $sqlStaff = "
                SELECT s.staff_id
                      ,CONCAT_WS(' ', s.first_name, s.last_name) AS staff_name
                FROM staff s
                WHERE s.team = 'In-house'   
                ORDER BY staff_name
                ";
                $rows .= "

                <td>
                    <select name='company_name' class='attendanceFilter'>
                        <option value=''>Select Company</option>
                        {$dbUtil->getDropDownFromSQLCols2($db, $sqlcompany, $company_name)}
                    </select>
                </td>
                <td>
                    <select name='project_name' class='attendanceFilter'>
                        <option value=''>Select project</option>
                        {$dbUtil->getDropDownFromSQLCols2($db, $sqlproject, $project_name)}
                    </select>
                </td>
                
                <td>
                    <select name='staff_id' class='attendanceFilter'>
                        <option value=''>Select Staff</option>
                        {$dbUtil->getDropDownFromSQLCols2($db, $sqlStaff, $staff_id)}
                    </select>
                </td>

                <td class='dateRange'>
                    <strong>Date Range</strong>
                    <input type='text' allowEdit='1' name='start_date' class='fld_date'
                    id='fld_start_date' value='{$start_date}' maxDate='{$current_date}' />
                    <input type='text' allowEdit='1' name='end_date' class='fld_date'
                    id='fld_end_date' value='{$end_date}' maxDate='{$current_date}' />
                </td>

                ";

            }

        }
        
        $text = "
        <form id='reportSearch'>
        <table class='search'>
            <tr>
                <!--<td><a href='javascript:void(0);' style='padding-right:5px;' onClick=\"javascript:$('#reportSearch').clearForm();\">reset</a></td>-->
                {$rows}
                <td>
                    <input type='hidden' name='report' value='{$report}'>
                    <input type='hidden' id='reportName' value='{$report}'>
                    <input type='submit' value='GO' class='button ml10'>
                </td>
            </tr>
        </table>
        </form>
        <script>
        </script>
        ";

        return $text;
    }

    function getDisplayReport($text){
        $fn = Zend_Registry::get('fn');
        $pager = Zend_Registry::get('pager');
        $report = $fn->getReqParam('report');

        $searchQueryString = $pager->removeQueryString(array("_spAction"));
        $exportLink = "{$searchQueryString}&_spAction=exportData&report={$report}&showHTML=0";

        if ($report == 'ageingReport'){
            $text = "
            <div>
                {$text}
            </div>
            ";
        } else {
            $text = "
            <div>
                <a href='{$exportLink}' class='exportLink button'>
                    <u>Export to Excel</u>
                </a>
                {$text}
            </div>
            ";
        }

        return $text;

        $json = array();
        $json['html'] = $text;

        return json_encode($json);
    }

    function getLiquiditySummary() {
        $fn = Zend_Registry::get('fn');

        $arr1 = $this->model->getSumByCategoryType('Cash Account');
        $arr2 = $this->model->getSumByCategoryType('Bank Account');
        $arr3 = $this->model->getSumByCategoryType('Sundry Creditor / Debtor');

        $cashTotal = abs($arr1[0] + $arr1[1]);
        $bankTotal = abs($arr2[0] + $arr2[0]);
        $recvTotal = abs($arr3[0]);
        $paybTotal = abs($arr3[1]);

        $netValue = $cashTotal + $bankTotal + $recvTotal - $paybTotal;

        $cashTotalF = number_format($cashTotal, 0);
        $bankTotalF = number_format($bankTotal, 0);
        $recvTotalF = number_format($recvTotal, 0);
        $paybTotalF = number_format($paybTotal, 0);
        $netValueF  = number_format($cashTotal + $bankTotal + $recvTotal - $paybTotal);

        $text = "
        <div id='liquiditySummary'>
            <div class='subcolumns'>
                <div class='c40l'>
                    <div class='subcl'>
                        <h1>Overall Liquidity Summary</h1>
                        <table class='thinlist'>
                            <tr>
                                <th>Cash in Hand</th>
                                <td>{$cashTotalF}</td>
                            </tr>
                            <tr>
                                <th>Cash at Banks</th>
                                <td>{$bankTotalF}</td>
                            </tr>
                            <tr>
                                <th>Receivables</th>
                                <td>{$recvTotalF}</td>
                            </tr>
                            <tr>
                                <th>Payables</th>
                                <td>{$paybTotalF}</td>
                            </tr>
                        </table>

                        <div class='netWorth'>
                            Net Worth: {$netValueF}
                        </div>
                    </div>
                </div>
                <div class='c60r'>  
                    <div class='subcr'>
                        <div class='reportLoading' id='chart_div'>
                        </div>
                        {$this->getLiquidityChart()}
                    </div>
                </div>
            </div>
        </div>
        ";

        return $text;
    }
}