<?
class CPL_Admin_Modules_EnggCrm_Attendance_View extends CP_Common_Lib_ModuleViewAbstract
{
    function getList($dataArray) {
        ...
        foreach ($dataArray as $row){
            ...
            if ($cpCfg['m.enggCrm.attendance.hasMultipleSessions'] == 1) {
            ...
            {$listObj->getListDateCell($row['staff_name'])}
        ...
        {$listObj->getListHeaderCell($cpCfg['m.enggCrm.staffFieldLabel'], 'b.staff_id')}
    ...
    function getNew(){
        ...
        $fieldset = "
        {$formObj->getDDRowBySQL($cpCfg['m.enggCrm.staffFieldLabel'], 'staff_id', $sqlStaff, $_SESSION['staff_id'])}
        ";
    ...
    function getEdit($row) {
        ...
        $fielset1  = "
        {$formObj->getDDRowBySQL($cpCfg['m.enggCrm.staffFieldLabel'],  'staff_id', $sqlStaff, $row['staff_id'], $expStf)}
        ...
        {$formObj->getTBRow('Latitude', 'latitude', $row['latitude'],$expNoEdit)}
        {$formObj->getTBRow('Longitude', 'longitude', $row['longitude'],$expNoEdit)}
    ...
    function getQuickSearch() {
        ...
        $SQL = "
        SELECT a.staff_id
              ,CONCAT_WS(' ', e.first_name, e.last_name) AS staff_name
        FROM staff a
   ...
   function getSendAttendanceReportToPM() {
        ...
        if($rowCounter > 0){
            ...
            $link .= "
            <p>
                <p><a href='http://studiouss.usoftsolutions.com/admin/index.php?_topRm=admin&module=enggCrm_attendance&_spAction=list&searchDone=1&staff_id={$staff_id}'>{$staff_name_disp}</a></p>
            </p>
            ";
