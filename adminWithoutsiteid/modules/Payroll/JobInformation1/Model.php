...
class CPL_Admin_Modules_Payroll_JobInformation_Model extends CP_Admin_Modules_Payroll_JobInformation_Model
{
    ...
    function getEditValidate() {
        ...
        if($overtime == 1){            
            $validate->validateData('over_time_rate', 'Please select overtime rate');
        }
    ...
    function getSave(){
        ...
        $fa['overtime_pay_rate'] = round($overtime_rate_final, 2);
        */
        $per_hour_rate = (($basic_pay / 30)/8);
        $overtime_rate = ($per_hour_rate * $fn->getPostParam('over_time_rate'));

        $fa['overtime_pay_rate'] = round($overtime_rate, 2);
    ...
    function getPrintEmploymentContract() {
        ...
        $templatePath = $cpCfg['cp.masterPath'].'modules/Payroll/Lib/templates/' . $template;
