<?
class CPL_Admin_Modules_Payroll_Supplier_Model extends CP_Common_Lib_ModuleModelAbstract
{
    /**
     *
     */
    ...
    function setSearchVar($linkRecType = '') {
        ...
        if ($supplier_id != "") {
            $searchVar->sqlSearchVar[] = "c.supplier_id = '{$supplier_id}'";
        } else if ($tv['record_id'] != '') {
            $searchVar->sqlSearchVar[] = "c.supplier_id = '{$tv['record_id']}'";
        } else {
            $fn->setSearchVarForLinkData($searchVar, $linkRecType, 'a.supplier_id');
            ...
            if ($tv['keyword'] != "") {
                $searchVar->sqlSearchVar[] = "(
                    ...
                    OR c.address_flat   LIKE '%{$tv['keyword']}%'  
                    OR c.address_street LIKE '%{$tv['keyword']}%'  
    ...
    function getNewValidate() {
        ...
        if ($company_name != '') {
            ...
            $company_name = str_replace('pte ltd', '', $company_name);
            $rec = $fn->getRecordByCondition('supplier', "REPLACE(lower(company_name), 'pte ltd', '') = '{$company_name}'");

            if (is_array($rec)){
                $validate->errorArray['company_name']['name'] = "company_name";
                $validate->errorArray['company_name']['msg']  = 'Supplier Name already exists';
            }
        }
    ...
    function getEditValidate() {
        ...
        $validate->validateData('company_name', 'Please enter Supplier Name');
        if ($company_name != '') {
            $count = $fn->getRecordCountBySQL('supplier', "company_name = '{$company_name}' AND supplier_id != '{$supplier_id}'");
            if ($count > 0) {
                $validate->errorArray['company_name']['name'] = "company_name";
                $validate->errorArray['company_name']['msg']  = 'Supplier Name already exists';
            }
            ...
            $company_name = strtolower($company_name);
            $company_name = str_replace('pte ltd', '', $company_name);
            $rec = $fn->getRecordByCondition('supplier', "REPLACE(lower(company_name), 'pte ltd', '') = '{$company_name}' AND supplier_id != '{$supplier_id}'");
    ...
    function getExportData($dataArray){
        ...
        $fa = array(
              'supplier_id'      => $phpExcel->getFldObj('Company ID')
     ...
     *
     */
    function getAccountSupplierPayrollContactLinkSQL($id) {
