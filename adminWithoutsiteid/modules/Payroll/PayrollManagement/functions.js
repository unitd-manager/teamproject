...
cpm.payroll.payrollManagement.init = function(){
    ...
    $(".ui-dialog-buttonset .ui-button-text").livequery('click', function (e){
        window.location.reload(true);
    });
    ...
    $(".m-payroll_payrollManagement .row_status select[name='status']").livequery('change', function(e){
        ...
        $(".m-payroll_payrollManagement .updateOt").livequery('click', function (e){
            ...
            Util.openDialogForLink.call(this, 'Update OT', 1050, 500, expObj);
        });

    $('.otHourPopup .otHours').live('change', function(){
        var parent = $(this).closest('tr');
        var ot_hours     = $('input[name=ot_hours]', parent).val();
        var rec_id = $(parent).attr('recid');
        var citizen     = $('input[name=citizen]', parent).val();
        var payRate = $('.otPayRate', parent).html();
        var url = 'index.php?module=payroll_payrollManagement&_spAction=updateOtHours&showHTML=0';

        $.get(url, {rec_id: rec_id, ot_hours: ot_hours, payRate: payRate, citizen: citizen}, function(html){
            /*if(html == undefined || html == ''){
               html = parseInt(0);
            }*/

            $('td.otAmount', parent).html(html);
        });
    });

    $('.otHourPopup .allowance1').live('change', function(){
        var parent = $(this).closest('tr');
        var allowance1     = $('input[name=allowance1]', parent).val();
        var rec_id = $(parent).attr('recid');
        var url = 'index.php?module=payroll_payrollManagement&_spAction=updateAllowance&showHTML=0';

        $.get(url, {rec_id: rec_id, allowance1: allowance1}, function(html){
        });
    });

    $('.otHourPopup .allowance2').live('change', function(){
        var parent = $(this).closest('tr');
        var allowance2     = $('input[name=allowance2]', parent).val();
        var rec_id = $(parent).attr('recid');
        var url = 'index.php?module=payroll_payrollManagement&_spAction=updateAllowance&showHTML=0';

        $.get(url, {rec_id: rec_id, allowance2: allowance2}, function(html){
        });
    });

    $('.otHourPopup .allowance3').live('change', function(){
        var parent = $(this).closest('tr');
        var allowance3     = $('input[name=allowance3]', parent).val();
        var rec_id = $(parent).attr('recid');
        var url = 'index.php?module=payroll_payrollManagement&_spAction=updateAllowance&showHTML=0';

        $.get(url, {rec_id: rec_id, allowance3: allowance3}, function(html){
        });
    });

    $('.otHourPopup .allowance4').live('change', function(){
        var parent = $(this).closest('tr');
        var allowance4     = $('input[name=allowance4]', parent).val();
        var rec_id = $(parent).attr('recid');
        var url = 'index.php?module=payroll_payrollManagement&_spAction=updateAllowance&showHTML=0';

        $.get(url, {rec_id: rec_id, allowance4: allowance4}, function(html){
        });
    });

    $('.otHourPopup .allowance5').live('change', function(){
        var parent = $(this).closest('tr');
        var allowance5     = $('input[name=allowance5]', parent).val();
        var rec_id = $(parent).attr('recid');
        var url = 'index.php?module=payroll_payrollManagement&_spAction=updateAllowance&showHTML=0';

        $.get(url, {rec_id: rec_id, allowance5: allowance5}, function(html){
        });
    });

    $('.otHourPopup .deduction1').live('change', function(){
        var parent = $(this).closest('tr');
        var deduction1     = $('input[name=deduction1]', parent).val();
        var rec_id = $(parent).attr('recid');
        var url = 'index.php?module=payroll_payrollManagement&_spAction=updateAllowance&showHTML=0';

        $.get(url, {rec_id: rec_id, deduction1: deduction1}, function(html){
        });
    });

    $('.otHourPopup .deduction2').live('change', function(){
        var parent = $(this).closest('tr');
        var deduction2     = $('input[name=deduction2]', parent).val();
        var rec_id = $(parent).attr('recid');
        var url = 'index.php?module=payroll_payrollManagement&_spAction=updateAllowance&showHTML=0';

        $.get(url, {rec_id: rec_id, deduction2: deduction2}, function(html){
        });
    });

    $('.otHourPopup .deduction3').live('change', function(){
        var parent = $(this).closest('tr');
        var deduction3     = $('input[name=deduction3]', parent).val();
        var rec_id = $(parent).attr('recid');
        var url = 'index.php?module=payroll_payrollManagement&_spAction=updateAllowance&showHTML=0';

        $.get(url, {rec_id: rec_id, deduction3: deduction3}, function(html){
        });
    });
    ...
    var populateAmount = {
        populateDeductionAmount: function(){
            ...
            $('.m-payroll_payrollManagement .totalDeduction').html(totalAmount.toFixed(3));
            ...
            $('.m-payroll_payrollManagement .netTotalPayrollMgmt').html(netTotal.toFixed(3));
        },
        ...
        populateEarningsAmount: function(){
            ...
            //$('.m-payroll_payrollManagement input[id=fld_ot_amount]').val(ot_amount.toFixed(3));
            $('.m-payroll_payrollManagement .ot_amount').html(ot_amount.toFixed(3));
            $('.m-payroll_payrollManagement .grossPay').html(totalAmount.toFixed(3));
