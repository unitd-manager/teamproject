<?
class CPL_Admin_Modules_Payroll_PayrollManagement_View extends CP_Admin_Modules_Payroll_PayrollManagement_View
{
    /**
     *
     */
    function getList($dataArray){
        ...
        foreach ($dataArray as $row){
            ...
            if($count == 0){
                 $formActionUpdateOt = "index.php?module=payroll_payrollManagement&_spAction=updateOtForm&showHTML=0";
                $updateOtBtn="
                <div class='float_left'>
                    <a href='{$formActionUpdateOt}' class='button updateOt'>Update OT</a>
                </div>
                ";
            }
    ...
    function getEdit($row){
        ...
        $urlPrintTimesheetPdf = "index.php?_topRm=payroll&module=payroll_payrollManagement&_spAction=timesheetPrintPdf&payroll_management_id={$row['payroll_management_id']}&employee_id={$row['employee_id']}&showHTML=0";
        ...
        <div class='linkPortalWrapper'>
            ...
            <div>
                ...
                <div class='float_right mb10'><a href='{$urlPrintTimesheetPdf}' target='_blank' class='button'>Print Timesheet</a></div>
    ...
    function getUpdateOtForm() {
        ...
        $appendsqlEmp = '';
        if ($employee_id != "") {
            $appendsqlEmp = "AND pm.employee_id = '{$employee_id}'";
        }

        $appendsqlStatus = '';
        if ($status != "") {
            $appendsqlStatus = "AND pm.status = '{$status}'";
        }
        ...
        SELECT pm.*
              ...
              ,e.citizen
              ,e.fin_no
              ,(SELECT SUM(et.employee_ot_hours) FROM employee_timesheet et
                      WHERE et.employee_id = pm.employee_id
                      AND DATE_FORMAT(et.date, '%Y') = '{$year}'
                      AND DATE_FORMAT(et.date, '%m') = '{$month}') AS employee_ot_hours
              ,(SELECT SUM(et.employee_ph_hours) FROM employee_timesheet et
                      WHERE et.employee_id = pm.employee_id
                      AND DATE_FORMAT(et.date, '%Y') = '{$year}'
                      AND DATE_FORMAT(et.date, '%m') = '{$month}') AS employee_ph_hours
        FROM payroll_management pm
        LEFT JOIN (employee e) ON (e.employee_id = pm.employee_id)
        WHERE pm.payroll_year = '{$year}'
          AND pm.payroll_month = '{$month}'
          AND e.citizen NOT IN ('Citizen', 'PR')
          {$appendsqlStatus}
          {$appendsqlEmp}
        ...
        while ($row = $db->sql_fetchrow($result)) {
            $rows .= "
            <tr recid='{$row['payroll_management_id']}'>
                <td class='txtCenter'>{$count}</td>
                <td>{$row['employee_name']} ({$row['fin_no']})</td>
                <td>{$row['employee_ot_hours']} / {$row['employee_ph_hours']}</td>
                <td class='otPayRate'>{$row['overtime_pay_rate']}</td>
                <td width='5%'><input type='text' value='{$row['ot_hours']}' class='text otHours' name='ot_hours'></td>
                <td class='otAmount' align='right'>{$row['ot_amount']}</td>
                <td width='5%'><input type='text' value='{$row['allowance1']}' class='text allowance1' name='allowance1'></td>
                <td width='5%'><input type='text' value='{$row['allowance2']}' class='text allowance2' name='allowance2'></td>
                <td width='5%'><input type='text' value='{$row['allowance3']}' class='text allowance3' name='allowance3'></td>
                <td width='5%'><input type='text' value='{$row['allowance4']}' class='text allowance4' name='allowance4'></td>
                <td width='5%'><input type='text' value='{$row['allowance5']}' class='text allowance5' name='allowance5'></td>
                <td width='5%'><input type='text' value='{$row['deduction1']}' class='text deduction1' name='deduction1'></td>
                <td width='5%'><input type='text' value='{$row['deduction2']}' class='text deduction2' name='deduction2'></td>
                <td width='5%'><input type='text' value='{$row['deduction3']}' class='text deduction3' name='deduction3'></td>
                <input type='hidden' name='citizen' value='{$row['citizen']}' />
            </tr>
            ";
            $count++;
        }
        ...
        $clientRows = "
        <table class='thinlist otHourPopup'><div class='otIntroText'>Please note data is auto saved</div>
            <thead>
                ...
                <th colspan='5' class='txtCenter'>Allowances
                    <table border='1'>
                        <tr>
                            <th style='width:5%'>{$cpCfg['m.jobInformation.allowance1Lbl']}</th>
                            <th style='width:5%'>{$cpCfg['m.jobInformation.allowance2Lbl']}</th>
                            <th style='width:75px;'>{$cpCfg['m.jobInformation.allowance3Lbl']}</th>
                            <th style='width:5%'>{$cpCfg['m.jobInformation.allowance4Lbl']}</th>
                            <th>{$cpCfg['m.jobInformation.allowance5Lbl']}</th>
                        </tr>
                    </table>
                </th>
                <th colspan='3' class='txtCenter'>Deductions
                    <table border='1'>
                        <tr>
                            <th style='width:5%'>{$cpCfg['m.jobInformation.deduction1Lbl']}</th>
                            <th style='width:60%'>{$cpCfg['m.jobInformation.deduction2Lbl']}</th>
                            <th>{$cpCfg['m.jobInformation.deduction3Lbl']}</th>
                        </tr>
                    </table>
                </th>
            </thead>
    ...
    function getTimeSheetPrintPdf() {
        ...
        $pmRec = $fn->getRecordRowByID('payroll_management', 'payroll_management_id', $payroll_management_id);
        $empRec = $fn->getRecordRowByID('employee', 'employee_id', $employee_id);
        $month = $pmRec['payroll_month'];
        $year = $pmRec['payroll_year'];
        ...
        AND DATE_FORMAT(et.date, '%Y-%m') = '{$pmRec['payroll_year']}-{$pmRec['payroll_month']}'
        ";
        ...
        for ($j=0; $j < $count2; $j++) {
            ...
            //$dayNameDate = substr($dayNameDate, 0, 2);

            $dayNameDate = strtoupper($dayNameDate);
            $dayContHeader .= '<th width="3%"><b>'. $dayHeaderCount .'</b></th>';

            $dayHeaderCount++;
        }

        if ($empRec['citizen'] == 'Citizen' || $empRec['citizen'] == 'PR') {
            $ic_no = $empRec['nric_no'];
        } else {
            $ic_no = $empRec['fin_no'];
        }

        $total_allowance = $pmRec['allowance1'] + $pmRec['allowance2'] + $pmRec['allowance3'] + $pmRec['allowance4'] + $pmRec['allowance5'];
        $total_allowance_display = number_format($total_allowance, 2);

        $total_deduction = round($pmRec['cpf_employee'], 2) + $pmRec['sdl'] + $pmRec['loan_amount'] + $pmRec['income_tax_amount'] + $pmRec['pay_cdac'] + $pmRec['pay_sinda'] + $pmRec['pay_mbmf'] + $pmRec['pay_eucf'] + $pmRec['deduction1'] + $pmRec['deduction2'] + $pmRec['deduction3'] + $pmRec['loan_deduction'];
        $total_deduction_display = number_format($total_deduction, 2);

        $gross_pay = $pmRec['basic_pay'] + $pmRec['ot_amount'] + $pmRec['commission'] + $pmRec['allowance1'] + $pmRec['allowance2'] + $pmRec['allowance3'] + $pmRec['allowance4'] + $pmRec['allowance5'];
        
        $net_total = $gross_pay - $total_deduction + $pmRec['reimbursement'] + $pmRec['director_fee'];
        $net_total = number_format($net_total, 2);

        $tbl1 = '
        <table border="0" width="100%">
            <tr>
                <td width="25%" align="left">Month & Year : <b>'. strtoupper($month_name) . ' ' . $year.'</b></td>
                <td width="50%" align="left">Name : <b>'. $empRec['first_name'] .'</b></td>
                <td width="25%" align="left">Fin : <b>'. $ic_no .'</b></td>
            </tr>
        </table>
        <table border="0" width="100%">
            <br/>
            <br/>
            <tr>
                <td width="25%" align="left">Basic Pay</td>
                <td width="25%" align="left">Total Allowance</td>
                <td width="25%" align="left">Total Deduction</td>
                <td width="25%" align="left">Total Amount</td>
            </tr>
            <tr>
                <td width="25%" align="left"><b>'. $pmRec['basic_pay'] .'</b></td>
                <td width="25%" align="left"><b>'. $total_allowance_display .'</b></td>
                <td width="25%" align="left"><b>'. $total_deduction_display .'</b></td>
                <td width="25%" align="left"><b>'. $net_total .'</b></td>
            </tr>
            <br/>
            <br/>
        </table>
        ...
        while ($row = $db->sql_fetchrow($resultTimesheet)) {
            ...
            for ($j= 0; $j < $count2; $j++) {
                ...
                $SQLTimesheetDays = "
                SELECT SUM(employee_hours) AS employee_hours
                      ,SUM(employee_ot_hours) AS employee_ot_hours
                      ,SUM(employee_ph_hours) AS employee_ph_hours
                      ,SUM(hourly_rate) AS hourly_rate
                      ,SUM(ot_hourly_rate) AS ot_hourly_rate
                      ,SUM(ph_hourly_rate) AS ph_hourly_rate
                ...
                if($timesheet_OT_days == 0){
                    $timesheet_OT_days = '';
                }
                if($timesheet_PH_days == 0){
                    $timesheet_PH_days = '';
                }

                $dayContRow   .= '<td width="3%" align="center" style="line-height:200%">'. $timesheet_days . '</td>';
                $dayContOTRow .= '<td width="3%" align="center" style="line-height:200%">'. $timesheet_OT_days . '</td>';
                $dayContPHRow .= '<td width="3%" align="center" style="line-height:200%">'. $timesheet_PH_days . '</td>';
            ...
            $overall_claim_amount = $total_claim_for_employee + $total_claim_for_employee_ot + $total_claim_for_employee_ph;
            ...
            $overall_total_of_all_employees += $total_amount_per_employee;
              ...
              $tbl2 = $tbl2.'
                      <tr align="center">
                          <td width="5%" style="line-height:200%">NH</td>'.
                          $dayContRow.'
                      </tr>
                      <tr>
                          <td width="5%" height="20px" align="center" style="line-height:200%">OT</td>'.
                          $dayContOTRow.'
                      </tr>
                      <tr>
                          <td width="5%" height="20px" align="center" style="line-height:200%">PH</td>'.
                          $dayContPHRow.'
                      </tr>
                      ';
        ...
        $download_title = $cpCfg['cp.companyName'] . '-' . $rowProject['first_name'] . '-Employee-Timesheet.pdf';
