<?
class CPL_Admin_Modules_Payroll_JobInformation_View extends CP_Admin_Modules_Payroll_JobInformation_View
{
    /**
     *
     */
    function getList($dataArray){
        ...
        foreach ($dataArray as $row){
            ...
            if ($row['hourly_pay_rate'] > '0') {
                $pay = $row['hourly_pay_rate'];   
            } else {
                $pay = $row['basic_pay'];   
            }
    ...
    function getEdit($row){
        ...
        $expPaymentType = array('hideFirstOption' => 1);
        ...
        */
        ...
        $otRateArr = array('useKey' => 1, 'hideFirstOption' => 1);
        $otRateArray = array(
            "1.5" => "1.5"
           ,"2.0" => "2.0"
        );

        $sunPhRateArr = array('useKey' => 1, 'hideFirstOption' => 1);
        $sunPhRateArray = array(
            "1.5" => "1.5"
           ,"2.0" => "2.0"
        );
        ...
        $urlContract = "index.php?module=payroll_jobInformation&_spAction=printEmploymentContract&job_information_id={$row['job_information_id']}&showHTML=0";
         $urlImplement = "index.php?module=payroll_jobInformation&_spAction=printImplement&job_information_id={$row['job_information_id']}&showHTML=0";
       $print ='';
       /*
       if ($row['citizen'] == 'Citizen' || $row['citizen'] == 'PR'){
        $print = "
        <div class='floatbox editTopButtonActionDiv'>
            <div class='float_left m4'>
                <a href='{$urlImplement}' class='btn btn-info button ml10'>
                    Print Increment
                </a>
        ...
        */
        ...
        $expArrivalDate     = array('maxDate' => date('Y-m-d'));
        $expResignationDate = array('maxDate' => date('Y-m-d'));
        $expTerminationDate = array('maxDate' => date('Y-m-d'));
        $dep_date = date('Y-m-d', strtotime(date('Y-m-d'). ' + 30 days'));
        $expDepartureDate   = array('maxDate' => $dep_date);
                ...
                <div class='linkPortalDataWrapper'>
                    <table class='thinlist'>
                        <tbody>
                            ...
                            <tr>
                                <td>{$formObj->getDateRow('Employment Start/<br/>Commencement Date', 'act_join_date', $row['act_join_date'], $expStartDate)}</td>
                            ...
                            <tr>
                                ...
                                <td>{$formObj->getDateRow('Joined/Arrival Date', 'join_date', $row['join_date'], $expArrivalDate)}</td>
                            ...
                            <tr>
                                <td>{$formObj->getDDRowByArr('Salary Period', 'payment_type', $paymenttypeArray, $row['payment_type'], $expPaymentType)}</td>
                            ...
                            <tr>
                                ...
                                <td>{$formObj->getTBRow('Hourly Rate *', 'hourly_pay_rate', $row['hourly_pay_rate'])}</td>
                                <td colspan='3'></td>
                            </tr>
                            <tr>
                                <td>{$formObj->getYesNoRRow('Overtime Applicable', 'overtime', $row['overtime'])}</td>
                                <td class='overTimeRate {$classOt}'>{$formObj->getDDRowByArr('Over Time Rate', 'over_time_rate', $otRateArray, $row['over_time_rate'], $otRateArr)}</td>
                                <td class='overTimeRate {$classOt}'>{$formObj->getTBRow('Overtime Pay Rate/ Hour', 'overtime_pay_rate', $row['overtime_pay_rate'], $expNoEdit)}</td>
                                <td class='sunPhRate {$classOt}'>{$formObj->getDDRowByArr('Sun/PH Rate', 'sun_ph_rate', $sunPhRateArray, $row['sun_ph_rate'], $sunPhRateArr)}</td>
                                <td class='overTimeRate {$classOt}'>{$formObj->getTBRow('Sun/PH Rate/ Hour', 'sun_ph_pay_rate', $row['sun_ph_pay_rate'], $expNoEdit)}</td>
                            </tr>
                            <tr>
                            ...
                            <tr>
                                ...
                                <td>{$formObj->getTBRow('Levy', 'levy_amount', $row['levy_amount'])}</td>
                                <td></td>
                            </tr>
                            ...
                            <tr>
                                ...
                                <td>{$formObj->getDateRow('Date of Resignation Notice', 'resignation_notice_date', $row['resignation_notice_date'], $expResignationDate)}</td>
                                <td>{$formObj->getDateRow('Termination/Cessation Date', 'termination_date', $row['termination_date'], $expTerminationDate)}</td>
                                <td>{$formObj->getTARow('Reason for Termination', 'termination_reason', $row['termination_reason'])}</td>
                                <td>{$formObj->getDateRow('Departure Date', 'departure_date', $row['departure_date'], $expDepartureDate)}</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
