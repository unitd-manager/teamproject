<?
class CPL_Admin_Widgets_EnggCrm_ProjectDeliveryOrderNote_Model extends CP_Common_Lib_WidgetModelAbstract
{
    /**
     *
     */
    ...
    function getDataArray() {
        ...
        $dataArray = $modelHelper->getWidgetDataArray($this->controller, 'enggCrm_projectDeliveryOrderNote');
    ...
    function getAddJobFormSubmit() {
        ...
        $fa['delivery_note_date']  = date('Y-m-d');
      
        $fa = $fn->addModificationDetailsToFieldsArray($fa, 'delivery_note');

        $delivery_note_id = $fn->addRecord($fa, 'delivery_note');       
 ...
 $job_order_id  = $fn->getReqParam('job_order_id');
    ...
    function getUpdateJobOrderCode() {
        ...
        $poCode = $fn->getSettingsValueByKey("nextdeliveryOrdernOteCode");

        $POCode = $fn->getSettingsValueByKey('deliveryOrderNoteCodePrefix') . ' '. $poCode . ' - ' . date("y");
    ...
    function getEditForQuoteValidate() {
     ...
     function getEditForJobSubmit() {
        ...
        $delivery_note_date       = $fn->getPostParam('delivery_note_date');   
                $delivery_note_code       = $fn->getPostParam('delivery_note_code');   
        $date       = $fn->getPostParam('delivery_date');
        $ref_po       = $fn->getPostParam('ref_po');
        $note       = $fn->getPostParam('note');
        $received_date       = $fn->getPostParam('received_date');   
        $received_by       = $fn->getPostParam('received_by');       
       ...
       if (!$this->getEditForQuoteValidate()){
        ...
        $fa = array();
        $fa['delivery_note_date']     = $delivery_note_date; 
                $fa['delivery_note_code']     = $delivery_note_code;               
        $fa['ref_po']     = $ref_po;    
        $fa['received_by']     = $received_by;
        $fa['note']     = $note;
        $fa['received_date']     = $received_date;


        $whereCondition = "WHERE delivery_note_id = {$delivery_note_id}";
        $SQL = $dbUtil->getUpdateSQLStringFromArray($fa, "delivery_note", $whereCondition);
        $db->sql_query($SQL);
