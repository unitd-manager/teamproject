$(function(){
    ...
    $(".m-enggCrm_project a.addMultipleWarranty").livequery('click', function (e){
        var title = "Add Warranty";
        var project_id = $(this).attr('project_id');
        var url = 'index.php?widget=enggCrm_projectWarranty&_spAction=addMultipleMaterials'
        ...
        var exp = {
           ...
           ,callbackOnSuccess: function(){
                ...
                var mgsalert = 'Warranty added successfully!';
                var n = noty({
                    text: mgsalert,
                    type: 'confirm',
                    dismissQueue: true,
                ...
                projectWarranty.reloadMaterialUsedPortal(project_id);
            }
        };
        Util.openFormInDialog.call(this, 'addMultipleWarrantyForm', title, 1200, 500, exp);
    });

    /* Adding row in new material */
    $("a.addMaterialRow").livequery('click', function (e){
        var url = 'index.php?widget=enggCrm_projectWarranty&_spAction=addMaterialRecord'
                + '&showHTML=0';
    ...
    $('.cancelMaterial').livequery('click', function (e){
        ...
        if (!confirm(msg)){
            ...
            var url = 'index.php?widget=enggCrm_projectWarranty&_spAction=cancelMaterial&showHTML=0';
    ...
    $('.virescoFactory').livequery('click', function (e){
        Util.showProgressInd();
        var url = 'index.php?widget=enggCrm_projectWarranty&_spAction=updateVirescoFactory&showHTML=0';
        var project_materials_id = $(this).attr('project_materials_id');
    ...
    $(".addMultipleMaterialsForm input.materialTitleFull").livequery(projectWarranty.poProductTitle);
    $("#editForMaterialUsed input[name=title]").livequery(projectWarranty.poProductTitleEdit);
    ...
    /* MATERIAL USED EDIT */
    $("a.editForWarranty").livequery('click', function (e){
        ...
        Util.openFormInDialog.call(this, 'editForWarranty', title, 1200, 500, expObj);
    });
    ...
    $('.viewAllReturnedMaterialHistory').livequery('click', function (){
        ...
        if(project_materials_id != "") {
            var url = "index.php?widget=enggCrm_projectWarranty&_spAction=returnedMaterialHistory&project_materials_id="+project_materials_id+"&showHTML=0";
            var exp = {
                url: url
            };
    ...
    $(".creationModificationMU").livequery('click', function (e){
        ...
        var url = "index.php?widget=enggCrm_projectWarranty&_spAction=creationModificationMU&project_materials_id="+project_materials_id+"&showHTML=0";
        var exp = {
            url: url
        };
...
var projectWarranty = {
    poProductTitle: function() {
        ...
        $(titleObj).autocomplete({
            source: function(request, response) {
                $.ajax({
                  url: 'index.php?widget=enggCrm_projectWarranty&_spAction=searchProductTitle&showHTML=0',
                  dataType: "json",
                  data: request,                    
    ...
    reloadMaterialUsedPortal: function(project_id){
        var url = 'index.php?widget=enggCrm_projectWarranty&_spAction=ProjectWarrantyPortal&showHTML=0';
        Util.showProgressInd();
        $.get(url, {project_id: project_id}, function(html){
            Util.hideProgressInd();
            $('#warrantyLinkPortal').html(html);
        });
    },
}