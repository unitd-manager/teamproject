<?
class CPL_Admin_Widgets_Project_InvoiceSummary_Model extends CP_Admin_Widgets_Project_InvoiceSummary_Model
{
    ...
    function getTotalOutstandingInvoices() {
        ...
        $SQL = "
        SELECT CONCAT('{$this->getCurPfx()}', FORMAT(SUM(invoice_amount{$this->getFldSfx()} + ((i.invoice_amount * i.gst_percentage) / 100)), 0)) AS total
        FROM invoice i
    ...
    function getTotalInvoicesPaidThisMonth() {
        ...
        SELECT CONCAT('{$this->getCurPfx()}', FORMAT(SUM(invoice_amount{$this->getFldSfx()} + ((i.invoice_amount * i.gst_percentage) / 100)), 0)) AS total
        FROM invoice i
        WHERE DATE_FORMAT(i.invoice_paid_date, '%Y-%m') = '{$thisMonth}'
          AND LOWER(i.status) = 'paid' 
          AND LOWER(i.status) != 'cancelled' 
        ";
    ...
    function getTotalInvoicesPaidLastMonth() {
        ...
        SELECT CONCAT('{$this->getCurPfx()}', FORMAT(SUM(invoice_amount{$this->getFldSfx()} + ((i.invoice_amount * i.gst_percentage) / 100)), 0)) AS total
        FROM invoice i
        WHERE DATE_FORMAT(i.invoice_paid_date, '%Y-%m') = '{$lastMonth}'
          AND LOWER(i.status) = 'paid' 
          AND LOWER(i.status) != 'cancelled' 
        ";
