<?
class CPL_Admin_Widgets_EnggCrm_ExpenseSummaryReport_Model extends CP_Common_Lib_WidgetModelAbstract
{
    /**
     *
     */
    ...
    function setSearchVar() {
        ...
        if ($status != '') {
            if($status == 'Due'){
                $searchVar->sqlSearchVar[] = "(e.payment_status = 'Due' OR e.payment_status = 'Partial Payment')";                
            } else {
                $searchVar->sqlSearchVar[] = "e.payment_status = '{$status}'";
            }
        }

        if ($start_date != '' && $end_date == '') {
            $searchVar->sqlSearchVar[] = "((e.date BETWEEN '{$start_date}' AND '{$current_date}') OR (e.date BETWEEN '{$start_date}' AND '{$current_date}'))";
        } else if ($start_date == '' && $end_date != ''){
            $start_date = date('Y') . '-' . date('m') . '-01';
            
            $searchVar->sqlSearchVar[] = "((e.date BETWEEN '{$start_date}' AND '{$end_date}') OR (e.date BETWEEN '{$start_date}' AND '{$end_date}'))";
        } else if ($start_date != '' && $end_date != '') {
            $searchVar->sqlSearchVar[] = "((e.date BETWEEN '{$start_date}' AND '{$end_date}') OR (e.date BETWEEN '{$start_date}' AND '{$end_date}'))";
        } else {
            $start_date = date('Y') . '-' . date('m') . '-01';
