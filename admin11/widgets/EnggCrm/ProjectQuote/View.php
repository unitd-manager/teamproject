<?
class CPL_Admin_Widgets_EnggCrm_ProjectQuote_View extends CP_Common_Lib_WidgetViewAbstract
{
    /**
     *
     */
    ...
    function getAddQuoteFormListView($opportunity_id = '', $project_id = '', $category = '') {
        ...
        while ($row = $db->sql_fetchrow($result)) {
          if($row['drawing_nos'] == 1) {
              ...
              $quoteActions    = '';
              $editForQuote    = "index.php?widget=enggCrm_projectQuote&_spAction=editForQuote&project_id={$project_id}&quote_id={$row['quote_id']}&showHTML=0";
              $urlPrintLinkPdf = "index.php?widget=enggCrm_projectQuote&_spAction=printDrawingQuotePdf&opportunity_id={$opportunity_id}&quote_id={$row['quote_id']}&showHTML=0";

              $formActionGroupForQuoteLineItem = "index.php?widget=enggCrm_projectQuote&_spAction=addLineItemForQuoteForm&opportunity_id={$opportunity_id}&quote_id={$row['quote_id']}&project_id={$project_id}&showHTML=0";
              ...
              if ($row['quote_status'] == 'Awarded' || $row['quote_status'] == 'Order Raised') {
                  ...
                  <div class='float_box clearfix'>
                      <div class='float_left'>
                          <a class='editForQuote' href='{$editForQuote}' title='Edit Quote'><img src='{$edit_image}' class='icon'></a>
                      </div>
                      <div class='printLink'>
                  ...
                  <div class='float_box clearfix'>
                      ...
                      <div class='float_left'>
                          <a  project_id={$row['project_id']} quote_id = {$row['quote_id']} class='addMultipleLineItem' title='Add Line Item'><img src='{$add_image}' class='icon'></a>
                      </div>
                  </div>
                  ";
              }
              ...
              $quote_amount = number_format($row['total_amount'] - $row['discount'], 3);
              $discount     = number_format($row['discount'], 3);

              $rows .= "
              <tbody class='quoteDetailRow'>
                  <tr class='addQuoteRow {$confirmedQuoteStatus} {$cancelledQuoteStatus}'>
                      <td data-label='Revision'>R-{$row['revision']}</td>
                      ...
                      <td data-label='Discount' class='txtRight'>{$discount}</td>
                      <td data-label='Amount' class='txtRight' colspan='2'>{$quote_amount}</td>
              ...
              while ($rowQuoteItems = $db->sql_fetchrow($resultForQuoteItems)) {
                  ...
                  if($rowQuoteItems['unit_price'] > 0 && $rowQuoteItems['quantity'] > 0) {
                      $subtotal_amount = round($rowQuoteItems['quantity'] * $rowQuoteItems['unit_price'], 3);
                  } elseif ($rowQuoteItems['unit_price'] > 0 && $rowQuoteItems['quantity'] == 0) {
                      $subtotal_amount = round($rowQuoteItems['unit_price'], 3);
                  } elseif ($rowQuoteItems['amount'] > 0) {
                      $subtotal_amount = round($rowQuoteItems['amount'], 3);
                  }
           ...
           if($numRowsForQuoteItems > 0) {
              ...
              $urlPrintLinkPdf  = "index.php?widget=enggCrm_projectQuote&_spAction=printLinkForPdfold&opportunity_id={$opportunity_id}&quote_id={$row['quote_id']}&showHTML=0";
            }else{
               $urlPrintLinkPdf  = "index.php?widget=enggCrm_projectQuote&_spAction=printLinkForPdf&opportunity_id={$opportunity_id}&quote_id={$row['quote_id']}&showHTML=0";
              ...
              if ($row['quote_status'] == 'Awarded' || $row['quote_status'] == 'Order Raised') {
                  ...
                  <div class='float_box clearfix'>
                      <!--<div class='duplicateQuote'>
                          <a  class='duplicateQuote'  quote_id='{$row['quote_id']}' project_id='{$row['project_id']}' quote_items_id='{$rowQuoteItems['quote_items_id']}'>Duplicate</a>
                      </div>-->
                  </div>
                  ";
              }
              ...
              $quote_amount = number_format($totalvalue - $row['discount'], 3);
          ...
          if ($orderRows > 0) {
              ...
              $urlOrderRecord = "index.php?widget=enggCrm_projectQuote&_action=edit&order_id={$orderRec['order_id']}";
              $gotoProjectBtn = "
          ...
          if ($quoteConfirmCount){
              ...
              $rowQuote = $fn->getRecordByCondition('quote', "project_id = '{$project_id}' AND quote_status = 'Awarded'", 'quote_id ASC');
          ...
          if($numRows == 0) {
            ...
            <div class='float_box mt10 mb10'>
              <a id='addQuoteProject' class='btn btn-primary' project_id='{$project_id}'>Add Quote</a>
            </div>
            ";
          }
                    ...
                    <thead>
                        <tr>
                            <th colspan='9' align='left' class='rightPanelHeading'>
                              Quotations
                              <a class='viewQuoteLog btn btn-primary ml20' project_id='{$project_id}'>View Quote Log</a>
                            </th>
                        </tr>
                        <tr>
                            ...
                            <th scope='col' class='txtRight'>Discount</th>
                            {$ChangeHead}
    ...
    function getAddLineItemForQuoteListView($opportunity_id, $project_id, $quote_id, $category) {
        ...
        while ($row = $db->sql_fetchrow($result)) {
            ...
            $editForLineItem = "index.php?widget=enggCrm_projectQuote&_spAction=editLineItem&opportunity_id={$opportunity_id}&quote_id={$row['quote_id']}&quote_items_id={$row['quote_items_id']}&project_id={$project_id}&showHTML=0";
            ...
            if($row['drawing_nos'] == "" || $row['drawing_nos'] == 0) {
              ...
              $unit_price   = number_format($row['unit_price'], 3);
    ...
    function getAddMultipleLineItem() {
        ...
        if($rowQuote['drawing_nos'] == 1) {
            ...
            $newRow = "
            <a  class='addDrawingRow btn btn-primary mb10' project_id='{$project_id}'>Add Line Item</a>
            ";
        ...
        $formAction = "index.php?widget=enggCrm_projectQuote&_spAction=addMultipleLineItemSubmit&showHTML=0";
    ...
    function getEditForQuote() {
        ...
        $formActionEditForQuote = "index.php?widget=enggCrm_projectQuote&_spAction=editForQuoteSubmit&lnkRoom={$tv['lnkRoom']}&quote_id={$rowQuote['quote_id']}&project_id={$project_id}&showHTML=0";
        ...
        if ($rowProject['category'] == 'Manpower Supply'){
            $introText = "{$formObj->getTextAreaRow('Intro Text', 'quote_intro_text_1',$rowQuote['quote_intro_text_1'])}";
            $invoices_payment_terms = "{$formObj->getTextAreaRow('Invoices & Payment Terms', 'invoices_payment_terms',$rowQuote['invoices_payment_terms'])}";
            $responsibility = "{$formObj->getTextAreaRow('Responsibility', 'responsibility',$rowQuote['responsibility'])}";
            $manPowerTermsInQuote = "
            <tr>
                <th colspan='2' class='txtCenter highlight'>WORKING HOURS - NORMAL DAY</th>
            </tr>
            <tr>
                <td>{$formObj->getTARow('Monday to Friday', 'monday_to_friday_normal_timing',$rowQuote['monday_to_friday_normal_timing'])}</td>
                <td>{$formObj->getTARow('Saturday', 'saturday_normal_timing',$rowQuote['saturday_normal_timing'])}</td>
            </tr>
            <tr>
                <th colspan='2' class='txtCenter highlight'>WORKING HOURS - NORMAL OT</th>
            </tr>
            <tr>
                <td>{$formObj->getTARow('Monday to Friday', 'monday_to_friday_ot_timing',$rowQuote['monday_to_friday_ot_timing'])}</td>
                <td>{$formObj->getTARow('Saturday', 'saturday_ot_timing',$rowQuote['saturday_ot_timing'])}</td>
            </tr>
            <tr>
                <th colspan='2' class='txtCenter highlight'>SUNDAY / PUBLIC HOLIDAY</th>
            </tr>
            <tr>
                <td>{$formObj->getTARow('Sunday/<br/>Public Holiday', 'sunday_and_publicholiday_ot_timing',$rowQuote['sunday_and_publicholiday_ot_timing'])}</td>
            </tr>
            <tr>
                <td colspan='2'>{$invoices_payment_terms}</td>
            </tr>
            <tr>
        ...
        <form id='editForQuote' class='yform columnar editQuote' method='post' action='{$formActionEditForQuote}'>
            <fieldset>
                <table width='100%'>
                    ...
                    <tr>
                        <td class='drawingQuoteFields {$hideDrawingQuote}' colspan='2'>{$formObj->getTBRow('Our Reference', 'our_reference', $rowQuote['our_reference'])}</td>
                        <td class='drawingQuoteFields {$hideDrawingQuote}'>{$formObj->getTBRow('Total Amount', 'total_amount', $rowQuote['total_amount'])}</td>
                    
                    </tr>
                    <tr>
                        <td colspan='4'>                          
                        <label>Terms Of Work</label>
                        {$formObj->getHTMLEditor('Terms & Condition', 'condition', $rowQuote['condition'])}</td>
                    </tr>
                     <tr>
                        <td colspan='4'>                          
                        <label>Note</label>
                ...
                <input type='hidden' name='project_id' value='{$project_id}' />
                <input type='hidden' name='quote_id' value='{$quote_id}' />
                <input type='hidden' name='revision' value='{$rowQuote['revision']}' />
    ...
    function getEditLineItem() {
        ...
        $formActionEditLineItem = "index.php?widget=enggCrm_projectQuote&_spAction=editLineItemSubmit&lnkRoom={$tv['lnkRoom']}&quote_items_id={$quote_items_id}&opportunity_id={$opportunity_id}&showHTML=0";
    ...
    function getPrintLinkForPdf() {
        ...
        if($company['discount'] && $_SESSION['userGroupName'] != 'Projects') {
            $tbl3 = $tbl3.'<tr>
                                ...
                                <td width="7%"  style="border-left:1px solid #000;border-right:1px solid #000;"></td>
                                <td width="12%" style="border-left:1px solid #000;border-right:1px solid #000;"></td>
                                <td width="13%" style="font-size:10px;border-right:1px solid #000;" align="right"><br/><br/>-'.number_format($company['discount'], 3).'</td>
                            </tr>
                    ';
        }
        ...
        if($company['gst'] == 1) {
            $tbl3 = $tbl3.'<tr>
                              ...
                              <td align="right" style="font-size:10px; font-weight:bold; border-top:1px solid #000;border-right:1px solid #000;">'.number_format($subtotalValue - $company['discount'],3).'</td>
                          </tr>
                          <tr>
                              ...
                              <td align="right" style="font-size:10px; font-weight:bold;border-right:1px solid #000;">'.number_format($gstvalue, 3).'</td>
                           </tr>
                           <tr>
                              <td align="right" colspan="2" style="font-size:10px; font-weight:bold; border-right:1px solid #000;"></td>
                              ...
                              <td align="right" style="font-size:10px; font-weight:bold;border-right:1px solid #000; border-bottom:1px solid #000;">'.number_format($totalvalue, 3).'</td>
                           </tr>
                          </tbody>
                        </table>';
        } else {
            $tbl3 = $tbl3.'<tr>
                              ...
                              <td align="right" style="font-size:10px; font-weight:bold; border-top:1px solid #000; border-right:1px solid #000; border-bottom:1px solid #000;">'.number_format($totalvalue, 3).'</td>
                           </tr>
                          </tbody>
                        </table>';
        }
        ...
        <table border="0" width="100%" cellpadding="3">
            ...
            <tr>
                <td width="100%" style="font-size:10px;font-weight:bold;">Mohamed Ibrahim<br/>
    ...
    function getPrintLinkForPdfold() {
        ...
        $amount_in_words = $fn->getConvertNumber($totalvalue);
            ...
            $tbl3 = $tbl3.'<tr>
                              ...
                              <td colspan="5" align="right" style="font-size:10px; border-top:1px solid #000;border-right:1px solid #000;border-left:1px solid #000; border-bottom:1px solid #000; font-weight:bold;">TOTAL Amount</td>
                              <td align="right" style="font-size:10px; font-weight:bold; border-top:1px solid #000; border-right:1px solid #000; border-bottom:1px solid #000;">'.number_format($subtotalValue, 3).'</td>
                           </tr>
                            <tr>
                              ...
                              <td align="right" style="font-size:10px; font-weight:bold; border-top:1px solid #000; border-right:1px solid #000; border-bottom:1px solid #000;">'.number_format($company['discount'],3).'</td>
                           </tr>
                            <tr>
                              ...
                              <td align="right" style="font-size:10px; font-weight:bold; border-top:1px solid #000; border-right:1px solid #000; border-bottom:1px solid #000;">'.number_format($subtotalValue - $company['discount'],3).'</td>
                           </tr>
                          </tbody>
                        </table>';
    ...
    function getViewQuoteLog() {
        ...
        while ($row = $db->sql_fetchrow($result)) {
          if($row['drawing_nos'] == 1) {
              ...
              $quoteActions    = '';
              $urlPrintLinkPdf = "index.php?widget=enggCrm_projectQuote&_spAction=printDrawingQuoteLogPdf&opportunity_id={$opportunity_id}&quote_log_id={$row['quote_log_id']}&showHTML=0";
              ...
              $urlPrintLinkPdf  = "index.php?widget=enggCrm_projectQuote&_spAction=printLinkForLogPdf&opportunity_id={$opportunity_id}&quote_log_id={$row['quote_log_id']}&showHTML=0";
    ...
    function getAddLineItemForQuoteLogListView($opportunity_id, $project_id, $quote_log_id) {
        ...
        while ($row = $db->sql_fetchrow($result)) {
            ...
            if($row['drawing_nos'] == "" || $row['drawing_nos'] == 0) {
              $total_amount = 0;
              if($row['unit_price'] > 0 && $row['quantity'] > 0) {
                  $total_amount = round($row['quantity'] * $row['unit_price'], 3);
              } elseif ($row['unit_price'] > 0 && $row['quantity'] == 0) {
                  $total_amount = round($row['unit_price'], 3);
              } elseif ($row['amount'] > 0) {
                  $total_amount = round($row['amount'], 3);
              }
    ...
    function getPrintLinkForLogPdf() {
        ...
        for($i = 0; $i <= $emptyRow; $i++) {
          $tbl3 = $tbl3.'<tr>
            ...
            $tbl3 = $tbl3.'<tr>
                              <td align="right" colspan="2" style="font-size:10px; font-weight:bold; border-top:1px solid #000;border-right:1px solid #000;"></td>
                              <td colspan="3" align="right" style="font-size:10px; border-top:1px solid #000;border-right:1px solid #000; border-bottom:1px solid #000; font-weight:bold;">TOTAL Amount</td>
                              <td align="right" style="font-size:10px; font-weight:bold; border-top:1px solid #000; border-right:1px solid #000; border-bottom:1px solid #000;">'.number_format($totalvalue, 3).'</td>
                           </tr>
                          </tbody>
                        </table>';
