<?
class CPL_Admin_Widgets_EnggCrm_ProjectJobCompletion_View extends CP_Common_Lib_WidgetViewAbstract
{
    /**
     *
     */
    ...
    function getJobCompletionPortal($project_id = '', $job_completion_id = '', $category = '') {
        ...
        $quoteRec    = $fn->getRecordRowByID('job_completion', 'job_completion_id', $job_completion_id);
        ...
        while ($row = $db->sql_fetchrow($result)) {
               ...
               $date   = $fn->getCPDate($row['job_completion_date'], 'd-m-Y');
             

          
              $quoteActions    = '';

              $editForQuote    = "index.php?widget=enggCrm_projectJobCompletion&_spAction=EditForJob&project_id={$project_id}&job_completion_id={$row['job_completion_id']}&showHTML=0";

              $urlPrintLinkPdf = "index.php?widget=enggCrm_projectJobCompletion&_spAction=printDrawingQuotePdf&project_id={$project_id}&job_completion_id={$row['job_completion_id']}&showHTML=0";

              $formActionGroupForQuoteLineItem = "index.php?widget=enggCrm_projectJobCompletion&_spAction=addLineItemForQuoteForm&job_completion_id={$row['job_completion_id']}&project_id={$project_id}&showHTML=0";
              ...
              $edit_image = $cpCfg['cp.localPath']."images/edit.png";
                  ...
                  <div class='float_box clearfix'>
                      <div class='float_left'>
                          ...
                          <a class='editForDoJob'  href='{$editForQuote}' title='Edit Quote'><img src='{$edit_image}' class='icon'></a>

                      </div>
                      <!--<div class='float_left'>
                          <a  class='deleteAddQuote' job_completion_id='{$row['job_completion_id']}'>Delete</a>
                      </div>-->
              ...
              <tbody class='deliverDetailRow'>
                  <tr class='addQuoteRow'>
                    <td data-label='Title'><p>{$row['job_title']}</p></td>

                      <td data-label='Code'><p>{$row['job_no']}</p></td>
                      <td data-label='Date'><p>{$date}</p></td>
                      
                      <td data-label='Status' class='' >{$row['job_status']}</td>
      ...
      <a id='addJobCompletion' class='btn btn-primary' project_id='{$project_id}'>Create Job Completion</a>
     ...
     *
     */
    function getAddLineItemForQuoteListView($project_id, $job_completion_id) {
        $db = Zend_Registry::get('db');
        $fn = Zend_Registry::get('fn');
        $cpCfg = Zend_Registry::get('cpCfg');
        $dateUtil = Zend_Registry::get('dateUtil');
        ...
        FROM `job_completion_items` qt
        LEFT JOIN job_completion d ON (qt.job_completion_id = d.job_completion_id)
        WHERE qt.job_completion_id = {$job_completion_id}
        ORDER BY qt.job_completion_items_id ASC
        ";
        $result  = $db->sql_query($SQL);
        $numRows = $db->sql_numrows($result);
        ...
        while ($row = $db->sql_fetchrow($result)) {
            ...
            $editForLineItem = "index.php?widget=enggCrm_projectJobCompletion&_spAction=editLineItem&job_completion_id={$row['job_completion_id']}&job_completion_items_id={$row['job_completion_items_id']}&project_id={$project_id}&showHTML=0";

            $editText = "
            <div class='float_left'>

                <a class='editForJobLineItemNote' href='{$editForLineItem}' title='Edit Line Item'><img src='{$edit_image}' class='icon'></a>
            ...
            <div class='float_left'>
                ...
                <a  class='deleteDoLineItem' job_completion_items_id='{$row['job_completion_items_id']}' job_completion_id= '{$row['job_completion_id']}'  title='Delete Line Item'><img src='{$delete_image}' class='icon'></a></td>
    ...
    function getAddMultipleJobLineItem1() {
        ...
        $project_id     = $fn->getReqParam('project_id');
        $job_completion_id       = $fn->getReqParam('job_completion_id');
                $job_completion_items_id       = $fn->getReqParam('job_completion_items_id');
        ...
        $rowQuote   = $fn->getRecordRowByID('job_completion_items', 'job_completion_items_id', $job_completion_items_id);
        ...
        SELECT qt.*
        FROM quote_items qt
        LEFT JOIN quote q ON (q.job_completion_id = qt.job_completion_id)
        ...
        <form id='addMultipleJobLineItemForm' class='addMultipleLineItemForm' method='post' action='{$formAction}'>
            ...
            <input type='hidden' name='job_completion_id' value='{$job_completion_id}' />
    ...
    function getAddMultipleLineItem() {
        ...
        $rowProject = $fn->getRecordRowByID('project', 'project_id', $project_id);
        $rowQuote   = $fn->getRecordRowByID('job_completion', 'job_completion_id', $job_completion_id);
        ...
        $formAction = "index.php?widget=enggCrm_projectJobCompletion&_spAction=addMultipleLineItemSubmit&showHTML=0";
        $expNoEdit  = array('isEditable' => 0);
    ...
    function getEditForJob() {
        ...
        $rowJob      = $fn->getRecordRowByID('job_completion', 'job_completion_id', $job_completion_id);
        
        $rowProject    = $fn->getRecordRowByID('project', 'project_id', $project_id);
                $rowComp = $fn->getRecordByCondition('company', "company_id = '{$rowJob['company_id']}'");


        $formActionEditForQuote = "index.php?widget=enggCrm_projectJobCompletion&_spAction=EditForJobSubmit&job_completion_id={$rowJob['job_completion_id']}&project_id={$project_id}&showHTML=0";
        ...
        $sqlStatus = $fn->getValueListSQL('jobStatus');
...
$sqlComp = "
        ...
        <form id='editForDoNote' class='yform columnar editQuote' method='post' action='{$formActionEditForQuote}'>
            ...
            <fieldset>
                <table width='100%'>
                    <tr>
                        <td>{$formObj->getTBRow('Code', 'job_no', $rowJob['job_no'])}</td>
                        <td>{$formObj->getDateRow('Completion Date', 'job_completion_date',$rowJob['job_completion_date'])}</td>
                        <td>{$formObj->getTBRow('Report No', 'report_no',$rowJob['report_no'])}</td>
                    ...
                    <tr>
                        ...
                        <td>{$formObj->getDDRowBySQL('Company', 'company_id', $sqlComp, $rowComp['company_id'])}</td>
                        <td>{$formObj->getTBRow('Title', 'job_title',$rowJob['job_title'])}</td>
                            <td>{$formObj->getDDRowBySQL('Status', 'job_status', $sqlStatus, $rowJob['job_status'], $expVl)}</td>
                    </tr>
                      <tr>
                        <td colspan='3'>                          
                        <label>Description Of Work</label>
                        {$formObj->getHTMLEditor('Terms & Condition', 'description', $rowJob['description'])}</td>
                    </tr>
    ...
    function getEditLineItem() {
        ...
        $rowProject     = $fn->getRecordRowByID('project', 'project_id', $project_id);
        $rowQuoteItem   = $fn->getRecordRowByID('job_completion_items', 'job_completion_items_id', $job_completion_items_id);
        $rowQuote       = $fn->getRecordRowByID('job_completion', 'job_completion_id', $rowQuoteItem['job_completion_id']);
        
        $exp   = array('sqlType' => 'OneField');
        $expVL = array('sqlType' => 'OneField');

        $formActionEditLineItem = "index.php?widget=enggCrm_projectJobCompletion&_spAction=editLineItemSubmit&lnkRoom={$tv['lnkRoom']}&job_completion_items_id={$job_completion_items_id}&showHTML=0";
          ...
          <form id='editForDoLineItem' class='yform columnar' method='post' action='{$formActionEditLineItem}'>
              ...
              <fieldset>
               ...
               {$formObj->getTBRow('Description', 'description', $rowQuoteItem['description'])}
                  ...
                  <input type='hidden' name='job_completion_items_id' value='{$job_completion_items_id}' />
                  <input type='hidden' name='project_id' value='{$project_id}' />
    ...
    function getPrintLinkForPdf() {
        ...
        SELECT q.*
              ...
              ,j.designation
        FROM quote q
        LEFT JOIN (quote_items qi) ON (qi.job_completion_id = q.job_completion_id)
        ...
        WHERE q.job_completion_id = {$job_completion_id}
        ...
        while ($row = $db->sql_fetchrow($result)) {
            ...
            $QICount = $fn->getRecordCount('quote_items', "job_completion_id = {$job_completion_id} AND s_no != ''");
   ...
   /**
    ...
    function getPrintDrawingQuotePdf() {
        ...
        include_once(CP_LIBRARY_PATH.'lib_php/tcpdf/tcpdf.php');
        include_once(CP_LOCAL_PATH.'lib/headfootjob.php');
        ...
        WHERE c.job_completion_id = '{$job_completion_id}'
       ORDER BY c.job_completion_id ASC
        ...
        $quote_date   = $fn->getCPDate($company['job_completion_date'], 'd/M/Y');
        $today      = date("d-m-Y");
      ...
      <table border="0" width="100%" cellpadding="4">
            <tr>
                <td width="50%" style="font-size:16px; font-weight:bold; text-decoration:underline; "><br/><br/><br/><br/><br/><br/>JOB COMPLETION REPORT<br/><br/>Date:  '.$quote_date.'</td>
                <td width="50%" align="right" style=""><img src="images/sidelogo.jpg" width="200"  /></td>

            </tr>
            <br/><br/>
             <tr>
                <td width="30%" style="font-size:16px; line-height:26px;">Job No : '.$company['job_no'].' </td>
                <td width="70%" align="right" style="line-height:26px;">Report No : '.$company['report_no'].' </td>
         ...
         <table border="1" width="100%" cellpadding="4">
            <tr>
                <td width="50%" style="font-size:16px;">Client:  '.$company['company_name'].'</td>
                <td width="50%" style="line-height:26px;">Po: No :  '.$company['po_no'].'</td>

            </tr>

             <tr>
                <td width="50%" style="font-size:16px;  line-height:26px;">Job Title : '.$company['job_title'].' </td>
                <td width="50%" style="line-height:26px;">Job Status : '.$company['job_status'].' </td>
...
<br/><br/><br/>
          <table border="0" width="100%" cellpadding="4">
            <tr>
                <td width="30%" style="font-size:16px;text-decoration:underline;">Description Of works</td>

            </tr>

             <tr>
               
                <td width="100%" style="line-height:26px;text-decoration:underline;">'.$company['description'].' </td>
        ...
        ';
             ...
             $tbl5 = '       
              ...
              <td  style="font-size:15px;">A Team Representative </td>
               
               </tr>
<br/>
               <tr>

              <td width="50%"  style="font-size:15px;  ">Name </td>
              <td width="50%" style="font-size:15px;">:'.$company['approved_by'].'</td>

               </tr>
               <br/>
                <tr>

              <td width="50%"  style="font-size:15px;  ">Date </td>
              <td width="50%" style="font-size:15px;">:</td>

               </tr>
               <br/>
                <tr>

              <td width="50%"  style="font-size:15px;  ">Sign </td>
              <td width="50%" style="font-size:15px;">:</td>
              ...
              <td  style="font-size:15px; ">Client Representative </td>
       ...
       $pdf->writeHTML($tbl1, true, false, false, false, '');
        ...
        $download_title = $cpCfg['cp.companyName'] . '-' . $company['job_no'] . '-Quote.pdf';
        //ob_end_clean();
        $pdf->Output($download_title, 'I');
    }
