<?
class CP_Admin_Themes_Course_Hooks_PluginCommonLogin
{
    /**
     *
     */
    function getLoginForm(){
        ...
        $expPass['placeholder']         = "Password"; 

        $expEmail = array('disableAutoComplete' => $cpCfg['p.common.login.disableAutoCompleteTextFld'], 'placeholder' => 'Email');
        ...
        if ($maindomain == 'fusiondev' || $maindomain == 'fusiondemo') {
            $blinktext = "<span class='blinkingText demoSystemText mt15'>Please note this is V2 Software</span><br/>";
        }
        ...
        {$blinktext}
        <div id='loginOuter'>
            <h1 class='txtCenter loginCompanyName'>
                <img src='/admin/images/logo.png' alt='logo'/>
            </h1>
            <h2 class='txtCenter loginHeading'>Login In</h2>
            <p class='txtCenter loginSubText'>Login in Using Your Email Address</p>
            <form name='loginForm' id='loginForm' class='yform columnar login cpJqForm' method='post' action='{$formAction}'>
                <fieldset>
                    ...
                    {$formObj->getTextBoxRow('', 'email', '', $expEmail)}
                    {$formObj->getTextBoxRow('', 'pass_word', '', $expPass)}
                    <input type='submit' name='submit' class='col-md-12' value='Login' />
                    <div class='flaotbox'>
