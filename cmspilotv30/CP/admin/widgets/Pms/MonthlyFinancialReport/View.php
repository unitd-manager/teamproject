<?
class CP_Admin_Widgets_Pms_MonthlyFinancialReport_View extends CP_Common_Lib_WidgetViewAbstract
{
    /**
     *
     */
        ...
        if ($rowsHTML != ""){
            ...
            <table class='thinlist summaryTable'>
                ...
                <tr>
                    ...
                    <td>To Month : {$end_date}</td>
                    <td>Mode of Payment : {$payment_mode}</td>
                    <td>Status : {$status}</td>
                    <td>Total no of Data : {$rowCount['total_students']}</td>
                    <td class='txtRight'>Grand Total : {$total_outstanding_amount}</td>
                </tr>
            </table>
    ...
    function getRowsHTML() {
        ...
        foreach($this->model->dataArray as $row){
            if ($row['amount_payable'] > 0) {
                ...
                <tr>
                    ...
                    <td>{$invoice_year}</td>
                    <td>{$this->getMonthVal($row['invoice_month'])}</td>
        ...
        {$rows}
        <tr>
            <td class='txtRight' colspan='9'>Total Outstanding Amount</td>
