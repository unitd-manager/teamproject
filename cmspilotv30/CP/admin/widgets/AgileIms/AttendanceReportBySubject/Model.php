<?
class CP_Admin_Widgets_AgileIms_AttendanceReportBySubject_Model extends CP_Common_Lib_WidgetModelAbstract
{
    ...
    function getSQL(){
        ...
        SELECT c.contact_id
              ...
              ,(
                ...
                FROM attendance a
                LEFT JOIN batch bat ON (bat.batch_id = a.batch_id)
                WHERE a.contact_id = c.contact_id 
                  AND a.status = 'Present'
                  AND bat.course_id = b.course_id
                  AND bat.subject_id = b.subject_id
        ...
        FROM attendance att
        LEFT JOIN batch b ON (att.batch_id = b.batch_id)
        LEFT JOIN batch_teacher bt ON (bt.batch_id = b.batch_id)
        ...
        LEFT JOIN contact c ON (c.contact_id = att.contact_id)
        ";
    ...
    function setSearchVar() {
        ...
        if ($subject_id != ''){
            $searchVar->sqlSearchVar[] = "b.subject_id = {$subject_id}";
        }
        ...
        $searchVar->groupBy = 'b.title, att.contact_id, b.course_id';
    ...
    function getDataArray() {
        ...
        $dataArray = $modelHelper->getWidgetDataArray($this->controller, 'agileIms_incomeByCourse');
    ...
    function getExportToExcel(){
        ...
        $actSheet->setCellValueByColumnAndRow($colc++, $rowc, 'Batch Title');
        $actSheet->setCellValueByColumnAndRow($colc++, $rowc, 'Subject');
        $actSheet->setCellValueByColumnAndRow($colc++, $rowc, 'Teacher');
        $actSheet->setCellValueByColumnAndRow($colc++, $rowc, 'Start Time');
        $actSheet->setCellValueByColumnAndRow($colc++, $rowc, 'End Time');
        ...
        if($month != '' && $batch_id != '') {
            $sqlAppend = "WHERE b.batch_id like '{$batch_id}' 
                           AND att.date like '{$search_date}'";
        }
        else if($batch_id != '') {
            $sqlAppend = "WHERE b.batch_id like '{$batch_id}'";
        }
        else if($month != '') {
            $sqlAppend = "WHERE att.date like '{$search_date}'";
        }
        ...
        LEFT JOIN contact c ON (c.contact_id = att.contact_id)
        {$sqlAppend}
        GROUP BY b.title, att.contact_id, b.course_id
        ...
        while ($row = $db->sql_fetchrow($result)) {
            ...
            $actSheet->setCellValueByColumnAndRow($colc++, $rowc, $row['subject_title']);
            $actSheet->setCellValueByColumnAndRow($colc++, $rowc, $row['teacher_name']);
            $actSheet->setCellValueByColumnAndRow($colc++, $rowc, $row['start_time']);
            $actSheet->setCellValueByColumnAndRow($colc++, $rowc, $row['end_time']);
