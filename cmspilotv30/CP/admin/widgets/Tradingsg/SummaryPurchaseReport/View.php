<?
class CP_Admin_Widgets_Tradingsg_SummaryPurchaseReport_View extends CP_Common_Lib_WidgetViewAbstract
{
    /*
     *
     */
    function getWidget() {
        ...
        <table class='thinlist summaryTable'>
            ...
            <tr>
                ...
                <td>Year : {$year}</td>
                <td>Supplier : {$companyRec['company_name']}</td>
            </tr>
        </table>
		...
		<table class='thinlist mt10'>
			<thead>
				<tr>
					...
					<th class='txtRight'>GST 7 %</th>
					<th class='txtRight'>Amount(after GST)</th>
				</tr>
			</thead>
			<tbody>
				{$this->getRowsHTML()}
    ...
    function getRowsHTML() {
        ...
        foreach ($this->model->dataArray as $row) {
            ...
            if($rowInvoice['order_item_id'] != ''){
    			...
    			<tr>
    				...
    				<td>{$row['formatted_quote_date']}</td>
                    <td>{$row['client_name']}</td>
                    <td>{$row['company_name']}</td>
    				<td class='txtRight'>{$format_total_purchase}</td>
    				<td class='txtRight'>{$before_gst}</td>
    				<td class='txtRight'>{$after_gst}</td>
    			</tr>
    			";
            }
        }
        ...
        $format_overall_before_gst = number_format($overall_before_gst, 2);
        $format_overall_after_gst  = number_format($overall_after_gst, 2);
        
        $rows .= "
        <tr>
            <td colspan='4' class='txtRight'>Overall Amount</td>
            <td class='txtRight'>{$format_overall_purchase}</td>
            <td class='txtRight'>{$format_overall_before_gst}</td>
            <td class='txtRight'>{$format_overall_after_gst}</td>
        </tr>
        ";
