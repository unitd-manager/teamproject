<?
class CP_Admin_Widgets_Tradingsg_SalesByClient_View extends CP_Common_Lib_WidgetViewAbstract
{
    /**
     *
     */
    ...
    function getRowsHTML() {
        ...
        foreach($this->model->dataArray as $row){
            ...
            if ($row['company_id'] != '') {
                $company_order_amount = number_format($row['company_order_amount'], 2);
                $rows .= "
			    <tr>
			    	<td>{$count}</td>
			    	<td>{$row['company_name']}</td>
					{$siteTitle}
			    	<td class='txtRight'>{$company_order_amount}</td>
			    </tr>
			    ";
			    $count++;
                $total += $row['company_order_amount'];
			}
        }
