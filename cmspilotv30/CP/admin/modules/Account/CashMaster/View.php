<?
class CP_Admin_Modules_Account_CashMaster_View extends CP_Common_Lib_ModuleViewAbstract
{
    var $jssKeys = array('baBBQ', 'searchabledropdown-1.0.7');
    ...
    function getList($dataArray) {
        ...
        $listObj2->addFld('acc_head', array('sort' => 'acc_head', 'title' => 'Account'));
        $listObj2->addFld('currency_code', array(
            'title' => 'Curr.'
           ,'align' => 'center'
           ,'sort' => 'currency'
        ));
        $listObj2->addFld('receipt_amount', array(
            'type' => 'number'
           ,'formatCb' => array($this, 'getFormatAmount')
        ));
        $listObj2->addFld('receipt_rate', array(
            'type' => 'number'
           ,'displayDecimalLength' => $cpCfg['cp.displayDecimalLength2']
        ));
        $listObj2->addFld('payment_amount', array(
            'type' => 'number'
           ,'formatCb' => array($this, 'getFormatAmount')
        ));
        $listObj2->addFld('payment_rate', array(
            'type' => 'number'
           ,'displayDecimalLength' => $cpCfg['cp.displayDecimalLength2']
        ));
        ...
        //$listObj2->addFld('cash_name', array('sort' => 'cash_name'));
    ...
    function getNew($rowMaster = null){
        ...
        $subAction2 = $subAction == 'receipt' ? 'buy' : 'sell';
        ...
        $expAccHead = array('fieldCls' => 'searchable');
        $expAmount  = array('fldPrefix' => ' ');
        if ($tv['action'] == 'new') {
            $staff_id   = $rowStaff['staff_id'];
            $entry_date = $cpUtil->getISODateStr();
        } else {
            if ($tv['action'] == 'detail') {
                $amount = $fn->getFormatNumber($rowMaster['amount_sum_cust_curr']);
            } else {
                $amount = $fn->getFormatNumber($rowMaster['amount_sum_cust_curr'], '', false);
            }

            $journal_master_id = $rowMaster['journal_master_id'];
            $acc_head_id       = $rowMaster['acc_head_id'];
            $entry_date        = $rowMaster['entry_date'];
            $staff_id          = $rowMaster['staff_id'];
            $narration_main    = $rowMaster['narration'];

            $expAccHead['detailValue'] = $rowMaster['acc_head_title'];
            $expAmount = array('fldPrefix' => $rowMaster['customer_currency_code']);
        }
        ...
        $url = '/admin/index.php?_topRm=cash&module=account_cashMaster'
             . "&_spAction=add&subAction={$subAction}&showHTML=0";

        $sqlAccHead = getCPModelObj('account_accHead')->getSundryCreditDebitAcHeadsSQL();
        if ($tv['action'] != 'detail') {
			$actionBtns = $this->getActionButtons();
        }
        ...
        $arr = array();
        if ($acc_head_id != '') {
            $arr = getCPModuleObj('account_accHead')->model->getAccHeadDetails($acc_head_id);
        }
        $exch_rate_cust_curr_system = isset($arr['exch_rate']) ? $arr['exch_rate'] : '';
        ...
        <div id='entry' class='mainFlds'>
            ...
            {$fo->getDDRowBySQL('Customer', 'acc_head_id_customer', $sqlAccHead,
                                $acc_head_id, $expAccHead)}
			...
			<div class='jbody'>
				...
				{$this->getSummaryRow($rowMaster, $newRowText)}
			</div>
			<div class='jfooter'>
			</div>
            {$actionBtns}
            ...
            <input type='hidden' name='c_action2' id='c_action2' value='{$subAction2}'>
            {$fo->getHiddenFldObj('exchRateDeviationLimit',
                $fn->getSettingsValueByKey('m.account.cashMaster.exchRateDeviationPercent'))}
            {$fo->getHiddenFldObj('exch_rate_cust_curr_system', $exch_rate_cust_curr_system)}
        ...
        {$fo->getFieldSetWrapped('', $fieldset, $exp)}
        ";
    ...
    private function getEntryRow($row = null, $rowNum = 1) {
        ...
        if ($tv['action'] == 'detail') {
            $debit       = $fn->getFormatNumber($debit);
            $credit      = $fn->getFormatNumber($credit);
            $debit_base  = $fn->getFormatNumber($debit_base);
            $credit_base = $fn->getFormatNumber($credit_base);
        } else {
            $debit       = $fn->getFormatNumber($debit, '', false);
            $credit      = $fn->getFormatNumber($credit, '', false);
            $debit_base  = $fn->getFormatNumber($debit_base, '', false);
            $credit_base = $fn->getFormatNumber($credit_base, '', false);
        }
        $currency_code = '';
        if ($tv['action'] == 'detail') {
        ...
        if ($tv['action'] == 'new') {
            ...
            $c_action = $row['action'];
            $amount      = $c_action == 'payment' ? $credit : $debit;
            $amount_base = $c_action == 'payment' ? $credit_base : $debit_base;
        }
        ...
        $sqlCashCurrency = getCPModuleObj('account_accHead')->model->getCashCurrencySQL();
        $expAccHead = array(
            'ddSQL' => $sqlCashCurrency
           ,'sqlType' => 'TwoFields'
    ...
    private function getSummaryRow($rowJM, $newRowText) {
        $tv = Zend_Registry::get('tv');
        $fn = Zend_Registry::get('fn');
        $cpCfg = Zend_Registry::get('cpCfg');
        ...
        $amount_base_sum              = '';
        $exch_rate_cust_curr          = '';
        $exch_rate_cust_curr_lc_to_fc = '';
        $customer_currency_code       = '';
        $amount_sum_cust_curr         = '';

        //if new record
        if ($rowJM) {
            if ($rowJM['action'] == 'receipt') {
                $amount_base_sum = $rowJM['debit_base_sum'];
            } else {
                $amount_base_sum = $rowJM['credit_base_sum'];
            }

            $exch_rate_cust_curr          = $rowJM['exch_rate_cust_curr'];
            $exch_rate_cust_curr_lc_to_fc = $rowJM['exch_rate_cust_curr_lc_to_fc'];
            $customer_currency_code       = $rowJM['customer_currency_code'];
            $amount_sum_cust_curr         = $rowJM['amount_sum_cust_curr'];

            if ($tv['action'] == 'detail') {
                $exch_rate_cust_curr = $fn->getFormatNumber($exch_rate_cust_curr,
                                                            $cpCfg['cp.account.exchangeRateDecimals']);
                $amount_sum_cust_curr = $fn->getFormatNumber($amount_sum_cust_curr);
                $amount_base_sum = $fn->getFormatNumber($amount_base_sum);
            } else {
                $exch_rate_cust_curr = $fn->getFormatNumber($exch_rate_cust_curr,
                                                            $cpCfg['cp.account.exchangeRateDecimals']);
                $amount_sum_cust_curr = $fn->getFormatNumber($amount_sum_cust_curr, '', false);
                $amount_base_sum = $fn->getFormatNumber($amount_base_sum, '', false);
            }
        }
        ...
        <th class='amount_base'>
            <input type='text' name='amount_base_sum'
                   class='fld-amount_base_sum fld-non-editable'
                   value='{$amount_base_sum}'>
		</th>
        <th class='narration'>
		</th>
        <th class='delete'>
        ...
        <td class='exch-rate-label'>
           Exch. Rate (FC to LC)
        </td>
        <td class='exch_rate_cust_curr'>
            <input type='text' name='exch_rate_cust_curr'
                   class='fld-exch_rate_cust_curr'
                   value='{$exch_rate_cust_curr}'>
		</td>
        <td class='exch-rate-lc-to-fc'>
            <span>&nbsp;Exch. Rate (LC to FC)</span>
            <input type='text' name='exch_rate_cust_curr_lc_to_fc'
                   class='fld-exch_rate_cust_curr_lc_to_fc'
                   value='{$exch_rate_cust_curr_lc_to_fc}'>
        </td>
        </tr>
        </table>
        </div>
        ...
        <td class='customer-curr'>
           {$customer_currency_code}
        </td>
        <td class='amount_base_sum'>
            <input type='text' name='amount_sum_cust_curr'
                   class='fld-amount_sum_cust_curr'
                    tabindex='-1' value='{$amount_sum_cust_curr}'>
		</td>
        <td class='empty'></td>
        </tr>
        </table>
    ...
    private function getActionButtons(){
        ...
        if ($tv['action'] == 'detail') {
            ...
            <div class='type-button float_right hid actBtnsInNewForm' style='display: block;'>
                <input type='button' id='actBtn_cancelCash' class='cancel'
    ...
    function getDetail() {
        ...
        $rowJM = getCPModuleObj('account_journalMaster')->model
                 ->getJournalMasterRow($journal_master_id);
    ...
    function getQuickSearch() {
        ...
        <td>
            <select name='acc_head_id'>
                ...
                {$dbUtil->getDropDownFromSQLCols2($db, $sqlCashCurrency, $acc_head_id)}
            </select>
        </td>
    ...
    function getAdditionalImportFields() {
        ...
        $sqlCashSetup = getCPModuleObj('account_cashSetup')->model->getCashSetupSQL();

        $text = "
        {$formObj->getDateRow('Entry Date', 'entry_date', $entry_date, $expEntryDate)}
        {$formObj->getDDRowBySQL('Cash Name', 'cash_setup_id', $sqlCashSetup, '', $expHideFO)}
