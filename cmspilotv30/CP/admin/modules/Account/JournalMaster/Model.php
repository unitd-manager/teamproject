<?
class CP_Admin_Modules_Account_JournalMaster_Model extends CP_Common_Lib_ModuleModelAbstract
{
    function getSQL() {
        $SQL = "
    ...
    function getJournalRows(){
        ...
        foreach ($_POST as $key => $value) {
            ...
            if ($fieldName == 'acc_head') {
                ...
                $currency_id_fld       = "currency_id-{$rowInd}";
                ...
                $arrRow['currency_id']       = $fn->getPostParam($currency_id_fld);
                ...
                if (  $arrRow['acc_head_id'] && $arrRow['currency_id'] && $arrRow['exch_rate_to_base']) {
                    $arrRows[] = $arrRow;
                }
            }
        }
       ...
       if ($validation['error']){
        ...
        if ($journal_master_id == '') { //new journal
            ...
            foreach ($journalRows as $journalRow) {
                ...
                $fa['currency_id']       = $journalRow['currency_id'];
                $fa['exch_rate_to_base'] = $journalRow['exch_rate_to_base'];
                ...
                if ($cpCfg['m.account.journalMaster.hasAvgStock']) {
