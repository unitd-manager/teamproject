Util.createCPObject('cpm.account.journalMaster');

cpm.account.journalMaster = {
    baseCurrDecimals: 2,
    debit_base_total: 0,
    credit_base_total: 0,
    difference: 0,
    init: function() {
        ...
        if (this.cpAction == 'new' || this.cpAction == 'edit') {
            ...
            cpm.account.journalMaster.initializeAccountTypeAhead();
            $('#entry input').live('keyup', cpm.account.journalMaster.onEnterPress);

            $('#entry .fld-debit, #entry .fld-credit')
                    .live('change', cpm.account.journalMaster.clearDebitCredit);
            $('#entry .fld-debit, #entry .fld-credit')
                    .live('change', cpm.account.journalMaster.setRowBgColor);

            $('#entry .fld-debit, #entry .fld-credit, #entry .fld-exch_rate')
                    .live('change', cpm.account.journalMaster.updateRowValues);

            $('#entry .mainFlds input, #entry .mainFlds select')
                    .keyup(cpm.account.journalMaster.onEnterPressMainFlds);

            $('#entry .delete-small')
                    .live('click', cpm.account.journalMaster.deleteRow);

            $('#entry #actBtn_saveContinue, #entry #actBtn_saveJournal')
                    .click(cpm.account.journalMaster.saveJournal);
        ...
        $('a.ledger-auth').click(cpm.account.journalMaster.ledgerAuthorize);
        //journal master ledger authorize
        $('a.ledger-pending').click(cpm.account.journalMaster.ledgerPending);
    },
    initializeAccountTypeAhead: function() {
        $('#entry .fld-account').autocomplete({
            ...
            select: function(event, ui) {
                cpm.account.journalMaster.updateRowValues.call(this, ui);
            }
        });
    },
    ...
    onEnterPress: function(e) {
        if (e.keyCode == 13) {
            var fld = cpm.account.lib.getCurrentFieldIndexArr($(this).attr('name'));
            ...
            if (fld.name == 'acc_head') {
                ...
                } else if (!cpm.account.journalMaster.isRowEmpty(rowDiv)) {
                    cpm.account.journalMaster.newBlankRow(rowDiv);
                } else {
    ...
    deleteRow: function() {
        if ($('#entry .jbody .row').length > 1) {
            ...
            cpm.account.journalMaster.clearRow(row);
        }
        cpm.account.journalMaster.setDifference();
    },
    updateRowValues: function(ui) {
        ...
        var baseCurrDecimals = cpm.account.journalMaster.baseCurrDecimals;

        var fld = cpm.account.lib.getCurrentFieldIndexArr($(this).attr('name'));
        var row = $(this).parents('div.row');
        var acc_head_id_obj = $(row).find('.fld-acc_head_id');
        var currency_id_obj = $(row).find('.fld-currency_id');
        var currency_obj = $(row).find('.fld-currency');
        var debit_obj = $(row).find('.fld-debit');
        var credit_obj = $(row).find('.fld-credit');
        var exch_rate_obj = $(row).find('.fld-exch_rate');
        var debit_base_obj = $(row).find('.fld-debit_base');
        var credit_base_obj = $(row).find('.fld-credit_base');
        var exch_rate = 0;
        if (fld.name == 'acc_head') {
            $(acc_head_id_obj).val(ui.item.id);
            $(currency_id_obj).val(ui.item.currency_id);
            var currency_curr = $(currency_obj).val();
            $(currency_obj).val(ui.item.currency);
            var exch_rate_curr = $(exch_rate_obj).val();
            ...
            if (ui.item.currency == currency_curr) {
                if (exch_rate_curr <= 0) {
                    $(exch_rate_obj).val(ui.item.exch_rate);
                }
            } else {
                $(exch_rate_obj).val(ui.item.exch_rate);
            }
        }
        exch_rate = $(exch_rate_obj).val();
    ...
    setDifference: function() {
        ...
        difference = difference.toFixed(cpm.account.journalMaster.baseCurrDecimals);

        cpm.account.journalMaster.debit_base_total = debit_base_total;
        cpm.account.journalMaster.credit_base_total = credit_base_total;
        cpm.account.journalMaster.difference = difference;
    ...
    validateJournal: function() {
        var difference = cpm.account.journalMaster.difference;
        //var difference = 1;
        var errorMessage = '';
        if (difference != 0) {
            errorMessage = 'The debit and credit total does not match';
            cpm.account.journalMaster.displayError(errorMessage);
            return false;
        }
        ...
        $('#entry .jbody div.row')
            ...
            if (acc_head_id == '' && debit == '' && credit == '' && exch_rate == '' &&
                    debit_base == '' && credit_base == '') {
                //do nothing. ok to have empty row
            } else {
                if (acc_head_id == '' || (debit == '' && credit == '') || exch_rate == '' ||
                        (debit_base == '' && credit_base == '')) {
                    errorMessage = "Please input values for account/debit/credit/exchange_rate properly";
                    cpm.account.journalMaster.displayError(errorMessage);
                    returnVal = false;
                }
            }
    ...
    saveJournal: function() {
        ...
        if (!cpm.account.journalMaster.validateJournal()) {
            return;
        }

        Util.showProgressInd('Saving...');
        Util.clearPrepopulatedTextbox('#entry');
        var url = 'index.php?module=account_journalMaster&_spAction=saveJournal&showHTML=0';
        var data = $('#entry input, #entry select').serialize();
        cpm.account.journalMaster.enableDisableJournalButtons('disable');
        $.post(url, data, function(json) {
            cpm.account.journalMaster.enableDisableJournalButtons('enable');
            Util.hideProgressInd();
            if (json.status == 'error') {
                cpm.account.journalMaster.displayError(json.errorMsg);
            } else {
                ...
                cpm.account.journalMaster.hideError();
                if (action == 'saveContinue') {
                    cpm.account.journalMaster.clearEntryScreen();
                } else if (action == 'save') {
    ...
    clearEntryScreen: function() {
        ...
        $('#entry .jbody div.row:gt(0)').remove();
        cpm.account.journalMaster.clearRow('#entry .jbody div.row');
        $('#entry #fld_entry_date').focus().select();
    },
    ledgerAuthorize: function() {
        ...
        var url = 'index.php?module=account_journalMaster&_spAction=ledgerAuthorize&showHTML=0' +
    ...
    ledgerPending: function() {
        ...
        var url = 'index.php?module=account_journalMaster&_spAction=ledgerPending&showHTML=0' +
                '&pending=' + pending +
