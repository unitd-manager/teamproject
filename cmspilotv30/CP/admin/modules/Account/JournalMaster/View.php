<?
class CP_Admin_Modules_Account_JournalMaster_View extends CP_Common_Lib_ModuleViewAbstract
{
    function getList($dataArray) {
        ...
        $voucher_code_temp = (count($dataArray) > 0) ? $dataArray[0]['voucher_code'] : '';
        $listRowClass = 'odd';

        $base_currency = getCPModelObj('account_accCompany')->getBaseCurrencyCode();

        foreach ($dataArray as $row){
            if ($row['voucher_code'] != $voucher_code_temp) {
                if ($listRowClass == 'even') {
                    $listRowClass = 'odd';
                } else {
                    $listRowClass = 'even';
                }
            }
            $exch_rate_to_base = $fn->getFormatNumber($row['exch_rate_to_base'],
            ...
            $avg_buy_rate   = $fn->getFormatNumber($row['avg_buy_rate'],
                                 $cpCfg['cp.account.exchangeRateDecimals']);
            $avg_stock_rate = $fn->getFormatNumber($row['avg_stock_rate'],
                                  $cpCfg['cp.account.exchangeRateDecimals']);
            ...
            {$listObj->getGoToDetailText($rowCounter, $row['entry_date'])}
            {$listObj->getListDataCell($row['voucher_code'])}
            {$listObj->getListDataCell($row['acc_head'])}
            {$listObj->getListDataCell($row['currency_code'], 'center')}
            {$listObj->getListDataCell($row['voucher_type'])}
            {$listObj->getListDataCell($this->getNarration($row))}
            {$listObj->getListDataCell($exch_rate_to_base, 'right')}
            {$listObj->getListDataCell($debit, 'right', '', '', $expDebit)}
            {$listObj->getListDataCell($credit, 'right', '', '', $expCredit)}
            {$listObj->getListDataCell($debit_base, 'right', '', '', $expDebitBase)}
            {$listObj->getListDataCell($credit_base, 'right', '', '', $expCreditBase)}
            {$ledgerAuthText}
            {$listObj->getListDataCell($row['short_code'])}
            <!--
            {$listObj->getListDataCell($avg_buy_rate, 'right')}
            {$listObj->getListDataCell($avg_stock_rate, 'right')}
            {$listObj->getListDataCell($row['margin'], 'right')}
            -->
        ...
        {$listObj->getListHeaderCell('Voucher Code', 'j.journal_master_id')}
        {$listObj->getListHeaderCell('Account', 'j.acc_head_id', 'accountListHeader')}
        {$listObj->getListHeaderCell('Currency', 'currency', 'headerCenter')}
        {$listObj->getListHeaderCell('Voucher Type', 'jm.voucher_type')}
        {$listObj->getListHeaderCell('Narration')}
        {$listObj->getListHeaderCell('Exch. Rate', '', 'headerRight')}
        {$listObj->getListHeaderCell('Debit', '', 'headerRight')}
        {$listObj->getListHeaderCell('Credit', '', 'headerRight')}
        {$listObj->getListHeaderCell("Debit - {$base_currency}", '', 'headerRight')}
        {$listObj->getListHeaderCell("Credit - {$base_currency}", '', 'headerRight')}
        {$ledgerAuthText}
        {$listObj->getListHeaderCell('Staff', 's.short_code')}
        <!--
        {$listObj->getListHeaderCell('Avg Buy', 'j.avg_buy_rate', 'headerRight')}
        {$listObj->getListHeaderCell('Avg Stock', 'j.avg_stock_rate', 'headerRight')}
        {$listObj->getListHeaderCell('Margin', 'j.margin', 'headerRight')}
        -->
    ...
    function getNew($rowJM = null){
        ...
        $action = $tv['action'];
        $voucherTypeArr = $cpCfg['m.account.journalMaster.voucherTypeArr'];
            ...
            <div class='subcolumns mainFlds'>
                <div class='c80l'>
                    <div class='subcl'>
                        ...
                        {$formObj->getDDRowByArr('Voucher Type', 'voucher_type', $voucherTypeArr, $voucher_type, $exp)}
    ...
    private function getActionButtons(){
        ...
        $url = "index.php?_topRm={$tv['topRm']}&module=account_journalMaster";
        $text = "
    ...
    private function getJournalRowHeader() {
        ...
        <th class='exch_rate'>Exch. Rate</th>
        <th class='debit_base'>Debit - {$base_currency}</th>
        <th class='credit_base'>Credit - {$base_currency}</th>
        <th class='delete'></th>
        </tr>
        </table>
        </div>
    ...
    private function getJournalRow($row = null, $rowNum = 1) {
        ...
        $currency          = ($row == null) ? '' : $row['currency'];
        $currency_id       = ($row == null) ? '' : $row['currency_id'];
        ...
        if ($action == 'detail') {
            ...
            $debit_base  = $fn->getFormatNumber($row['debit_base'], '', false);
        ...
        $deleteText = "<a href='#' class='delete-small'></a>";
        if ($action == 'detail') {
        ...
        <td class='account'>
            ...
            <input type='hidden' name='currency_id-{$rowNum}' value='{$currency_id}' class='fld-currency_id'>
        </td>
        <td class='currency'>
            <input type='text' name='currency-{$rowNum}' class='fld-currency fld-non-editable {$editableClassDetail}'
                   readonly='readonly' value='{$currency}' tabindex='-1'>
        </td>
        <td class='debit'>
        ...
        <td class='debit_base'>
            <input type='text' name='debit_base-{$rowNum}' class='fld-debit_base debit_base-color fld-non-editable'
                   readonly='readonly' tabindex='-1' value='{$debit_base}'>
        </td>
        <td class='credit_base'>
            <input type='text' name='credit_base-{$rowNum}' class='fld-credit_base credit_base-color fld-non-editable'
                   readonly='readonly' tabindex='-1' value='{$credit_base}'>
		</td>
        <td class='delete'>
            {$deleteText}
        </td>
    ...
    function getQuickSearch() {
        ...
        <td>
            <select name='voucher_type'>
                ...
                {$cpUtil->getDropDown1($cpCfg['m.account.journalMaster.voucherTypeArr'], $voucher_type)}
            </select>
        </td>
        <td>
            <select name='show_counter'>
                <option value=''>Show Counter</option>
                {$cpUtil->getDropDown1($arr, $show_counter, 1)}
            </select>
        </td>
        ";
