<?
class CP_Admin_Modules_Account_Ledger_View extends CP_Common_Lib_ModuleViewAbstract
{
    function getList($dataArray) {
        ...
        $viewJM = getCPViewObj('account_journalMaster');
        ...
        $debit_base_sum  = $fn->getFormatNumber($debit_base_sum);
        ...
        foreach ($dataArray as $row){
            ...
            $debit_base = $fn->getFormatNumber($row['debit_base']);
            ...
            $editUrl = "index.php?module=account_journalMaster&_action=edit&record_id={$row['journal_master_id']}";
            $journalEditText = "<a class='edit-journal' href='{$editUrl}' title='Edit Journal'>&nbsp;</a>";

            $ledgerAuthText = '';
            if ($userGroupType != 'User') {
                $ind = $viewJM->getLedgerAuthorizeInd($row, false);
            ...
            {$ledgerAuthText}
            {$listObj->getListDataCell($viewJM->getPendingInd($row, false), 'center')}
        ...
        {$ledgerAuthText}
        {$listObj->getListHeaderCell('Pending', 'j.pending', 'headerCenter')}
        ...
        <td align='right' class='debit_base-color'>{$debit_base_sum}</td>
        <td align='right' class='credit_base-color'>{$credit_base_sum}</td>
        <td></td>
        {$ledgerAuthTextBlankTd}
    ...
    function getAccountInfo($rowAccHead) {
        ...
        <div id='accountInfo' class='ui-corner-all'>
            ...
            <td>|</td>
            <th>Currency:</th>
            <td>{$rowAccHead['currency_code']}</td>
            <td>|</td>
            <th>B/F:</th>
            <td>{$rowAccHead['currency_code']}{$brough_forward}</td>
            <td>|</td>
            <th>Ledger Bal.:</th>
            <td>{$rowAccHead['currency_code']}<span class='bold {$ledger_balance_class}'>{$ledger_balance}</span></td>
            <td>|</td>
            <th>Available Bal.:</th>
            <td>{$rowAccHead['currency_code']}<span class='bold {$available_balance_class}'>{$available_balance}</span></td>
            </tr>
            </table>
        </div>
        ";
