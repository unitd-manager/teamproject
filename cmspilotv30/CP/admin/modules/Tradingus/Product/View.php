<?
class CP_Admin_Modules_Tradingus_Product_View extends CP_Common_Lib_ModuleViewAbstract
{
    //==================================================================//
    function getList($dataArray) {
        ...
        foreach ($dataArray as $row){
            ...
            SELECT price
            FROM product_price
            WHERE price_category = 'Base Price'
            AND product_id = {$row['product_id']}
            ";
            $resultBasePrice = $db->sql_query($sqlBasePrice);
            $rowBasePrice    = $db->sql_fetchrow($resultBasePrice);

            $sqlWholesale = "
            SELECT price
            FROM product_price
            WHERE price_category = 'Wholesale Price'
            AND product_id = {$row['product_id']}
            ";
            $resultWholesale  = $db->sql_query($sqlWholesale);
            $rowBaseWholesale = $db->sql_fetchrow($resultWholesale);

            $sqlRetailer = "
            SELECT price
            FROM product_price
            WHERE price_category = 'Retailer Price'
            AND product_id = {$row['product_id']}
            ";
            $resultRetailer  = $db->sql_query($sqlRetailer);
            $rowBaseRetailer = $db->sql_fetchrow($resultRetailer);

            $Base_Price      = number_format($rowBasePrice['price'],2);
            $Wholesale_Price = number_format($rowBaseWholesale['price'],2);
            $Retailer_Price  = number_format($rowBaseRetailer['price'],2);
            ...
            {$listObj->getListDataCell($Base_Price, 'right')}
            {$listObj->getListDataCell($Wholesale_Price, 'right')}
            {$listObj->getListDataCell($Retailer_Price, 'right')}
        ...
        //{$listObj->getListHeaderCell('BC', 'p.price')}
        ...
        {$listObj->getListHeaderCell('Base Price')}
        {$listObj->getListHeaderCell('Wholesale Price')}
        {$listObj->getListHeaderCell('Retailer Price')}
    ...
    function getRightPanel($row){
        ...
        $text ="
        {$media->getRightPanelMediaDisplay('Picture', 'tradingus_product', 'picture', $row)}
        {$media->getRightPanelMediaDisplay('Related Picture', 'tradingus_product', 'relatedPicture', $row)}
        {$displayLinkData->getLinkPortalMain('tradingus_product', 'tradingus_companyLink', 'Supplier Linked', $row)}
        {$links}
        <div id='priceLinkPortal'>{$this->getPriceDisplay($row['product_id'])}</div>
        ";
        return $text;
    }
     ...
     *
     */
    function getPriceDisplay($product_id=''){
        $cpCfg = Zend_Registry::get('cpCfg');
        $fn = Zend_Registry::get('fn');
        $db = Zend_Registry::get('db');
        $formObj = Zend_Registry::get('formObj');
        ...
        $Prices = $this->getPriceDisplayDetail($product_id);
        ...
        $formActionAddPrice    = "index.php?module=tradingus_product&_spAction=addPrice&product_id={$product_id}&showHTML=0";

        $add = "<div class='actBtns'>
                    <a id='addPriceNew' href='{$formActionAddPrice}' product_id={$product_id}>Add</a>
                </div>";

        $text = "
        <div class='linkPortalWrapper tradingus_product__product_priceLink'>
            <div class='header' expanded='1'>
                <div class='floatbox'>
                    <div class='float_left'>Price Linked</div>
                    <div class='txtRight'>
     ...
     *
     */
    function getPriceDisplayDetail($product_id = ''){
        $cpCfg = Zend_Registry::get('cpCfg');
        $fn = Zend_Registry::get('fn');
        $db = Zend_Registry::get('db');
        $formObj = Zend_Registry::get('formObj');
        ...
        $count = 1;
        while ($rowPrice = $db->sql_fetchrow($result)) {

            $formActionEditPrice   = "index.php?module=tradingus_product&_spAction=editPrice&product_price_id={$rowPrice['product_price_id']}&product_id={$product_id}&showHTML=0";

            $deleteIcon ="
                <div class='float_right'>
                    <a class='deletePriceRecord' href='#'  product_id='{$product_id}' product_price_id='{$rowPrice['product_price_id']}'>
            ...
            $editIcon ="
                <div class='float_right'>
                    <a class='editPrice' href='{$formActionEditPrice}' product_id='{$product_id}' product_price_id='{$rowPrice['product_price_id']}'>
                        <img src='/cmspilotv30/CP/admin/images/icons/btn_edit.png'>
                    </a>
                </div>
                ";

            $productPrice = number_format($rowPrice['price'],2);

            $rows .= "
                <tr>
                    <td>{$count}</td>
                    <td>{$rowPrice['price_category']}</td>
                    <td class='txtRight'>{$productPrice}</td>
                    <td>{$rowPrice['modified_by']} {$rowPrice['modification_date']}</td>
                    <td>
                        {$deleteIcon}
                        {$editIcon}
                    </td>
        ...
        if($numRows == 0){
            $rows .= "
                <tr>
                    <td class='noPrice'>No Records Linked</td>
                </tr>
            ";
    ...
    function getQuickSearch() {
        ...
        //$sqlProductGroup = $fn->getDDSql('tradingus_productGroup');
        $sqlProductGroup = "
