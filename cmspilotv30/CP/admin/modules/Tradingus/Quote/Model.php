<?
class CP_Admin_Modules_Tradingus_Quote_Model extends CP_Common_Lib_ModuleModelAbstract
{
    function getSQL() {
    ...
    */
     ...
     *
     */
    function gettradingusQuoteTradingsgProductLinkSQL($id) {
        $cpCfg = Zend_Registry::get('cpCfg');
        $fn = Zend_Registry::get('fn');
        $db = Zend_Registry::get('db');
     ...
     *
     */
    function gettradingusQuoteTradingsgProductLinkSQLOld12092013($id) {
        $cpCfg = Zend_Registry::get('cpCfg');
        $fn = Zend_Registry::get('fn');
        $db = Zend_Registry::get('db');
     ...
     *
     */
    function gettradingusQuoteTradingsgExpenseLinkSQL($id) {
     ...
     *
     */
    function gettradingusQuoteTradingsgPurchaseOrderLinkSQL($id) {
        $cpCfg = Zend_Registry::get('cpCfg');
        ...
        SELECT po.purchase_order_id
              ...
              ,(CONCAT_WS('', '<a href=\'index.php?module=tradingus_quote&_spAction=printPurchaseOrder&id=', po.purchase_order_id, '\' target=\'_blank\'>PDF to Supplier</a>'))
              ,(CONCAT_WS('', '<a href=\'index.php?module=tradingus_quote&_spAction=printPurchaseOrderWithPrice&id=', po.purchase_order_id, '\' target=\'_blank\'>PDF  with Price</a>'))
    ...
    function getDuplicateQuote() {
        ...
        $url = "index.php?_topRm=order&module=tradingus_quote" .
     ...
     */
     function getUpdateProductLineItems() {
        ...
        */
        ...
        SELECT price
        FROM product_price
        WHERE price_category = '{$companyRec['price_category']}'
        AND product_id = '{$product_id}'
        ";
        $resultPriceCat = $db->sql_query($sqlPriceCat);
        $rowPriceCat    = $db->sql_fetchrow($resultPriceCat);

        $cost_price = '';
        if($rowPriceCat['price'] != ''){
            $cost_price = $rowPriceCat['price'];
        }else{
            $cost_price = $row['price'];
        }

        $selling_price = $cost_price_margin + $cost_price;
        ...
        SELECT SUM(round((qp.selling_price * qp.qty),2)) AS total_selling_price_sum
              ,SUM(round(qp.selling_price,2)) AS total_selling_price
        ...
        if ($product_id != ''){
            ...
            if ($numRowsQP > 0 || $numRowsPP > 0){
                $viewHistory = "
                <a href='index.php?_topRm=order&module=tradingus_quote&_spAction=previousOrderForClient&showHTML=0&quote_product_id={$rec_id}'  class='productViewHistory'> View</a>
                ";
            }
        }

        $arr['price']        = $cost_price;
        $arr['title']        = $row['title'];
        ...
        $arr['total_selling_price']     = $rowTotal['total_selling_price'];
    ...
    function getUpdateSellingLineItems() {
        ...
        $SQL ="SELECT
              ...
              ,(SELECT SUM(round((qp.selling_price) - (qp.selling_price) * (qp.discount_percentage/100),2))
              FROM quote_product qp WHERE qp.quote_id = {$row['quote_id']}
              )
              AS total_selling_price
        ";
        ...
        $arr['total_selling_price']            = $row['total_selling_price'];
        $arr['mark_up_value']     = round($mark_up_value,2);
        //$arr['discount_value']  = round($discount_value,2);
        $arr['selling_price']     = number_format($selling_price,2);
        $arr['totalCostPrice']    = number_format($totalCostPrice,2);
        $arr['totalSellingPrice'] = number_format($totalSellingPrice,2);
    ...
    function getUpdateDiscountByGroupFormSubmit() {
        ...
        while ($row = $db->sql_fetchrow($result)) {
            ...
            $selling_price = $row['cost_price'] + number_format($mark_up,2) + number_format($discount_price,2);
            $sqlUpdate = "
            UPDATE quote_product SET selling_price = '{$selling_price}'
            WHERE quote_product_id = {$row['quote_product_id']}
            ";
    ...
    function getGenerateBulkProduct() {
        ...
        $formAction = "index.php?module=tradingus_quote&_spAction=generateProductFormSubmit&showHTML=0";
    ...
    function getDeleteCheckedProductsLinked() {
        ...
        $deleteSql = "
        DELETE FROM order_item
        WHERE order_id = {$orderRec['order_id']} AND record_id NOT IN
        (SELECT product_id FROM quote_product WHERE quote_id = {$quote_id} AND product_id > 0)
        ";
        $resultDelete = $db->sql_query($deleteSql);
    }
