...
class CP_Admin_Modules_Tradingus_Quote_View extends CP_Common_Lib_ModuleViewAbstract
{
   //========================================================//
    ...
    function getEdit($row){
        ...
        if ($cpCfg['m.tradingsg.quote.summaryDisplayForSplCase'] == 1){
            $summaryDisplay = $this->getSummaryDisplay($row);
        } else {
     ...
     *
     */
    function getRightPanelSyedSept18th($row){
        $fn = Zend_Registry::get('fn');
        ...
        if($cpCfg['m.tradingus.quote.printQuoteGeneral']){
            $printText ="";
            $urlQuote = "index.php?module=tradingus_quote&_spAction=printQuoteExcelGeneral&id={$row['quote_id']}&showHTML=0";
        }
        else{
            $printText ="";
            $urlQuote = "index.php?module=tradingus_quote&_spAction=printQuoteExcel&id={$row['quote_id']}&showHTML=0";
        }
        $urlRaisePo = "index.php?module=tradingus_quote&_spAction=raisePurchaseOrder&id={$row['quote_id']}&showHTML=0";

        if ($cpCfg['m.tradingus.quote.printQuoteGeneralTrading'] == 1) {
            $urlQuoteGeneral = "index.php?module=tradingus_quote&_spAction=printQuoteGeneralTrading&id={$row['quote_id']}&showHTML=0";
        } else {
            $urlQuoteGeneral = "index.php?module=tradingus_quote&_spAction=printQuoteExcelBasic&id={$row['quote_id']}&showHTML=0";
        }

        $formActionCategory = "index.php?_topRm=order&module=tradingus_quote&_spAction=updateMarkupByCategoryForm&quote_id={$row['quote_id']}&showHTML=0";
        $formActionGroup = "index.php?_topRm=order&module=tradingus_quote&_spAction=updateMarkupByGroupForm&quote_id={$row['quote_id']}&showHTML=0";
        $formActionDiscGroup = "index.php?_topRm=order&module=tradingus_quote&_spAction=updateDiscountByGroupForm&quote_id={$row['quote_id']}&showHTML=0";
        $formActionUpdateDiscount = "index.php?_topRm=order&module=tradingus_quote&_spAction=updateDiscountForm&quote_id={$row['quote_id']}&showHTML=0";
        $bulkGenerateUrl  = "index.php?module=tradingus_quote&_spAction=generateBulkProduct&id={$row['quote_id']}&showHTML=0";

        $updateDiscountByGroup = '';
        if ($cpCfg['m.tradingus.quote.updateDiscountByGroup'] == 1) {
        	$updateDiscountByGroup = "
        ...
        $urlExportAsPdf = "index.php?module=tradingus_quote&_spAction=printExportAsPdf&id={$row['quote_id']}&showHTML=0";
        ...
        if ($row['quote_type'] == 'General Quotation') {
            ...
            <div class='floatbox  btnbackground'>
    	        <div class='actionBtnsDetail actionbtnwidth '>
    	            ...
    	            <div class='button mb5'>
    	                <a href='#' id='raiseInvoice' quote_id='{$row['quote_id']}'>Generate Finance Records</a>
    	            </div>
    	        </div>
    	        <div class='actionBtnsDetail actionbtnwidth'>
    	            <div class='button mb5'>
    	                <a href='{$formActionGroup}' id='updateMarkupByGroup'>Update Markup by Department</a>
    	            </div>
    	            <div class='button mb5'>
        ...
        $urlInvoice = "index.php?_topRm=finance&module=tradingus_order&_action=edit&record_id={$orderRec['order_id']}";
        $invoiceLink = '';
        ...
        <div class='subcolumns summary'>
            <div class='c50l'>
            <div class='subcl'>
                {$media->getRightPanelMediaDisplay("Attachments", "tradingus_quote", "attachment", $row)}
            </div>
            </div>
            <div class='c50r'>
            <div class='subcr'>
                {$displayLinkData->getLinkPortalMain('tradingus_quote', 'tradingsg_expenseLink', 'Expense Linked', $row)}
            </div>
            </div>
        </div>

        {$printText}
        {$displayLinkData->getLinkPortalMain('tradingus_quote', 'tradingsg_productLink',
                                             'Products Linked - No. of items (' . $rowCount['total'] .')
                                             ' . $invoiceLink .' ' . $bulkAdd .'
                                              ', $row)}
        {$displayLinkData->getLinkPortalMain('tradingus_quote', 'tradingsg_purchaseOrderLink', 'Purchase Order Linked', $row)}
        ";
    ...
    function getRightPanel($row){
        ...
        if ($row['quote_type'] == 'General Quotation') {
            ...
            <div class='floatbox  btnbackground'>
    	        <div class='actionBtnsDetail actionbtnwidth '>
    	            {$generalQuotation}
    	            <div class='button mb5'>
    	                <a href='#' id='convertClientRequirement' quote_id='{$row['quote_id']}'>Convert GQ to Client Requirement</a>
    	            </div>
    	        </div>
    	        <div class='actionBtnsDetail actionbtnwidth'>
    	            {$ExportToExcellC1}
            ...
            <div class='floatbox  btnbackground'>
    	        ...
    	        <div class='actionBtnsDetail actionbtnwidth'>
    	            <div class='button mb5'>
    	                <a href='{$formActionGroup}' id='updateMarkupByGroup'>Update Markup </a>
    	            </div>
    	            <div class='button mb5'>
    	                <a href='{$formActionUpdateDiscount}' id='updateDiscountForm'>Update Discount</a>
    	            </div>
        ...
        <!--{$displayLinkData->getLinkPortalMain('tradingus_quote', 'tradingsg_productLink',
                                             ...
                                             ' . $invoiceLink .' ' . $bulkAdd .'
                                              ', $row)}-->
        <div id='quoteProductLinkPanel'>
            {$this->getQuoteProductLink($row['quote_id'])}
        </div>
        {$displayLinkData->getLinkPortalMain('tradingus_quote', 'tradingsg_purchaseOrderLink', 'Purchase Order Linked', $row)}
        ";
    ...
    function getQuoteProductLink($quote_id = '') {
        ...
        $quoteRecord = $fn->getRecordRowByID('quote', 'quote_id', $quote_id);
        ...
        $bulkGenerateUrl  = "index.php?module=tradingus_quote&_spAction=generateBulkProduct&id={$quote_id}&showHTML=0";
        ...
        $sqlmarkupType = $fn->getValueListSQL('markUpTpe');
        ...
        SELECT qp.quote_product_id
              ...
              (SELECT qphist.cost_price
                ...
                LIMIT 0 , 1)
                ,CONCAT_WS(''
                ,'<a href=index.php?_topRm=order&module=tradingus_quote&_spAction=previousOrderForClient&showHTML=0&quote_product_id=', qp.quote_product_id, '  class=productViewHistory', '> View</a>', '::'
                ,(SELECT qphist.cost_price
                FROM quote_product qphist
                LEFT JOIN quote qt ON ( qt.quote_id = qphist.quote_id )
                WHERE qt.company_id = q.company_id
        ...
        $total_cost_price_sum           = 0;
        $mark_up_amount_sum             = 0;
        ...
        while ($row = $db->sql_fetchrow($result)) {
            ...
            if ($row['product_id'] != ''){
                ...
                AND qphist.product_id = {$row['product_id']}
                AND qphist.product_id > 0
                AND qphist.quote_product_id != {$row['quote_product_id']}
                ORDER BY qphist.quote_id
                ";
                ...
                WHERE poprd.product_id = {$row['product_id']}
                ORDER BY c.company_name
                ";
                ...
                if ($numRowsQP > 0 || $numRowsPP > 0){
                    $viewHistory = "
                    <a href='index.php?_topRm=order&module=tradingus_quote&_spAction=previousOrderForClient&showHTML=0&quote_product_id={$row['quote_product_id']}'  class='productViewHistory'> View</a>
                    ";
                }
            }

            $rows .= "
            <tr recid='{$row['quote_product_id']}' class='portal-row2 row-tradingus_quote__tradingsg_productLink'>
                <td class='item-code w25'>{$row['item_code']}</td>
                <td class='product-title w300'>
                    <input type='text' value='{$row['product_title']}' name='product_title'>
                </td>
                <td class='client-id w75 client_id'>
                    <select name='client_id'>
                        <option value=''>Please Select</option>
                        {$dbUtil->getDropDownFromSQLCols2($db, $sqlSupplier, $row['client_id'])}
                    </select>
                </td>
                <td class='cost-price w50 al-right'>
                    <input type='text' value='{$row['cost_price']}' name='cost_price'>
                </td>
                <td class='unit w35'>{$row['unit']}</td>
                <td class='qty w50 al-right'>
                    <input type='text' value='{$row['qty']}' name='qty'>
                </td>
                <td class='view-history w35'>
                    {$viewHistory}
                </td>
                <td class='total-cost-price al-right w50 totalCp'>
                    {$row['total_cost_price']}
                </td>
                <td class='mark-up-type w75'>
                    <select name='mark_up_type'>
                        <option value=''>Please Select</option>
                        {$dbUtil->getDropDownFromSQLCols1($db, $sqlmarkupType, $row['mark_up_type'])}
                    </select>
                </td>
                <td class='mark-up-amount w50 serviceCostSum'>
                    <input type='text' value='{$row['mark_up_amount']}' name='mark_up_amount'>
                </td>
                <td class='selling-price-amount al-right w50'>
                    {$row['selling_price_amount']}
                </td>
                <td class='discount-percentage-amount al-right discountSum'>
                    {$row['discount_percentage_amount']}
                </td>
                <td class='total-selling-price al-right w50 totalSp'>
            ...
            $total_cost_price_sum           = $row['total_cost_price_sum'];
            $mark_up_amount_sum             = $row['mark_up_amount_sum'];
            $total_selling_price_sum        = $row['total_selling_price_sum'];
            $total_selling_price           += $row['selling_price_amount'];
        }
        ...
        $total_selling_price = number_format($total_selling_price, 2);
        ...
        <table keyfld='quote_product_id' id='quoteProductLink'>
            <thead>
                <tr>
                    <th class='item-code w25'>Item Code</th>
                    <th class='name-of-the-item w300'>Name of the Item</th>
                    <th class='supplier w75 client_id'>Supplier</th>
                    <th class='cp w50'>CP</th>
                    <th class='uom w35'>UOM</th>
                    <th class='qty w50'>Qty</th>
                    <th class='view-history w35'>History</th>
                    <th class='tcp al-right w50 totalCp'>TCP</th>
                    <th class='mark-up-type w75'>Mark Up Type</th>
                    <th class='mark-up w50 serviceCostSum'>Mark Up</th>
                    <th class='sp al-right w50'>SP</th>
                    <th class='discount al-right discountSum'>Discount%</th>
                    <th class='tsp-disc al-right w50 totalSp'>TSP-Disc</th>
                    <th class='delete w25'>Delete</th>
                </tr>
            </thead>
            <tbody>
                <tr class='summary-row'>
                    <th colspan='7'></th>
                    <th class='al-right w50 totalCp'>{$total_cost_price_sum}</th>
                    <th></th>
                    <th class='w25 serviceCostSum'>{$mark_up_amount_sum}</th>
                    <th class='al-right selling_price_amount_sum'>{$total_selling_price}</th>
                    <th class='al-right discountSum'>{$discount_percentage_amount_sum}</th>
    ...
    function getSummaryDisplayGeneralTrading($row){
        ...
        SELECT (SUM(qp.selling_price * qp.qty)) AS total_selling_price
              ...
              ,(SUM(qp.discount_percentage)) AS total_discount_percentage
              ,(count(qp.qty)) AS total_count
              ,(SUM(((qp.selling_price * qp.discount_percentage)/100) * qp.qty)) AS total_discount
              ,(SELECT
              ($subSqlForPercentSum)
               +
              ($subSqlForValueSum)
        ...
        /* Finding Cost Price of Non Haram Products */
        $total_cost_price_non_haram = $rowQuoteProd['total_cost_price'];
        $gst_for_non_haram = $total_cost_price_non_haram * $cpCfg['amtForGSTCalc'] / 100;
        $gst_for_non_haram_format = number_format($gst_for_non_haram, 2);
        ...
        $customer_type = $companyRec['customer_type'];
        ...
        <div>
            <div class='linkPortalDataWrapper'>
                <table class='thinlist mb10'>
                    ...
                    <tr>
                        <td>{$printTaxName}</td>
                        <td class='txtRight'>{$gst_for_non_haram_format}</td>
                    </tr>
     ...
     */
     function getUpdateMarkupByGroupForm() {
        ...
        $formAction = "index.php?_topRm=order&module=tradingus_quote&_spAction=updateMarkupByGroupFormSubmit&showHTML=0";
     ...
     */
     function getUpdateDiscountByGroupForm() {
        ...
        $formAction = "index.php?_topRm=order&module=tradingus_quote&_spAction=updateDiscountByGroupFormSubmit&showHTML=0";
     ...
     */
     ...
     function getUpdateDiscountForm() {
        ...
        $formAction = "index.php?_topRm=order&module=tradingus_quote&_spAction=updateDiscountFormSubmit&showHTML=0";
     ...
     */
     function getUpdateMarkupByCategoryForm() {
        ...
        $formAction = "index.php?_topRm=order&module=tradingus_quote&_spAction=updateMarkupByCategoryFormSubmit&showHTML=0";
    ...
    function getAddNotePo() {
        ...
        $formAction = "index.php?_topRm=order&module=tradingus_quote&_spAction=addNoteFormSubmit&showHTML=0";
    ...
    function getPrintQuoteGeneralTrading() {
        ...
        */
        ...
        while ($row = $db->sql_fetchrow($result)) {
            ...
            $arr['quote_currency']  = $row['quote_currency'];
            $arr['sub_total']       = $row['sub_total'];
            $arr['payment_terms']   = $row['payment_terms'];
            $arr['discount']        = $row['discount_percentage_amount_sum'];

    		$arr['discount_percent'] = number_format($arr['discount'] * 100 / $arr['sub_total'], 2);
            //$arr['discount_percent'] = number_format($discountPercent,2);
    ...
    function getPreviousOrderForClient() {
        ...
        SELECT q.quote_code
              ...
              ,qp.qty
              ,format(
                ((qp.selling_price -(qp.selling_price * (qp.discount_percentage / 100))
                - qp.cost_price
                ) * qp.qty)
                ,2) as profit
        FROM quote_product qp
        ...
        WHERE q.company_id = {$quoteRec['company_id']}
          AND qp.product_id = {$quoteProductRec['product_id']}
        ...
        if ($numRows == 0) {
            ...
            while ($row = $db->sql_fetchrow($result)) {
                $quote = "<a target='_blank' href='index.php?_topRm=order&module=tradingus_quote&record_id={$row['quote_id']}&_action=edit'><u>{$row['quote_code']}</u></a>";
                $rows .= "
                <tr>
                    <td>{$quote}</td>
                ...
                ";
                $companyName_client = $row['company_name'];
            }
        ...
        /*$sqlSupplier = "
        SELECT c.company_name as supplier_name
              ,qp.cost_price
              ,DATE_FORMAT(q.quote_date, '%d-%m-%Y') AS quote_date
              ,qp.qty
              ,format(
                ...
                ,2) as profit
        FROM quote_product qp
        LEFT JOIN (purchase_order po) ON (qp.product_id = po.product_id)
        ...
        SELECT q.quote_code
              ...
              ,poprd.price
              ,DATE_FORMAT(poprd.creation_date, '%d-%m-%Y') AS po_date
              ,poprd.qty
        FROM po_product poprd
        LEFT JOIN (company c) ON (poprd.supplier_id = c.company_id)
        LEFT JOIN (quote q) ON (poprd.quote_id = q.quote_id)
        WHERE poprd.product_id = {$quoteProductRec['product_id']}
        ORDER BY c.company_name
        LIMIT 0, 5
        ";
        ...
        if ($numRows == 0) {
            ...
            <div class='header mt20' expanded='1'>
                <div class='floatboxQuoteHistory'>
                    <div class='float_left'>Purchase History</div>
                </div>
            </div>
            <table class='thinlist'>
                <td>Sorry, no previous Purchase records from any supplier</td>
            </table>";
        }
        else{
            $rows = '';
     ...
     *
     */
    function getPrintQuoteThermalPrinter() {
        $cpCfg = Zend_Registry::get('cpCfg');
        $ln = Zend_Registry::get('ln');
        $fn = Zend_Registry::get('fn');
        $db = Zend_Registry::get('db');
        ...
        $text="
            ...
            <h1>How to directly Print Excel XLS without Preview or Printer Dialog</h1>
            ...
            <br /><br />
            <input type='button' style='font-size:18px' onclick='javascript:jsWebClientPrint.print('useDefaultPrinter=' + $('#useDefaultPrinter').attr('checked') + '&printerName=' + $('#installedPrinterName').val());' value='Print Excel Spreadsheet...' />
        ...
        //echo WebClientPrint::createScript(Utils::getRoot().'/PrintExcelSample/PrintExcel.php')
        //echo WebClientPrint::createScript("http://printreceipt.localhost/PrintExcel.php")
        echo WebClientPrint::createScript("http://trademsart.localhost/index.php?module=tradingus_quote&_spAction=printQuoteThermalPrinter");
     }
}