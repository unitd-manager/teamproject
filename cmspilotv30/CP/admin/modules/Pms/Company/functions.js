...
$('a#addCourseRows').livequery('click', function(){
    ...
    var url = $('#scopeRootAlias').val() + 'index.php?module=pms_orderLink&_spAction=courseRow&showHTML=0';
...
$('a.newContactDetails').livequery('click', function(e) {
    ...
    var exp = {
        callbackOnSuccess: function(){
            ...
            $('#dialog1').remove();
            var url = 'index.php?module=pms_orderLink&_spAction=selectedTraineeResultRow&showHTML=0';
            Util.showProgressInd();
    ...
    var expObj = {
       ...
       ,callbackOnSuccess: function(){
            ...
            Util.alert(msg, function(){
                ...
                Links.reloadPortalRecords('pms_company#pms_orderLink', 'pms_company');
...
cpm.pms.company.init = function(){
    $(window).load(function(){
        $('.pms_company__pms_orderLink select[name=course_id]').livequery(function(){
           cpm.pms.company.populateBatch.call(this);
        });
        
        $('.m-pms_company #traineeSelectedForm #fld_course_id').livequery('change', function(){
            ...
            cpm.pms.company.populateBatch.call(this);
            cpm.pms.company.populateSubsidy.call(this);
            cpm.pms.company.populateDiscount.call(this);
            Util.hideProgressInd();
        });
    });
}

cpm.pms.company.populateBatch = function(){
    Util.showProgressInd();
    var parent = $(this).closest('tr');
    $(this).each(function(){
        courseId = $(this).val();
        //batchObj = $('select[name=batch_id]', parent);
        batchObj = $('.m-pms_company #traineeSelectedForm #fld_batch_id');
...
cpm.pms.company.populateSubsidy = function(){
    courseId = $(this).val();
    $('.traineesSelectedLinked tr').each(function(){
        ...
        subsidyObj = $('.m-pms_company #traineeSelectedForm #fld_course_subsidy_history_id');
...
cpm.pms.company.populateDiscount = function(){
    var parent = $(this).closest('tr');
    $(this).each(function(){
    //alert (123);
        courseId = $(this).val();
        discountObj = $('select[name=discount_id]', parent);
        discountObj = $('.m-pms_company #traineeSelectedForm #fld_discount_id');
...
cpm.pms.company.calculateCourseSummaryTotal = function(){
...
cpm.pms.company.populateCourseSummary = function(){
    ...
    course_id = $('.m-pms_company #traineeSelectedForm #fld_course_id').val();
    var url = 'index.php?module=pms_courseLink&_spAction=courseCompanySummary&num_rows=' + num_rows + '&showHTML=0';
    
    $.get(url, {course_id: course_id}, function(html){
        $('tr#courseAmount').html(html);
        $('table#courseSummary').removeClass('hideme');
        cpm.pms.company.calculateCourseSummaryTotal();
    });
}
