<?
class CP_Admin_Modules_Pms_Parent_Model extends CP_Common_Modules_Pms_Parent_Model
{
    /**
     *
     */
     ...
     *
     */
	function getPmsParentPmsContactLinkSQL($id) {
    ...
    function getGenerateDda(){
        ...
        $parentRec   = $fn->getRecordRowByID('parent', 'parent_id', $parent_id);
        $parent_code = substr($parentRec['parent_code'], 4, 4);

        $fa = array();
        $fa['dda'] = $parent_code;

        $whereCondition = "WHERE parent_id = {$parent_id}";
        $SQLUpdate      = $dbUtil->getUpdateSQLStringFromArray($fa, 'parent', $whereCondition);
        $resultUpdate   = $db->sql_query($SQLUpdate);
     ...
     *
     */
    function getParentTransferFormSubmitLatest1() {
        $fn = Zend_Registry::get('fn');
        $validate = Zend_Registry::get('validate');
        $db = Zend_Registry::get('db');
        $dbUtil = Zend_Registry::get('dbUtil');
        ...
        */
        ...
        $contact_id_arr = $fn->getReqParam('contact_id', array());
        $site_id_arr    = $fn->getPostParam('site_id', array());
        $batch_id_arr   = $fn->getPostParam('batch_id', array());
        
        $count = count($batch_id_arr);
        for ($i = 0; $i < $count; $i++){
            if (count($batch_id_arr)) {
                $batch_id   = $batch_id_arr[$i];
                $site_id    = $site_id_arr[$i];
                $contact_id = $contact_id_arr[$i];
            } else {
                $batch_id = '';
                continue;
            }
            ...
            if ($batch_id != '') {            
                ...
                $wherePar   = "WHERE contact_id = {$contact_id}";
                $sqlPar     = $dbUtil->getUpdateSQLStringFromArray($faPar, 'parent_contact', $wherePar);
                $resultPar  = $db->sql_query($sqlPar);
                ...
                /* Updating Site id in Course Contact table and Order Table*/
                $current_year = date('Y');
                $sqlCc = "
                SELECT cc.course_contact_id, cc.order_id FROM course_contact cc
                WHERE cc.contact_id = {$contact_id}
                  AND cc.year_of_enrollment >= {$current_year}
                ";
                $resultCc = $db->sql_query($sqlCc);
                while ($rowCc = $db->sql_fetchrow($resultCc)) {
                    ...
                    $whereOrder         = "WHERE order_id = {$rowCc['order_id']}";
                    $sqlOrder           = $dbUtil->getUpdateSQLStringFromArray($faOrder, 'order', $whereOrder);
                    $resultOrder        = $db->sql_query($sqlOrder);
                    ...
                    $sqlInv = "
                    SELECT i.invoice_id FROM invoice i
                    WHERE i.order_id = {$rowCc['order_id']}
                      AND i.status = 'Due'
                    ";
                    $resultInv = $db->sql_query($sqlInv);
                    while ($rowInv = $db->sql_fetchrow($resultInv)) {
    ...
    function getParentTransferFormSubmitLatest() {
        ...
        */
        ...
        $course_contact_id_arr = $fn->getReqParam('course_contact_id', array());
        $site_id_arr           = $fn->getPostParam('site_id', array());
        $batch_id_arr          = $fn->getPostParam('batch_id', array());
        
        $count = count($site_id_arr);        
        for ($i = 0; $i < $count; $i++){
            $batch_id          = $batch_id_arr[$i];
            $site_id           = $site_id_arr[$i];
            $course_contact_id = $course_contact_id_arr[$i];
            
            if ($site_id) {
                $ccRec      = $fn->getRecordRowById('course_contact', 'course_contact_id', $course_contact_id);
                $contact_id = $ccRec['contact_id'];
                ...
                $whereCc      = "WHERE course_contact_id = {$course_contact_id}";
                $sqlUpdate    = $dbUtil->getUpdateSQLStringFromArray($faCc, 'course_contact', $whereCc);
                $resultUpdate = $db->sql_query($sqlUpdate);
                ...
                $whereOrder         = "WHERE order_id = {$ccRec['order_id']}";
                $sqlOrder           = $dbUtil->getUpdateSQLStringFromArray($faOrder, 'order', $whereOrder);
                $resultOrder        = $db->sql_query($sqlOrder);
                ...
                $sqlInv = "
                SELECT i.invoice_id FROM invoice i
                WHERE i.order_id = {$ccRec['order_id']}
                  AND i.status = 'Due'
                ";
                $resultInv = $db->sql_query($sqlInv);
                while ($rowInv = $db->sql_fetchrow($resultInv)) {
