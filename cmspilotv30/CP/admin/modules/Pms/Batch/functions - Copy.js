...
cpm.pms.batch.init = function(){
    ...
    $('.studentGrade').live('click', function(e){
        Util.showProgressInd();
        var batch_id = $(this).attr('batch_id');
        var url = 'index.php?module=pms_batch&_spAction=studentGrade&id=' +  batch_id + '&showHTML=0';
        var exp = {
            url: url
           ,onCloseFn: cpm.pms.batch.studentGradePortalReload
        };
        Util.openDialogForLink('Student Grade',  900, 600, 0, exp);
    });
    ...
    $('.studentAssessment #fld_exam_type').livequery('change', cpm.pms.batch.showHideSubject);
    ...
    //$('.viewSurah').livequery('click', cpm.pms.batch.newAssessmentSurah);

    $('.addAssessmentSurahForm input[name=student_subject]').livequery('change', cpm.pms.batch.newAssessmentSurahSubject);

    $('.addAssessmentSurahForm select[name=iqrayear]').livequery('change', cpm.pms.batch.newAssessmentSurahYear);
    $('.editAssessmentSurahForm .fld_iqrayear_row').livequery('change', cpm.pms.batch.newAssessmentSurahYear);
    $('.iqraRemarks textarea[name=remarks]').livequery('change', cpm.pms.batch.addRemarksToIqra);
    $('.viewRemarks textarea[name=remarks_main]').livequery('change', cpm.pms.batch.addRemarksToStudentGrade);
    ...
    $('.viewSurahLink').livequery('click', cpm.pms.batch.viewSurahLink);
}
