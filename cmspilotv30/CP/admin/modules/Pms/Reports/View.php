<?
class CP_Admin_Modules_Pms_Reports_View extends CP_Common_Lib_ModuleViewAbstract
{
    ...
    function getReportsDropdown() {
        ...
        if ($cpCfg['m.pms.reports.showMonthlyEnrollmentForPvt']){
            $enrollmentForPvt .= "
        ...
        if ($cpCfg['m.pms.reports.showStudentStatusReports']){
        ...
        if ($cpCfg['m.pms.reports.showStudentProgressionReports']){
            $studentProgressionReports .= "<option value='studentProgressionReports'>Student Progression Reports</option>";
        }

        //Attendance Reports : Used in Mass Ims
        if ($cpCfg['m.pms.reports.showAttendanceReportBySubject']){
        ...
        if ($cpCfg['m.pms.reports.showStaffAttendanceReport']){
        ...
        if ($cpCfg['m.pms.reports.showStaffAttendanceOverallReport']){
        ...
        if ($cpCfg['m.pms.reports.showDailyAccountsReport']){
        ...
        if ($cpCfg['m.pms.reports.showMarketingCallByStaffReport']){
        ...
        if ($cpCfg['m.pms.reports.showMarketingCallOverallReport']){
        ...
        if ($cpCfg['m.pms.reports.showPaymentOutstandingReport']){
            $paymentOutstandingReport = "<option value='paymentOutstandingReport'>Financial Ageing Report of Student</option>";
        }

        //Used in Enterprise system
        if ($cpCfg['m.pms.reports.showTeacherAttendanceReport']){
        ...
        if ($cpCfg['m.pms.reports.showReportsForPvt'] == false){
            
            $specialReport = '';
            if ($cpCfg['m.pms.reports.showSpecialReportsForPvt']) {
                $specialReport = "
                ...
                <td>
                    <select name='report' class='report'>
                        ...
                        <optgroup label='Standard Reports'>
                            ...
                            <option value='studentEnrollmentReport'>Student Enrollment Report</option>
                            <option value='studentStatusReports'>Student Status Reports</option>
                        </optgroup>
                        <optgroup label='Financial Reports'>
                            {$paymentOutstandingReport}
                            <option value='monthlyFinancialReport'>Monthly Outstanding Report</option>
                            <!--<option value='statementofAccountReport'>Statement of Account Report</option>-->
                        </optgroup>
                        <optgroup label='Academic Reports'>
                            <option value='enrollmentByYearReport'>Student Enrollment Summary By Year</option>
                            <option value='enrollmentBySummaryReport'>Student Enrollment Summary By Class / Session</option>
                        </optgroup>
                        <!--<optgroup label='Standard Reports'>
                            {$attendanceReports}
    ...
    function getSearch() {
        ...
        foreach($searchFldsArr AS $searchFld){
            ...
            if ($report == 'paymentOutstandingReport') {
                ...
                ";
            } else if ($report == 'monthlyFinancialReport'){
                if ($month == '') {
                    $month = date('m');
                }
                ...
                $rows .= "               
                <td class='month_drop_down'>
                    <select name='month' class='ml10 mr10'>
                        {$cpUtil->getDropDownFromArr($monthArr, $month)}
                    </select>
                </td>
                
                <td class='year_drop_down'>
                    <select name='year' class='monthlyFinancialReport'>
                        <option value=''>Enrollment Year</option>
                        {$dbUtil->getDropDownFromSQLCols1($db, $sqlYear, $year)}
                    </select>
                </td>
                ...
                if ($report == 'studentEnrollmentReport') {
                    $rows .= "
                    <td class='new_student_drop_down'>
                        <select name='new_student'>
                ...
                if ($report == 'statementofAccountReport') {
                    $sqlParent = "
                    ...
                    $rows .= "               
                    <td class='parent_id pl10'>
                        <select name='parent_id'>
                            <option value=''>Parent</option>
                            {$dbUtil->getDropDownFromSQLCols2($db, $sqlParent, $parent_id)}
                        </select>
                    </td>
                    ";
                }
    ...
    function getDisplayReport($text){
        ...
        if ($report == 'receiptSummary' || $report == 'paymentOutstandingReport' 
         || $report == 'studentEnrollmentReport' 
         || $report == 'monthlyFinancialReport'
         || $report == 'enrollmentByYearReport'
         || $report == 'enrollmentBySummaryReport'
