...
Util.createCPObject('cpm.pms.giroPayment');

cpm.pms.giroPayment = {
    init: function(){
        $('.click-all-top .check-all').click(cpm.pms.giroPayment.checkAllCol);
        $('.click-all-top .uncheck-all').click(cpm.pms.giroPayment.uncheckAllCol);
    },
...
$('#btnGenerateDBSTxtFile').livequery('click', function (e){
    ...
    if (!confirm(msg)){
        ...
        var url = 'index.php?_topRm=finance&module=pms_giroPayment&_spAction=generateDBSTxtFile&showHTML=0';
        Util.showProgressInd();
        //var receipt_code = $(this).attr('receipt_code');
        //$.get(url,{receipt_code: receipt_code}, function(){
        $.get(url,{}, function(){
...
$('.m-pms_giroPayment form.giroForm .invoiceCode').livequery('click', function(e){
    var invoice_code  = $(this).val();
    var is_checked  = $(this).is(':checked');

    var url = 'index.php?module=pms_giroPayment&_spAction=setInvoiceCodeForSession&showHTML=0';
...
$('.m-pms_giroPayment form.giroForm .invoiceCodeToPrint').livequery('click', function(e){
    var invoice_code  = $(this).val();
    var is_checked  = $(this).is(':checked');

    var url = 'index.php?module=pms_giroPayment&_spAction=setInvoiceCodeToPrintForSession&showHTML=0';
...
$('.m-pms_giroPayment #giroSearch a.selectedStudents').livequery('click', function (e){
    e.preventDefault();
    var month_val = $(this).attr('month_val');
    
    var url = 'index.php?module=pms_giroPayment&_spAction=displayGiroFailures&month_val=' + month_val + '&showHTML=0';
    Util.showProgressInd();
    $.get(url, function(html){
        Util.hideProgressInd();
        //window.location.reload(true);
        $('.m-pms_giroPayment .room-giroPayment-table').html(html);
    });
});

/*$('.m-pms_giroPayment #giroSearch a.failedStudents').livequery('click', function (e){
    e.preventDefault();
    var month_val = $(this).attr('month_val');
    
    var url = 'index.php?module=pms_giroPayment&_spAction=failedStudents&month_val=' + month_val + '&showHTML=0';
    Util.showProgressInd();
    $.get(url, function(){
        Util.hideProgressInd();
        //window.location.reload(true);
