<?
class CP_Admin_Modules_Pms_BatchTransfer_View extends CP_Common_Lib_ModuleViewAbstract
{
    function getList(){
        ...
        $modGroupCourse = getCPModuleObj('pms_course');
        $sqlCourse = $modGroupCourse->model->getCourseSQL();        
        $courseRow = $formObj->getDDRowBySQL($modulesArr['pms_course']['title'], 'enrollment_course_id', $sqlCourse, '', array('rowCls' => 'showme'));
        
        $sqlYear = "SELECT DISTINCT year_of_enrollment FROM course_contact";
        $expYear = array('sqlType' => 'OneField');

        $batchRow = $formObj->getDDRowBySQL($modulesArr['pms_batch']['title'], 'enrollment_batch_id', '', '', array('rowCls' => 'hideme'));

        $formAction = "index.php?module=pms_batchTransfer&_spAction=batchTransferStudentSubmit&showHTML=0";
    ...
    function getCourseTraineeSearchForm(){
        ...
        $modGroupCourse = getCPModuleObj('pms_course');
        $sqlCourse = $modGroupCourse->model->getCourseSQL();        
        $courseRow = $formObj->getDDRowBySQL($modulesArr['pms_course']['title'], 'course_id', $sqlCourse);
        
        $sqlYear = "SELECT DISTINCT year_of_enrollment FROM course_contact ORDER BY year_of_enrollment DESC LIMIT 0,2";
        $expYear = array('sqlType' => 'OneField');

        $batchRow = $formObj->getDDRowBySQL($modulesArr['pms_batch']['title'], 'batch_id', '', '', array('rowCls' => 'hideme'));

        $action ='index.php?module=pms_batchTransfer&_spAction=studentSearchResult&showHTML=0';
        $text = "
    ...
    function getStudentSearchResult(){
        ...
        while ($row = $db->sql_fetchrow($result)) {
            ...
            <tr>
                ...
                <td class='txtCenter'>{$row['registration_no']}</td>
