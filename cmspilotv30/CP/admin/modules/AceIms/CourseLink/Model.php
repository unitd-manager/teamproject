<?
class CP_Admin_Modules_AceIms_CourseLink_Model extends CP_Common_Lib_ModuleLinkModelAbstract
{
    function getFields(){
    ...
    */
    function getAddCoursePvtLinkValidate() {
        ...
        if ($course_type == 'Long Term') {
            ...
            if ($count == 0 && $course_id != '') {
                $msg = 'Please check atleast one subject for enrollment';
        ...
        //if ($fa['course_id'] > 0){
            ...
            */
            ...
            //if there is change in subsidy/batch record below code will update the subsidy in order items
                ...
                $expOrderItemSubsidy = array('condn' => " AND module = 'aceIms_subsidy'");
                ...
                if ($discount){
                    $expOrderItemDiscount = array('condn' => " AND module = 'aceIms_discount'");
    ...
    */
    function getSaveCoursePvtLink() {
        ...
        if($existingCCRec['course_id'] != $course_id){
            ...
            $expOrderItemCourse = array('condn' => " AND module = 'aceIms_course'");
    ...
    */
    ...
    /**
     * Generate Invoice and Invoice Item for Order
     */
    function getGenerateInvoiceAndInvoiceItemForOrder($order_id, $total_amount_payable) {
        $fn = Zend_Registry::get('fn');
        $db = Zend_Registry::get('db');

        $modObj = getCPModuleObj('aceIms_invoice');
        $invoice_code = $modObj->model->getFetchInvoiceCode($order_id);
        ...
        while ($rowOi = $db->sql_fetchrow($resultOi)) {
            ...
            $faInvItem['course_type']       = $rowOi['course_type'];
    ...
    /**
     * Generate Receipt and Invoice Receipt History for Order and Update Invoice Status to Paid
     */
    function getGenerateReceiptForOrder($order_id) {
        $fn = Zend_Registry::get('fn');
        $db = Zend_Registry::get('db');

        $modObj = getCPModuleObj('aceIms_receipt');
        $receipt_code = $modObj->model->getFetchReceiptCode($order_id);

        $invoiceRec = $fn->getRecordByCondition('invoice', "order_id = {$order_id} AND status != 'Cancelled'");
        $gst_amount = 0;
        ...
        $total_amount_payable = $invoiceRec['invoice_amount'] + $gst_amount;
        ...
        $faRec['year_of_enrollment'] = $invoiceRec['year_of_enrollment'];

        $orderRec = $fn->getRecordRowByID('order', 'order_id', $order_id);
        $faRec['company_contact_salutation'] = $orderRec['company_contact_salutation'];
        $faRec['company_contact_name']       = $orderRec['company_contact_name'];
        $faRec['cust_first_name']            = $orderRec['cust_first_name'];
        $faRec['cust_email']                 = $orderRec['cust_email'];
        $faRec['cust_address1']              = $orderRec['cust_address1'];
        $faRec['cust_address2']              = $orderRec['cust_address2'];
        $faRec['cust_address_po_code']       = $orderRec['cust_address_po_code'];
        $faRec['cust_address_country_code']  = $orderRec['cust_address_country_code'];
        $faRec['contact_reg_no']             = $orderRec['contact_reg_no'];
        ...
        $faInvRecHist = array();
        $faInvRecHist['invoice_id']          = $invoiceRec['invoice_id'];
        ...
        $fn->saveRecord($faInv, 'invoice', 'invoice_id', $invoiceRec['invoice_id']);
    }

    /**
     * Update Order status to paid
     */
    function getUpdateOrderStatusToPaid($order_id) {
        $fn = Zend_Registry::get('fn');
        $db = Zend_Registry::get('db');
        ...
        WHERE order_id = {$order_id}
          AND (status = 'Partial Payment' OR status = 'Due')
        ";
        $resultInv  = $db->sql_query($sqlInv);
        $numRowsInv = $db->sql_numrows($resultInv);

        if ($numRowsInv == 0) {
            /* Updating Order record to Paid */
            $faOrder = array();
            $faOrder['order_status']      = 'Paid';
            $faOrder['modification_date'] = date('Y-m-d H:i:s');
            $faOrder['modified_by']       = $fn->getSessionParam('userName');
            $fn->saveRecord($faOrder, 'order', 'order_id', $order_id);
        }
    }
}