<?
class CP_Admin_Modules_AceIms_CourseLink_View extends CP_Common_Lib_ModuleLinkViewAbstract
{
    ...
    */
    function getNew(){
        ...
        $course = 'Course';
        $batch = 'Batch';
        if($cpCfg['m.aceIms.courseLink.hasLabelChangeEnt']){
            $course = 'Class';
            $batch = 'Session';
        }
    ...
    */
    function getSubsidyData($subsidy_discount_id, $course_contact_id, $course_id){
        ...
        FROM subsidy_discount sd
        LEFT JOIN (course_subsidy_history csh) ON (sd.subsidy_discount_id = csh.subsidy_discount_id)
        WHERE sd.course_subsidy_history_id = {$subsidy_discount_id}
        ";
    ...
    */
    function getNewCoursePvtLink(){
        ...
        <form id='portalForm' class='yform columnar' method='post' action='{$formAction}'>
            ...
            <input type='hidden' name='registration_fee' value='{$cpCfg['registrationFeePvt']}' />
            ...
            <table id='courseSummaryPvt' class='thinlist hideme'>
                ...
                <tr id='subjectAmount' class=''></tr>
                <tr id='insuranceAmount'></tr>
    ...
    */
    function getEditCoursePvtLink(){
        ...
        if($row['discount']){
            $trDiscount = getCPModelObj('aceIms_courseLink')->getDiscountValueForPvt('',$row['discount'], $row['course_id'], $medical_insurance, $full_time);
            $discount_amount = getCPModelObj('aceIms_courseLink')->getDiscountValueForPvt(1, $row['discount'], $row['course_id'], $medical_insurance, $full_time);
        }
        else{
            $trDiscount = '';
        }
        ...
        if ($count != ''){
            ...
            if($full_time == 1){
                $subject_total     = getCPModelObj('aceIms_subjectLink')->getAddSubjectAmountToTotal('', 1);
                $trCourse = $subject_total ;

                //to get the subject total amount
                $subject_amount = getCPModelObj('aceIms_subjectLink')->getAddSubjectAmountToTotal(1,1);
            }
            else{
                $subject_total     = getCPModelObj('aceIms_subjectLink')->
                getAddSubjectAmountToTotal('','',$row['no_of_months']);
                $trCourse = $subject_total ;

                //to get the subject total amount
                $subject_amount = getCPModelObj('aceIms_subjectLink')->getAddSubjectAmountToTotal(1,'',$row['no_of_months']);
            }
