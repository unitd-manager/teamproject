<?
class CP_Admin_Modules_AceIms_Course_View extends CP_Common_Modules_AceIms_Course_View
{
    function getList($dataArray){
        ...
        foreach ($dataArray as $row){
            ...
            {$rowGroup}
            {$listObj->getListDataCell($row['price'], 'right')}
        ...
        // This is used to dispaly Program Group
        if($cpCfg['m.aceIms.course.hasProgramGroup']){
            $textGroup = $listObj->getListHeaderCell($modulesArr['aceIms_programGroup']['title'], 'program_title');
        }
        ...
        {$textGroup}
        {$listObj->getListHeaderCell('Fees (SGD)', 'price', 'headerRight')}
    ...
    */
    function getNew(){
        ...
        // This is used to dispaly Program Group
        if($cpCfg['m.aceIms.course.hasProgramGroup']){
            $modGroup = getCPModuleObj('aceIms_programGroup');
            $sqlGroup = $modGroup->model->getProgramGroupSQL();

            $textGroup = $formObj->getDDRowBySQL($modulesArr['aceIms_programGroup']['title'], 'program_group_id', $sqlGroup);
        }
    ...
    */
    function getEdit($row){
        ...
        if($cpCfg['m.aceIms.course.hasProgramGroup']){
            ...
            $modGroup = getCPModuleObj('aceIms_programGroup');
            $sqlGroup = $modGroup->model->getProgramGroupSQL();
            $textGroup = $formObj->getDDRowBySQL($modulesArr['aceIms_programGroup']['title'], 'program_group_id', $sqlGroup, $row['program_group_id'], $expGroup);
        }
        ...
        {$formObj->getTARow('Course Learning outcome(s)', 'course_learning_outcome', $row['course_learning_outcome'])}
        {$formObj->getTARow('Course Schedule', 'course_schedule', $row['course_schedule'])}
    ...
    */
    function getRightPanel($row){
        ...
        $text = "
        {$displayLinkData->getLinkPortalMain('aceIms_course', 'aceIms_batchLink', $modulesArr['aceIms_batch']['title'], $row)}
        {$displayLinkData->getLinkPortalMain('aceIms_course', 'aceIms_subjectLink', 'Subject Linked', $row)}
        {$displayLinkData->getLinkPortalMain('aceIms_course', 'aceIms_courseSubsidyLink', 'Subsidy Discount Link', $row)}
        ";
    ...
    */
    function getQuickSearch() {
        ...
        $courseType = '';
        if($cpCfg['m.aceIms.course.otherDetailsPvt']){
            $courseType = "
            <td>
