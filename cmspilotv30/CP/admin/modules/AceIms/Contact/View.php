<?
class CP_Admin_Modules_AceIms_Contact_View extends CP_Common_Modules_AceIms_Contact_View
{
    ...
    function getEdit($row){
        ...
        if($cpCfg['m.aceIms.contact.otherDetailsPvt'] == false){
    ...
    function getFullWidthContactEdit($row){
        ...
        $additionalDetails .= "
	    <tr>
	        <td colspan='4' class='innerHeading'>International Student Details</td>
	    </tr>

	    <tr class='citizenNo{$hide}'>
	        <td>{$formObj->getDateRow('Date of Arrival', 'date_of_arrival', $row['date_of_arrival'])}</td>
            <td>{$formObj->getDDRowBySQL('Passport Country of Issue', 'passport_country_issued', $sqlCountry, $row['passport_country_issued'])}</td>
	        <td>{$formObj->getTBRow('Overseas Number', 'overseas_contact_no', $row['overseas_contact_no'])}</td>
            <td>{$formObj->getDateRow('STP Application Date', 'stp_application_date', $row['stp_application_date'])}</td>
	    </tr>

	    <tr class='citizenNo{$hide}'>
	        <td>{$formObj->getDateRow('STP IPA Date', 'stp_ipa_date', $row['stp_ipa_date'])}</td>
            <td>{$formObj->getDateRow('STP Issue Date', 'stp_issue_date', $row['stp_issue_date'])}</td>
	        <td>{$formObj->getDateRow('STP Expiry Date', 'stp_expiry_date', $row['stp_expiry_date'])}</td>
            <td>{$formObj->getDateRow('STP Cancellation Date', 'stp_cancellation_date', $row['stp_cancellation_date'])}</td>
	    </tr>

	    <tr class='citizenNo{$hide}'>
	        <td>{$formObj->getTBRow('Overseas Address 1', 'overseas_address_flat', $row['overseas_address_flat'])}</td>
            <td>{$formObj->getTBRow('Overseas Address 2', 'overseas_address_street', $row['overseas_address_street'])}</td>
	        <td>{$formObj->getTBRow('Overseas Postal Code', 'overseas_address_po_code', $row['overseas_address_po_code'])}</td>
            <td>{$formObj->getDDRowBySQL('Overseas Country', 'overseas_address_country', $sqlCountry, $row['overseas_address_country'])}</td>
	    </tr>
        ";
                ...
                <div class='linkPortalDataWrapper'>
            		<table class='thinlist'>
            			<tbody>
            			    ...
            			    <tr>
            			        <td colspan='4' class='innerHeading'>Parent Details</td>
            			    </tr>

            			    <tr>
            			        <td>{$formObj->getTBRow('Parent Name', 'emergency_contact_name', $row['emergency_contact_name'])}</td>
                                <td>{$formObj->getTBRow('Parent NRIC / Passport No.', 'parent_id_card_no', $row['parent_id_card_no'])}</td>
            			        <td>{$formObj->getTBRow('Parent Mobile', 'emergency_contact_mobile', $row['emergency_contact_mobile'])}</td>
                                <td>{$formObj->getTBRow('Parent Office No.', 'emergency_contact_office_no', $row['emergency_contact_office_no'])}</td>
            			    </tr>

            			    <tr>
            			        <td>{$formObj->getDDRowBySQL('Parent Passport Country of Issue', 'parent_passport_country_issued', $sqlCountry, $row['parent_passport_country_issued'])}</td>
                                <td>{$formObj->getDDRowBySQL('Parent Nationality', 'parent_nationality', $sqlNationality, $row['parent_nationality'], $expVL)}</td>
            			        <td>{$formObj->getTBRow('Parent Occupation', 'parent_occupation', $row['parent_occupation'])}</td>
                                <td></td>
            			    </tr>
    ...
    */
    function getRightPanel($row){
        ...
        {$media->getRightPanelMediaDisplay('Picture', 'aceIms_contact', 'picture', $row)}
        {$media->getRightPanelMediaDisplay('Attachment', 'aceIms_contact', 'attachment', $row)}
    ...
    */
    function getQuickSearch() {
        ...
        if($cpCfg['m.aceIms.contact.showBatchInSearch']){
            if ($course_id){
                $sqlBatch = "
