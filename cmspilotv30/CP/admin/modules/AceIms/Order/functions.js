$("select[name='subsidy_paid']").change(function(){
    ...
    var url = 'index.php?_topRm=reports&module=aceIms_order&_spAction=updateSubsidyStatus&showHTML=0&order_item_id=' + order_item_id + '&subsidy_paid_status' + subsidy_paid_status;
...
Util.createCPObject('cpm.aceIms.order');

$('.check-all').livequery('click', function (e){
    cpm.aceIms.order.checkAllRow.call(this)
});

$('.uncheck-all').livequery('click', function (e){
    cpm.aceIms.order.uncheckAllRow.call(this)
});
...
$('.cancelInvoice').livequery('click', function (e){
    ...
    if (!confirm(msg)){
        ...
        var url = 'index.php?_topRm=finance&module=aceIms_invoice&_spAction=cancelInvoice&showHTML=0';
        Util.showProgressInd();
...
$('.cancelReceipt').livequery('click', function (e){
    ...
    if (!confirm(msg)){
        ...
        var url = 'index.php?_topRm=finance&module=aceIms_order&_spAction=cancelReceipt&showHTML=0';
        Util.showProgressInd();
        var receipt_code = $(this).attr('receipt_code');
        var order_id     = $(this).attr('order_id');
        $.get(url,{receipt_code: receipt_code, order_id:order_id}, function(){
...
cpm.aceIms.order = {
    init: function(){
        $('.check-all').click(cpm.aceIms.order.checkAllRow);
        $('.click-all-side .uncheck-all').click(cpm.aceIms.order.uncheckAllRow);
    },
    ...
    printOrder:function(){
        ...
        var url = 'index.php?module=aceIms_order&_spAction=printOrder&order_id=' + recordId;
        $.get(url, function(html){
        //alert ('Status Updated Succesfully');
        //Util.hideProgressInd();
        });
...
$('.editReceiptPvt').livequery('click', function (e){
    ...
    $("select[name='mode_of_payment']").livequery('change', function (e){
        cpm.aceIms.order.populatePaymentMode.call(this);
    });
...
$(".clearInvoiceCode").livequery('click', function (e){
    ...
    var url = 'index.php?module=aceIms_order&_spAction=clearInvoice&showHTML=0';
...
$('#generateReceiptPvt').livequery('click', function (e){
    ...
    cpm.aceIms.order.populateReceiptModeOfPayment.call(this);
...
$('#generateMiscReceiptPvt').livequery('click', function (e){
    ...
    $(".miscReceiptFormForPvt input[name='late_fees']").livequery('change', function(e){
        cpm.aceIms.order.populateMiscTotalAmount.call(this);
    });
...
$('input.invoiceCode').livequery('click', function (e){
    ...
    var url = 'index.php?_topRm=finance&module=aceIms_order&_spAction=populateReceiptAmount&showHTML=0';
...
$('input.invoiceCodePvt').livequery('click', function (e){
    ...
    var url = 'index.php?_topRm=finance&module=aceIms_order&_spAction=populateReceiptAmountPvt&showHTML=0';
...
$('input.invoiceCodeMiscPvt').livequery('click', function (e){
    ...
    var url = 'index.php?_topRm=finance&module=aceIms_order&_spAction=populateReceiptAmountMiscPvt&showHTML=0';
    $.get(url,{invoice_hist_id: invoice_hist_id,checkedVal:checkedVal}, function(html){
        $('input[id=fld_amount]').val(html);
        Util.hideProgressInd();
    });
...
$('input.receiptId').livequery('click', function (e){
    Util.showProgressInd();
    receipt_id   = $(this).val();
    var checked    = $(this).attr('checked') ? 'checked' : '';
    var checkedVal = checked == 'checked' ? 1 : 0;

    var url = 'index.php?_topRm=finance&module=aceIms_order&_spAction=populateRefundAmount&showHTML=0';
    $.get(url,{receipt_id: receipt_id, checkedVal: checkedVal}, function(html){
        $('input[id=fld_amount]').val(html);
        Util.hideProgressInd();
    });
});
...
$("#fld_invoice_id").livequery('change', function(){
    ...
    var url = 'index.php?_topRm=finance&module=aceIms_order&_spAction=populateCreditNoteAmount&showHTML=0';
    $.get(url,{invoice_id: invoice_id}, function(html){
        $('input[id=fld_amount]').val(html);
        Util.hideProgressInd();
    });
...
cpm.aceIms.order.populatePaymentMode = function(){
...
cpm.aceIms.order.populateReceiptModeOfPayment = function(){
    $("select[name='mode_of_payment']").livequery('change', function (e){
        var paymentMode = $(this).val();
        if (paymentMode == 'Cheque') {
            Util.showProgressInd();
...
cpm.aceIms.order.populateMiscTotalAmount = function(){
    ...
    var url = 'index.php?_topRm=finance&module=aceIms_order&_spAction=populateMiscTotalAmount&showHTML=0';
...
$(".invoiceDisplay select[name='status']").livequery('change', function (e){
    ...
    var url = 'index.php?module=aceIms_order&_spAction=invoiceRecords&showHTML=0';
...
$('.receiptForm .populateAmountPayable').livequery('click', function (e){
    ...
    var url = 'index.php?module=aceIms_order&_spAction=calculateAmountPayable&showHTML=0';
    Util.showProgressInd();
...
$('#sendReminderEmailToStudent').livequery('click', function (e){
    msg = "Do you like to send email to the Student?";
    if (!confirm(msg)){
        return false;
    } else {
        var url = 'index.php?_topRm=order&module=aceIms_order&_spAction=sendReminderEmail&showHTML=0';
        Util.showProgressInd();
        var order_id = $(this).attr('order_id');
        $.get(url,{order_id: order_id}, function(html){
            /* Checking for one or more receipt for the invoice */
            if (html == 'All installments paid') {
                alert ('The student has paid all his installments');
                Util.hideProgressInd();
            } else {
