<?
class CP_Admin_Modules_AceIms_Order_View extends CP_Common_Lib_ModuleViewAbstract
{
    function getList($dataArray){
        ...
        foreach ($dataArray as $row){
            ...
            if($cpCfg['m.aceIms.ecommerce.order.orderAmountForPvt'] == 1){
                $SQLPvt = "
                SELECT oi.*
                      ,o.order_id
                      ,o.contact_module
                ...
                if($row['registration_type']  == 'Only Registration'){
                    $orderAmount =  $fn->getSettingsValueByKey("registrationFeePvt");
                }
            }
            else{
            ...
            {$listObj->getListDataCell($orderAmount)}
    ...
    function getEdit($row) {
        ...
        if ($cpCfg['cp.forAceIms'] == 0) {
            $mode_of_payment = $formObj->getDDRowByVL('Mode of Payment', 'payment_method',  'paymentType', $row['payment_method'], $expNoEdit);
        }
    ...
    function getRightPanel($row){
        ...
        $bookReceipt = '';
        if ($cpCfg['m.aceIms.order.hasBookReceipt']){
            $bookReceipt = $this->getBookReceiptPortalDisplay($row);
        }
        $bookReceipt = '';

        // This is used to display Insurance Link Portal
        if($cpCfg['m.aceIms.order.hasInsuranceLink'] && $rowCourse['course_type'] != 'Short Term'){
            $links .= $displayLinkData->getLinkPortalMain("aceIms_order", "aceIms_insuranceLink", "Insurance Linked", $row);
        }
    ...
    function getOrderItemPortalDisplay($row){
        ...
        while ($rowOrderItem = $db->sql_fetchrow($result)) {
            ...
            if ($cpCfg['m.aceIms.order.hasCheckBoxForInvoiceItem']) {
        ...
        if($cpCfg['m.aceIms.ecommerce.order.orderItemDisplayForPvt'] == 1){
            if($registration_type  == 'Only Registration'){
                $total =  $fn->getSettingsValueByKey("registrationFeePvt");
            ...
            $expCourse   = array('condn' => " AND module='aceIms_course'");
            ...
            $total = "
                <tr style='background-color:#EAEAE8;text-align:center;font-weight:bold;'>
                    <td class='txtRight' colspan=5>Total $regnText : $total</td>
                    <td></td>
                </tr>
            ";
        }

        $formAction = "index.php?module=aceIms_order&_spAction=generateInvoice&showHTML=0";
        $formActionPvt = "index.php?_topRm=finance&module=aceIms_order&_spAction=generateInvoiceFormPvt&showHTML=0&order_id={$row['order_id']}";
        ...
        if ($recordCount > 0){
            ...
            $invoiceBtn = "<button href='{$formActionPvt}' id='generateInvoicePvt' class='button mt5 ml5 mb20'>Generate Invoice</button>";
        }

        $text = "
        <tr class=''>
        <td>
            <div id='aceIms_company#aceIms_orderLink' class=''>
    ...
    function getInvoicePortalDisplay($row){
        ...
        while ($rowInvoice = $db->sql_fetchrow($result)) {
            ...
            if ($row['contact_type'] == 'Indvidual') {
                $urlPrint = "index.php?_topRm=finance&module=aceIms_order&_spAction=printInvoiceIndividual&invoice_code={$rowInvoice['invoice_code']}&showHTML=0";
            } else {
                $urlPrint = "index.php?_topRm=finance&module=aceIms_order&_spAction=printInvoice&invoice_code={$rowInvoice['invoice_code']}&showHTML=0";
            }
            ...
            $mediaLink = "index.php?plugin=common_media&_spAction=saveMedia&room=aceIms_invoice&recordType=attachment&media_id={$mediaRec['media_id']}&showHTML=0";
            ...
            if($invoice_code == '' || $invoice_code != $rowInvoice['invoice_code']){
                ...
                $editRow = '<td></td>';
                if ($cpCfg['m.aceIms.order.hasEditInvoiceForPvt']){
                    if ($rowInvoice['status'] == 'Due'
                     || $rowInvoice['status'] == ''
                     || $rowInvoice['status'] == 'Partial Payment'
                    ) {
                        $editURL = "index.php?_topRm=finance&module=aceIms_order&_spAction=editInvoiceFormPvt&showHTML=0&invoice_id={$rowInvoice['invoice_id']}&order_id={$row['order_id']}";
                ...
                $deleteRow = '';
                if ($cpCfg['m.aceIms.order.hasDeleteInvoiceForPvt']){
                    $deleteURL = "index.php?_topRm=finance&module=aceIms_order&_spAction=deleteInvoiceFormPvt&showHTML=0&invoice_id={$rowInvoice['invoice_id']}";
            ...
            if ($cpCfg['m.aceIms.ecommerce.order.invoiceForPvt'] == 1){
        ...
        $formActionCreditNote = "index.php?_topRm=finance&module=aceIms_order&_spAction=generateCreditNoteForm&showHTML=0&order_id={$row['order_id']}";

        $creditNote = '';
        if ($cpCfg['m.aceIms.order.hasCreditNoteLink']){
            $formAction = "index.php?_topRm=finance&module=aceIms_order&_spAction=generateReceiptFormPvt&showHTML=0&order_id={$row['order_id']}";
            $creditNote = "
        ...
        $miscFormAction = "index.php?_topRm=finance&module=aceIms_order&_spAction=generateMiscReceiptFormPvt&showHTML=0&order_id={$row['order_id']}";
        ...
        if($cpCfg['m.aceIms.ecommerce.order.receiptForPvt']) {
            ...
            $formAction = "index.php?_topRm=finance&module=aceIms_order&_spAction=generateReceiptForm&showHTML=0&order_id={$row['order_id']}";
            $receiptBtn = "
    ...
    function getInvoiceRecords($row){
        ...
        if ($cpCfg['m.aceIms.ecommerce.order.invoiceForPvt'] == true){
            $SQL = "
            SELECT inst.amount as invoice_hist_amount
                   ,inst.invoice_date as invoice_due_date
                   ,inst.invoice_paid_status as invoice_hist_status
            ...
            ORDER BY inst.installment_id
            ";
        } else if ($cpCfg['m.aceIms.order.invoiceForEnt']){
            $SQL = "
            SELECT i.*
                   ,(
                   SELECT GROUP_CONCAT(r.receipt_code ORDER BY r.receipt_code SEPARATOR ', ')
        ...
        while ($rowInvoice = $db->sql_fetchrow($result)) {
            ...
            if($invoice_code == '' || $invoice_code != $rowInvoice['invoice_code']){
                ...
                $month = '';
                $tdRow = '';
                if ($cpCfg['m.aceIms.order.invoiceForEnt']) {
                ...
                $reportLink = "index.php?module=aceIms_order&_spAction=printInvoiceInFpdf&record_id={$rowInvoice['invoice_id']}&showHTML=0";
        ...
        // This is used to display Receipt for Pvt : Used for Mass IMS
        $miscReceiptBtn = '';
        if ($cpCfg['m.aceIms.order.hasMiscReceipt']){
            $miscFormAction = "index.php?_topRm=finance&module=aceIms_order&_spAction=generateMiscReceiptFormPvt&showHTML=0&order_id={$row['order_id']}";
        ...
        } else if ($cpCfg['m.aceIms.order.receiptForEnt']) {
            $formAction = "index.php?_topRm=finance&module=aceIms_order&_spAction=generateReceiptForEntForm&showHTML=0&order_id={$row['order_id']}";
            $receiptBtn = "
    ...
    function getCreditNotePortalDisplay($row){
        ...
        while ($rowCredit = $db->sql_fetchrow($result)) {
            ...
            $mediaLink = "index.php?plugin=common_media&_spAction=saveMedia&room=aceIms_creditNote&recordType=attachment&media_id={$mediaRec['media_id']}&showHTML=0";
        ...
        <tr class=''>
        <td>
            <div id='' class='linkPortalWrapper aceIms_company__aceIms_orderLink'>
            <h2>Credit Note(s)</h2>
                <table class='thinlist'>
                    {$header}
                    {$rows}
    ...
    function getReceiptPortalDisplay($row){
        ...
        while ($rowReceipt = $db->sql_fetchrow($result)) {
            ...
            if ($row['contact_type'] == 'Indvidual') {
                $urlPrint = "index.php?_topRm=finance&module=aceIms_order&_spAction=printReceiptIndividual&receipt_code={$rowReceipt['receipt_code']}&showHTML=0";
            } else {
                $urlPrint = "index.php?_topRm=finance&module=aceIms_order&_spAction=printReceipt&receipt_code={$rowReceipt['receipt_code']}&showHTML=0";
            }
            ...
            $mediaLink = "index.php?plugin=common_media&_spAction=saveMedia&room=aceIms_receipt&recordType=attachment&media_id={$mediaRec['media_id']}&showHTML=0";

            $editRow = '';
            if ($cpCfg['m.aceIms.order.hasEditReceiptForPvt'] && $count == $numRows){
                $editURL = "index.php?_topRm=finance&module=aceIms_order&_spAction=editReceiptFormPvt&receipt_id={$rowReceipt['receipt_id']}&order_id={$row['order_id']}&showHTML=0";
        ...
        $editHeader = '';
        if ($cpCfg['m.aceIms.order.hasEditReceiptForPvt']){
            $editHeader = "<th>Edit</th>";
        }
        ...
        $formAction = "index.php?_topRm=finance&module=aceIms_order&_spAction=generateRefundForm&showHTML=0&order_id={$row['order_id']}&receipt_id={$receiptRec['receipt_id']}";
    ...
    function getRefundPortalDisplay($row){
        ...
        while ($rowRefund = $db->sql_fetchrow($result)) {
            ...
            if ($row['contact_type'] == 'Indvidual') {
                $urlPrint = "index.php?_topRm=finance&module=aceIms_order&_spAction=printRefundIndividual&refund_code={$rowRefund['refund_code']}&showHTML=0";
            } else {
                $urlPrint = "index.php?_topRm=finance&module=aceIms_order&_spAction=printRefund&refund_code={$rowRefund['refund_code']}&showHTML=0";
            }

            $expMedia = array('condn' => " AND media_type = 'attachment' AND room_name = 'aceIms_refund' AND actual_file_name LIKE '%{$rowRefund['refund_code']}%'");
            $mediaRec = $fn->getRecordRowByID('media', 'record_id', $rowRefund['refund_id'], $expMedia);
            $mediaLink = "index.php?plugin=common_media&_spAction=saveMedia&room=aceIms_refund&recordType=attachment&media_id={$mediaRec['media_id']}&showHTML=0";
        ...
        $total = "
            <tr style='background-color:#EAEAE8;text-align:center;font-weight:bold;'>
                <td colspan='2' class='txtRight'>Total : $total</td>
                <td colspan='2'></td>
            </tr>
        ";
        ...
        $formAction = "index.php?_topRm=finance&module=aceIms_order&_spAction=generateRefundForm&showHTML=0&order_id={$row['order_id']}";
    ...
    function getTotalForPvtInst($result, $getTotalOnly = ''){
        ...
        while ($row = $db->sql_fetchrow($result)) {
            ...
            if($course_type == 'Long Term' && $row['module'] == 'aceIms_subject'){
            ...
            else if($course_type == 'Short Term' && $row['module'] == 'aceIms_course'){
                $total = $row['unit_price'];
            }
     ...
     */
     function getGenerateReceiptForEntForm() {
        ...
        $formAction = "index.php?_topRm=finance&module=aceIms_order&_spAction=generateReceiptFormSubmit&showHTML=0";
     ...
     */
     function getGenerateMonthlyInvoiceForEntFormOld() {
        ...
        $formAction = "index.php?_topRm=finance&module=aceIms_order&_spAction=generateMonthlyInvoiceForEntFormSubmit&showHTML=0";
     ...
     */
     function getGenerateCreditNoteForm() {
        ...
        $formAction = "index.php?_topRm=finance&module=aceIms_order&_spAction=generateCreditNoteFormSubmit&showHTML=0";
     ...
     */
     function getGenerateRefundForm() {
        ...
        while ($row = $db->sql_fetchrow($result)) {
            ...
            <div class='floatbox'>
                <div class='float_left'>
                <input type='checkbox' name='receiptId[]' value='{$row['receipt_id']}' class='receiptId'>
                </div>
        ...
        $formAction = "index.php?_topRm=finance&module=aceIms_order&_spAction=generateRefundFormSubmit&showHTML=0";
     ...
     */
     function getGenerateReceiptFormPvtOld() {
        ...
        $formAction = "index.php?_topRm=finance&module=aceIms_order&_spAction=generateReceiptFormSubmitPvt&showHTML=0";
     ...
     */
     function getGenerateReceiptFormPvt() {
        ...
        if ($invoice_id == '') {
            return "Generate invoice first to make the payments";
        }
     ...
     */
     function getEditInvoiceFormPvt() {
        ...
        while ($rowInst = $db->sql_fetchrow($resultInst)) {
            ...
            $expEdit = array('isEditable' => 0);
            $amount = number_format($rowInst['amount'], 2);
            $rows .= "
            <div class='floatbox'>
        ...
        $expEdit = array('isEditable' => 0);
        $formAction = "index.php?_topRm=finance&module=aceIms_order&_spAction=editInvoiceFormSubmitPvt&showHTML=0";
        $text = "
     ...
     */
     function getEditReceiptFormPvt() {
        ...
        $formAction = "index.php?_topRm=finance&module=aceIms_order&_spAction=editReceiptFormSubmitPvt&showHTML=0";
        ...
        <form id='portalForm' class='yform columnar editReceiptFormForPvt receiptForm' method='post' action='{$formAction}'>
            {$rows}
            {$formObj->getTBRow('Amount', 'amount', $rowReceipt['amount'], $expEdit)}
            {$receipt_code}
            ...
            <input type='hidden' name='receipt_Amount' value='{$rowReceipt['amount']}' />
            <input type='hidden' name='receipt_id' value='{$receipt_id}' />
            <input type='hidden' name='order_id' value='{$order_id}' />
     ...
     */
     function getGenerateBookReceiptForm() {
        ...
        $formAction = "index.php?_topRm=finance&module=aceIms_order&_spAction=generateBookReceiptFormSubmit&showHTML=0";
    ...
    function getBookReceiptPortalDisplay($row){
        ...
        $refundButton = '';
        if ($cpCfg['m.aceIms.order.hasRefund']){
            $refundButton = "
            <button href='{$formAction}' id='generateRefund'
            class='button mt5 ml5 mb20'>Generate Refund</button>
            ";
    ...
    */
    function getViewSubjectsForOrderItem(){
        ...
        $total_amount = 0;
        while ($rowOi = $db->sql_fetchrow($resultOi)) {
            $item_price = number_format($rowOi['unit_price'], 2);
            $rows .= "
            <tr>
                <td>{$rowOi['item_title']}</td>
                <td class='txtRight'>{$item_price}</td>
            </tr>
            ";
            $total_amount += $rowOi['unit_price'];
        }
        ...
        <table class='thinlist'>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class='txtRight'><strong>{$total_amount_formatted}</strong></td>
                </tr>
            </tbody>
        </table>
        ";
    ...
    function getSummaryInOrder ($row) {
        ...
        SELECT o.*
              ...
              ,(SELECT SUM(i.invoice_amount)*SUM(gst_percentage)/100 FROM invoice i
                WHERE i.order_id = o.order_id
                AND i.status != 'Cancelled'
                ) AS gst_amount
              ,(SELECT SUM(r.amount)
        ...
        $invoiceAmt = number_format(($row['invoice_amount'] + $row['gst_amount']),2);
        $receiptAmt = number_format($row['receipt_amount'] ,2);

        $outstandingInvoiceAmt = number_format(($row['invoice_amount'] + $row['gst_amount']) - $row['receipt_amount'], 2);

        $rows = "
        <div class='linkPortalWrapper'>
            <div class='header' expanded='1'>
                <div class='floatbox'>
                    <div class='float_left headerForRightPanel'>FINANCE SUMMARY</div>
                </div>
            </div>
            <table class='oderSummaryList'>
                <tr>
                    <td class='titleSummary'>TOTAL ORDER AMOUNT</td>
                    <td class='summaryAmount'>{$invoiceAmt}</td>
                <tr>
                ...
                <tr>
                    ...
                    <td class='summaryAmount'>{$receiptAmt}</td>
                <tr>
                <tr>
                    <td class='titleSummary'>OUTSTANDING INVOICE</td>
                    <td class='summaryAmount'>{$outstandingInvoiceAmt}</td>
                <tr>
            </table>
        </div>
        ";
