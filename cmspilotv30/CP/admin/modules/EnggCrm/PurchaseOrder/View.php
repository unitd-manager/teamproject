<?
class CP_Admin_Modules_EnggCrm_PurchaseOrder_View extends CP_Common_Lib_ModuleViewAbstract
{
    /**
     *
     */
    function getList($dataArray){
       ...
       $this->getProductCode();
        ...
        foreach ($dataArray as $row){
            ...
            $total_cost = number_format($row['amount'] * $row['quantity'], 2);
    ...
    function getEdit($row){
        ...
        $statusArr = $cpCfg['m.enggCrm.purchaseOrder.statusArr'];
        ...
        $quote = "<a href='index.php?_topRm={$tv['topRm']}&module=enggCrm_quote&record_id={$row['quote_id']}&_action=edit'>{$row['quote_code']}</a>";
    ...
    function getRightPanel($row){
        ...
        $actionButtons = '';
        $links = '';
        $formActionInvoice = "index.php?module=enggCrm_purchaseOrder&_spAction=raiseInvoiceForm&purchase_order_id={$row['purchase_order_id']}&showHTML=0";
        ...
        $text = "
        {$actionButtons}
        {$media->getRightPanelMediaDisplay('Attachments', 'enggCrm_purchaseOrder', 'attachment', $row)}
        {$displayLinkData->getLinkPortalMain('enggCrm_purchaseOrder', 'tradingsg_productLink', 'Product Linked', $row)}
        {$links}
        ";
    ...
    function getInvoicePortalDisplay($row){
        ...
        while ($rowInvoice = $db->sql_fetchrow($result)) {
            ...
            $urlPrint = "index.php?_topRm=finance&module=enggCrm_order&_spAction=printInvoiceRecordForPurchaseOrder&invoice_code={$rowInvoice['invoice_code']}&showHTML=0";
            ...
            if($invoice_code == '' || $invoice_code != $rowInvoice['invoice_code']){
                ...
                if ($rowInvoice['status'] == 'Due'
                 ...
                 || $rowInvoice['status'] == 'Partial Payment'
                    ...
                    $editURL = "index.php?_topRm=order&module=enggCrm_purchaseOrder&_spAction=editInvoiceForm&showHTML=0&invoice_id={$rowInvoice['invoice_id']}&purchase_order_id={$row['purchase_order_id']}";
    ...
    function getQuickSearch() {
        ...
        <td>
            <select name='status'>
                ...
                {$cpUtil->getDropDown1($cpCfg['m.enggCrm.purchaseOrder.statusArr'], $status)}
            </select>
        </td>
        <td>
     ...
     */
     function getRaiseInvoiceForm() {
        ...
        $formAction = "index.php?_topRm=finance&module=enggCrm_purchaseOrder&_spAction=raiseInvoiceFormSubmit&showHTML=0";
     ...
     */
     function getEditInvoiceForm() {
        ...
        $formAction = "index.php?_topRm=finance&module=enggCrm_purchaseOrder&_spAction=editInvoiceFormSubmit&showHTML=0";
