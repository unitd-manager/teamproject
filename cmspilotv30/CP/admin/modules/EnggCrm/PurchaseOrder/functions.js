Util.createCPObject('cpm.enggCrm.purchaseOrder');

cpm.enggCrm.purchaseOrder = {
    init: function(){
        $('.m-enggCrm_purchaseOrder #actBtn_apply, .m-enggCrm_purchaseOrder #actBtn_save')
        .click(cpm.enggCrm.purchaseOrder.validateCreateInventory);

        //for edit portal product
        $('#fld_quantity, #buy_unit_price')
        .live('change', cpm.enggCrm.purchaseOrder.calculateValues);

    },

    calculateValues: function(){
        var url = "index.php?_topRm=main&module=enggCrm_product"
    ...
    editInventoryForm: function(purchase_order_items_id) {
        var url = 'index.php?module=enggCrm_purchaseOrder&_spAction=editInventoryForm' +
        ...
        var exp = {
           ...
           ,afterOpen: function() {
                ...
                $('#btnUpdateInventory').click(cpm.enggCrm.purchaseOrder.saveInventoryForm);
    ...
    saveInventoryForm: function() {
        var url = "index.php?module=enggCrm_purchaseOrder"
    ...
    printPO: function() {
        ...
        var url = 'index.php?_spAction=printReport&record_id=' +
                   purchase_order_id + '&showHTML=0&roomName=enggCrm_purchaseOrder&report=purchaseOrder';
...
$('.room-po-table input.poProductId, .room-po-table input.invoiceItemId, .room-po-table tbody tr input[id=fld_qty]').livequery('change', function (e){
    ...
    if(qty == 0 && checked){
        ...
        cpm.enggCrm.purchaseOrder.updateInvoiceAmount();
    }

    Util.hideProgressInd();
});

/*$('.m-enggCrm_purchaseOrder input.poProductId').livequery('click', function (e){
    Util.showProgressInd();
    po_product_id = $(this).val();
    var checked    = $(this).attr('checked') ? 'checked' : '';
    var checkedVal = checked == 'checked' ? 1 : 0;
    
    var url = 'index.php?_topRm=finance&module=enggCrm_purchaseOrder&_spAction=populateReceiptAmount&showHTML=0';
...
$('.m-enggCrm_purchaseOrder #raiseInvoice').livequery('click', function (e){
...
$('.m-enggCrm_purchaseOrder #editInvoice').livequery('click', function (e){
