<?
class CP_Admin_Modules_EnggCrm_CallRegistry_Model extends CP_Common_Lib_ModuleModelAbstract
{
    function getSQL() {
    ...
    function getEditValidate() {
        ...
        $validate->resetErrorArray();
        $validate->validateData('industry', 'Please select the industry');
    ...
    function getConvertToOpportunity() {
        ...
        if ($numRows == 0){
            $faLead = array();
            $faLead['status'] = "Converted to Opportunity";
            $faLead = $fn->addModificationDetailsToFieldsArray($faLead, 'call_registry');
            
            $whereCondition = "WHERE call_registry_id = {$call_registry_id}";
            $sqlUpdate = $dbUtil->getUpdateSQLStringFromArray($faLead, "call_registry", $whereCondition);
            ...
            $fa3 = array();
            $fa3['first_name']   = $callRegistryRec['contact_name'];                            
            ...
            $SQLUpdate = "UPDATE setting SET value = (value+1) WHERE key_text = 'nextOpportunityCode'";
            $resultUpdate = $db->sql_query($SQLUpdate);
            ...
            $cpUtil->redirect("index.php?_topRm=project&module=enggCrm_callRegistry&_action=edit&call_registry_id={$call_registry_id}");
        }
    }
