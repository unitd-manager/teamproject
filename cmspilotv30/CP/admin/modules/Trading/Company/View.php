<?
class CP_Admin_Modules_Trading_Company_View extends CP_Common_Lib_ModuleViewAbstract
{
    /**
     *
     */
    function getList($dataArray){
        ...
        foreach ($dataArray as $row){
            ...
            {$listObj->getListDataCell($row['phone_country_code'] . ' ' . $row['phone'])}
        ...
        {$listObj->getListHeaderCell('Sales Agent', 'c.sales_agent')}
    ...
    function getEdit($row){
        ...
        $exp = array('sortBy' => 'sort_order');
        $sqlRegion = $fn->getDDSql('trading_region', $exp);

        $cond = 'hide_in_company != 1 OR hide_in_company IS NULL';
        $expPT = array('sortBy' => 'sort_order', 'condn' => $cond);
        $sqlPricingType = $fn->getDDSql('trading_pricingType', $expPT);
        $sqlSalesAgent = $this->model->getSalesAgentSQL();
        ...
        if ($row['category'] == 'Customer') {
            ...
            {$formObj->getYesNoDropDownRow('Internal Client', 'internal_customer', $row['internal_customer'])}
        ...
        $expTypePricing = array('detailValue' => $row['pricing_type']);
        $expRegion = array('detailValue' => $row['region_name']);
        $expSalesAgent = array('detailValue' => $row['sales_agent']);
        ...
        {$formObj->getDDRowBySQL('Type of Pricing', 'pricing_type_id', $sqlPricingType, $row['pricing_type_id'], $expTypePricing)}
        ...
        {$formObj->getDropDownRowByArray('Country', 'address_country', $countryArr,
                   $row['address_country'], $expCountry)}
        {$formObj->getTBRow('Port', 'port', $row['port'])}
        {$formObj->getDDRowBySQL('Region', 'region_id', $sqlRegion, $row['region_id'], $expRegion)}
        {$formObj->getDDRowBySQL('Sales Agent Company', 'company_id_sales_agent',
                                  $sqlSalesAgent, $row['company_id_sales_agent'], $expSalesAgent)}
        {$supplierClientText}
        ";
    ...
    function getRightPanel($row){
        ...
        if ($row['category'] == 'Customer') {
            $deliveryText = "
            {$displayLinkData->getLinkPortalMain('trading_company', 'trading_deliveryAddressLink', 'Delivery Address', $row)}
            ";

        }

        $links = "
        {$displayLinkData->getLinkPortalMain('trading_company', 'trading_contactLink', 'Contacts Linked', $row)}
        {$displayLinkData->getLinkPortalMain('trading_company', 'trading_paymentTermsLink', 'Payment Terms', $row)}
        {$displayLinkData->getLinkPortalMain('trading_company', 'trading_deliveryTermsLink', 'Delivery Terms', $row)}
        {$deliveryText}
        ";
    ...
    function getExportData(){
        ...
        $actSheet->setCellValueByColumnAndRow($colc++, $rowc, 'Town/City');
