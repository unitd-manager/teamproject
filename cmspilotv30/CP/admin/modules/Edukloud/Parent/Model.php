<?
class CP_Admin_Modules_Edukloud_Parent_Model extends CP_Common_Modules_Edukloud_Parent_Model
{
    /**
     *
     */
    ...
    function getExportData($dataArray){
        ...
        $file_name = "Parent_" . date("d-m-Y") . ".xls";
     ...
     *
     */
	function getEdukloudParentEdukloudContactLinkSQL($id) {
    ...
    function getParentTransferFormSubmit() {
        ...
        /* Updating Site id in Parent table */
        $faPar = array();
        $faPar['site_id']           = $site_id;
        $faPar['modification_date'] = date("Y-m-d H:i:s");
        $faPar['modified_by']       = $fn->getSessionParam('userName');
        
        $wherePar   = "WHERE parent_id = {$parent_id}";
        $sqlPar     = $dbUtil->getUpdateSQLStringFromArray($faPar, 'parent', $wherePar);
        ...
        while ($rowContact = $db->sql_fetchrow($resultContact)) {
            ...
            $whereCont  = "WHERE contact_id = {$rowContact['contact_id']}";
            ...
            WHERE cc.contact_id = {$rowContact['contact_id']}
