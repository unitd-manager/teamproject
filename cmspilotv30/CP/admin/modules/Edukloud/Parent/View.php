<?
class CP_Admin_Modules_Edukloud_Parent_View extends CP_Common_Modules_Edukloud_Parent_View
{
    function getList($dataArray){
        ...
        foreach ($dataArray as $row){
            ...
            if($cpCfg['m.edukloud.parent.hasSalutation'] == 1){
                $rowsSalutation = "
    ...
    function getRightPanel($row){
        ...
        if ($cpCfg['m.edukloud.parent.hasStudentEnrollment']) {
            $rows .= $this->getCourseStudentDisplay($row);
        }

        $formAction = "index.php?_topRm=main&module=edukloud_parent&_spAction=parentTransferForm&showHTML=0&parent_id={$record_id}";
        $text = "
        <div>
            <a href='{$formAction}' id='parentTransfer' class='button mt5 ml5 mb10' parent_id={$record_id}>Transfer to other Branch</a>
        </div>
        {$media->getRightPanelMediaDisplay('Picture', 'edukloud_parent', 'picture', $row)}
        {$displayLinkData->getLinkPortalMain('edukloud_parent', 'edukloud_contactLink', 'Student Linked', $row)}
        {$rows}
        {$comment->getView(array(
             'roomName' => 'edukloud_parent'
            ,'recordId' => $record_id
        ))}
        ";
    ...
    function getImportInstructions() {
        ...
        $url = 'index.php?_spAction=streamFile&showHTML=0&modname=edukloud_parent&filename=parent-import-template.xls';
        $text = "
    ...
    function getCourseStudentDisplay($row){
        ...
        while ($rowCC = $db->sql_fetchrow($result)) {
            if ($order_id != $rowCC['order_id']){
                ...
                $editurl = "index.php?_topRm=main&_spAction=bulkParentStudentEnrollment&module=edukloud_orderLink&srcRoom=edukloud_company&lnkRoom=edukloud_orderLink&parent_id={$row['parent_id']}&order_id={$rowCC['order_id']}&showHTML=0
                ";
                ...
                <tr style='background-color: #F4F4F4; color:#000000;'>
                    ...
                    <td class='portalActBtns'>
                        <div style='float:right'>
                            ...
                            <a class='deletePortalRecord' srcroomid={$row['parent_id']} link='/admin/index.php?_spAction=deletePortalRecordByID&srcRoom=edukloud_parent&lnkRoom=edukloud_orderLink&id={$rowCC['order_id']}&showHTML=0' href='javascript:void(0);'>
            ...
            $contact_link = "/admin/index.php?_topRm=main&module=edukloud_contact&_action=detail&record_id={$rowCC['contact_id']}";
        ...
        $text = "
        <div class='linkPortalWrapper edukloud_parent__edukloud_orderLink' id='edukloud_parent#edukloud_orderLink'>
            <div expanded='1' class='header'>
                <div class='linkPortalDataWrapper'>
                    <div class='actBtns'>
                        <a id='bulkParentCourseLink' class='' dialogtitle='Bulk Parent Contact Link' href='index.php?_topRm=main&_spAction=bulkParentStudentEnrollment&module=edukloud_orderLink&srcRoom=edukloud_parent&lnkRoom=edukloud_orderLink&parent_id={$row['parent_id']}&showHTML=0';'> 
     ...
     */
     function getParentTransferForm() {
       ...
       $formAction = "index.php?_topRm=main&module=edukloud_parent&_spAction=parentTransferFormSubmit&showHTML=0";
