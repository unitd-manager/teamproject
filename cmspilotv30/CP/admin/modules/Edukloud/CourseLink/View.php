<?
class CP_Admin_Modules_Edukloud_CourseLink_View extends CP_Common_Lib_ModuleLinkViewAbstract
{
    ...
    */
    function getNew(){
        ...
        $course = 'Course';
        $batch = 'Batch';
        if($cpCfg['m.edukloud.courseLink.hasLabelChangeEnt']){
            $course = 'Class';
            $batch = 'Session';
        }
    ...
    */
    function getEditCoursePvtLink(){
        ...
        if($row['discount']){
            $trDiscount = getCPModelObj('edukloud_courseLink')->getDiscountValueForPvt('',$row['discount'], $row['course_id'], $medical_insurance, $full_time);
            $discount_amount = getCPModelObj('edukloud_courseLink')->getDiscountValueForPvt(1, $row['discount'], $row['course_id'], $medical_insurance, $full_time);
        }        
        else{
            $trDiscount = '';
        }
        ...
        if ($count != ''){
            ...
            if($full_time == 1){
                $subject_total     = getCPModelObj('edukloud_subjectLink')->getAddSubjectAmountToTotal('', 1);
                $trCourse = $subject_total ;
                
                //to get the subject total amount
                $subject_amount = getCPModelObj('edukloud_subjectLink')->getAddSubjectAmountToTotal(1,1);
            }
            else{
                $subject_total     = getCPModelObj('edukloud_subjectLink')->
                getAddSubjectAmountToTotal('','',$row['no_of_months']);
                $trCourse = $subject_total ;
                
                //to get the subject total amount
                $subject_amount = getCPModelObj('edukloud_subjectLink')->getAddSubjectAmountToTotal(1,'',$row['no_of_months']);
            }
