Util.createCPObject('cpm.edukloud.parent');
$('a#addCourseRows').livequery('click', function(){
    Util.showProgressInd();
    var url = $('#scopeRootAlias').val() + 'index.php?module=edukloud_orderLink&_spAction=courseRow&showHTML=0';
...
$('a.newContactDetails').livequery('click', function(e) {
    ...
    var exp = {
        callbackOnSuccess: function(){
            ...
            $('#dialog1').remove();
            var url = 'index.php?module=edukloud_orderLink&_spAction=selectedTraineeResultRow&showHTML=0';
            Util.showProgressInd();
            $.get(url, function(html){
                ...
                cpm.edukloud.company.populateCourseSummary.call(this);
...
$('table.traineeSearchRow .addTrainee').livequery('click', function(e){
    ...
    var url = 'index.php?module=edukloud_orderLink&_spAction=selectedStudentListRow&showHTML=0';
...
$('table.traineesSelectedLinked .removeTrainee').livequery('click', function(e){
    ...
    var contact_id = $(this).attr('contact_id');
    var order_id = $(this).attr('order_id');
    var url = 'index.php?module=edukloud_orderLink&_spAction=removeTrainee&showHTML=0';
    ...
    var expObj = {
       ...
       ,callbackOnSuccess: function(){
            ...
            Util.alert(msg, function(){
                ...
                Links.reloadPortalRecords('edukloud_company#edukloud_orderLink', 'edukloud_company');
...
cpm.edukloud.company.init = function(){
    $(window).load(function(){
        $('.edukloud_company__edukloud_orderLink select[name=course_id]').livequery(function(){
           cpm.edukloud.company.populateBatch.call(this);
        });
        
        $('.m-edukloud_company #traineeSelectedForm #fld_course_id').livequery('change', function(){
            ...
            cpm.edukloud.company.populateBatch.call(this);
            cpm.edukloud.company.populateSubsidy.call(this);
            cpm.edukloud.company.populateDiscount.call(this);
            Util.hideProgressInd();
        });
    });
}

cpm.edukloud.company.populateBatch = function(){
    Util.showProgressInd();
    var parent = $(this).closest('tr');
    $(this).each(function(){
        courseId = $(this).val();
        //batchObj = $('select[name=batch_id]', parent);
        batchObj = $('.m-edukloud_company #traineeSelectedForm #fld_batch_id');
...
cpm.edukloud.company.populateSubsidy = function(){
    courseId = $(this).val();
    $('.traineesSelectedLinked tr').each(function(){
        ...
        subsidyObj = $('.m-edukloud_company #traineeSelectedForm #fld_course_subsidy_history_id');
...
cpm.edukloud.company.populateDiscount = function(){
    var parent = $(this).closest('tr');
    $(this).each(function(){
    //alert (123);
        courseId = $(this).val();
        discountObj = $('select[name=discount_id]', parent);
        discountObj = $('.m-edukloud_company #traineeSelectedForm #fld_discount_id');
...
cpm.edukloud.company.calculateCourseSummaryTotal = function(){
...
cpm.edukloud.company.populateCourseSummary = function(){
    ...
    course_id = $('.m-edukloud_company #traineeSelectedForm #fld_course_id').val();
    var url = 'index.php?module=edukloud_courseLink&_spAction=courseCompanySummary&num_rows=' + num_rows + '&showHTML=0';
    
    $.get(url, {course_id: course_id}, function(html){
        $('tr#courseAmount').html(html);
        $('table#courseSummary').removeClass('hideme');
        cpm.edukloud.company.calculateCourseSummaryTotal();
    });
}
