<?
class CP_Admin_Modules_Edukloud_GiroPayment_View extends CP_Common_Lib_ModuleViewAbstract
{
    function getListOld($dataArray){
        ...
        if ($year == '' && $month == '') {
            return "{$this->getSearch()}<div align='center'><strong>Select values for Year and Month from Drop down</strong></div>";
        } else if ($year == '' && $month != '') {
            return "{$this->getSearch()}<div align='center'><strong>Select value for Year</strong></div>";
        } else if ($year != '' && $month == '') {
            return "{$this->getSearch()}<div align='center'><strong>Select value for Month</strong></div>";
        }
        ...
        foreach ($dataArray as $row){
            ...
            if($contact_name != $repeat_name){
                $namdeDiv ="<td class='mb5'><strong>{$contact_name}</strong></td>";
                $rows .= "
                <tr>
                {$namdeDiv}
                ";
            ...
            */
            //$invoiceRec = $fn->getRecordByCondition('invoice', "order_id = {$row['order_id']} AND contact_id = {$row['contact_id']}");
            ...
            <tr>
                ...
                <td>
                    <a class='button' href='index.php?_topRm=finance&module=edukloud_order&_action=edit&order_id={$row['order_id']}'  target=''><u>Goto Finance</u></a>
                </td>
            </tr>
            ";
            /*
            ...
            <table class='thinlist room-giroPayment-table'>
                <tr>
                    <thead>
                        ...
                        <th>
                            <input id='btnGiroSubmit' class='button' type='submit' value='Generate GIRO Payment failures'>
                        </th>
                        <th>
                            <a href='{$url}'  id='btnGenerateDBSTxtFile12' class='button' >Export File for DBS</a>
                        </th>
                        <th></th>
    ...
    function getList($dataArray){
        ...
        */
        ...
        if ($mode_of_payment == 'Giro') {
            $DBSTxtUrl = "index.php?_topRm=finance&module=edukloud_giroPayment&_spAction=generateDBSTxtFile&month={$month}&showHTML=0";
            $export_btn = "
            <th>
                <a href='{$DBSTxtUrl}'  id='btnGenerateDBSTxtFile1' class='button' >Export File for DBS</a>
            </th>
            ";
            $ddaHeader = "<td><strong>DDA</strong></td>";
            $topHeaderDda = "<th></th>";
        } else {
            $export_btn = "<th></th>";
            $ddaHeader  = "";
        ...
        foreach ($dataArray as $row){
            ...
            <tr>
                ...
                <td>
                    <a class='button' target='_blank' href='index.php?_topRm=finance&module=edukloud_order&_action=edit&order_id={$row['order_id']}'  target=''><u>Goto Finance</u></a>
        ...
        $formAction = "index.php?module=edukloud_giroPayment&_spAction=giroPaymentSubmit&month={$month}&mode_of_payment={$mode_of_payment}&showHTML=0";
        ...
        */
        $printAlLDueInvoiceLink = "index.php?module=edukloud_giroPayment&_spAction=printAllDueInvoiceForMonth&year={$year}&month=$month&showHTML=0";
    ...
    function getSearch() {
        ...
        $action = "index.php?_topRm=finance&module=edukloud_giroPayment&_action=list";
        $failedStudentsLink = "index.php?module=edukloud_giroPayment&_spAction=printGiroFailures&month={$month}";

        $reportLink = "index.php?module=edukloud_order&_spAction=printSelectedInvoices&month_val={$month}&showHTML=0";
    ...
    /**
     * Invoice PDF in Fpdf for more than one student for a parent of Iedukloud
     */
