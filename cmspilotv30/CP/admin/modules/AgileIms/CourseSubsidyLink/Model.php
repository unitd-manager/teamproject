<?
class CP_Admin_Modules_AgileIms_CourseSubsidyLink_Model extends CP_Common_Lib_ModuleLinkModelAbstract
{
    /**
     *
     */
    function getSubsidyValueForDropDown() {
        ...
        LEFT JOIN (course_subsidy_history s) ON (s.subsidy_discount_id = sd.subsidy_discount_id)
    ...
    function getDiscountValueForDropDown() {
        ...
        AND valid_from_date <= '{$today}'
        AND valid_to_date >= '{$today}'
        ";
        $result = $db->sql_query($SQL);
    ...
    function getSQL($linkRecType) {
        $modObj = getCPModuleObj('agileIms_subsidyDiscount');
