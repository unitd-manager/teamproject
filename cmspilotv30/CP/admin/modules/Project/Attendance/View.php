<?
class CP_Admin_Modules_Project_Attendance_View extends CP_Common_Lib_ModuleViewAbstract
{
    function getList($dataArray) {
        ...
        foreach ($dataArray as $row){
            ...
            if ($cpCfg['m.project.attendance.hasMultipleSessions'] == 1) {
            ...
            {$listObj->getListDataCell($row['update_mail'], 'center')}
            {$listObj->getListDataCell($row['productive_level'], 'center')}
            {$listObj->getListDataCell($row['check_task_level'], 'center')}
            {$listObj->getListDataCell($row['late_level'], 'center')}
            {$listObj->getListDataCell($row['attendance_id'], 'center')}
            {$listObj->getListDataCell($task,'taskinList')}
        ...
        {$listObj->getListHeaderCell('Update Comment', 'a.update_mail', 'headerCenter')}
        {$listObj->getListHeaderCell('Productive Level', 'a.productive_level', 'headerCenter')}
        {$listObj->getListHeaderCell('Check Task Level', 'a.check_task_level', 'headerCenter')}
        {$listObj->getListHeaderCell('Late Level', 'a.late_level', 'headerCenter')}
        {$listObj->getListHeaderCell('ID', 'a.attendance_id', 'headerCenter')}
        {$listObj->getListHeaderCell('Task', '','taskinList')}
    ...
    function getEdit($row) {
        ...
        $fielset1  = "
        {$formObj->getDDRowBySQL($cpCfg['m.project.staffFieldLabel'],  'staff_id', $sqlStaff, $row['staff_id'], $expStf)}
        ...
        {$formObj->getTARow('Task', 'task', $row['task'])}
   ...
   function getSendAttendanceReportToPM() {
        ...
        if($rowCounter > 0){
            ...
            $link .= "
            <p>
                <p><a href='http://studiouss.usoftsolutions.com/admin/index.php?_topRm=admin&module=project_attendance&_spAction=list&searchDone=1&staff_id={$staff_id}'>{$staff_name_disp}</a></p>
            </p>
            ";
