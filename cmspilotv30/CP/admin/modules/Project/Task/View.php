<?
class CP_Admin_Modules_Project_Task_View extends CP_Common_Lib_ModuleViewAbstract
{
    /**
     *
     */
    function getList($dataArray) {
        ...
        foreach ($dataArray as $row){
            ...
            if ($cpCfg['m.project.task.hightlightDue'] == 1) {
            ...
            <div align='left' style='float:left'>
                <a href='index.php?_topRm=project&module=project_timesheet&_action=list&task_id={$row['task_id']}'>
        ...
        if ($cpCfg['m.project.task.daysLbl'] == 1){
        ...
        {$taskListFieldsOrderGroupHeader}
        {$listObj->getListHeaderCell($cpCfg['m.project.staffFieldLabel'], 'staff_names')}
    ...
    function getEdit($row) {
        ...
        if($cpCfg['m.project.task.showReleaseStatus'] == 1){
            $releaseStatus = "
    ...
    function getRightPanel($row) {
        ...
        $links = '';
        if ($cpCfg['m.project.task.hasTaskHistory'] == 1){
            $links = $displayLinkData->getLinkPortalMain('project_task', 'project_taskHistoryLink', 'Task History', $row);
        }
        
        $text = "
        {$media->getRightPanelMediaDisplay('Attachment', 'project_task', 'attachment', $row)}
        {$displayLinkData->getLinkPortalMain('project_task', 'project_timesheetLink', 'Timesheet', $row)}
        {$displayLinkData->getLinkPortalMain('project_task', 'core_staffLink', $cpCfg['m.project.staffFieldLabel'], $row)}
        {$links}
        {$comment->getView(array(
             'roomName' => 'project_task'
            ,'recordId' => $record_id
        ))}
        ";
    ...
    function getSendTaskUpdatesToPM() {
        ...
        SELECT t.title
              ...
              ,(SELECT SEC_TO_TIME(AVG(TIME_TO_SEC(time_in)))
               FROM attendance 
                WHERE staff_id = s.staff_id
                 AND record_date BETWEEN '{$from_date}' AND '{$today}'
                 AND (on_leave IS NULL
                 OR on_leave = 0)
             ) AS avg_time_in
              ,(SELECT SEC_TO_TIME(AVG(TIME_TO_SEC(leave_time)))
               FROM attendance 
                WHERE staff_id = s.staff_id
                 AND record_date BETWEEN '{$from_date}' AND '{$today}'
                 AND (on_leave IS NULL
        ...
        while ($row = $db->sql_fetchrow($result, MYSQL_ASSOC)) {
            if($row['check_task_list'] == ''){
                $check_task_level = 5;
            } else{
                $check_task_level = 10;                
            }
            $SQLUpdateCheckTask = "
            UPDATE attendance SET check_task_level = '{$check_task_level}'
            WHERE attendance_id = {$row['attendance_id']} 
            ";
            $resultUpdateCheckTask  = $db->sql_query($SQLUpdateCheckTask);
        ...
        foreach ($data as $staff_name => $rows){
            ...
            foreach ($rows as $key => $row ){
                ...
                $break = strtotime('01:30:00');

                $rowText .= "
                <tr>
                    <td width='200'>{$row['project_title']} - {$row['task_history_title']}</td>
                    <td colspan='3' width='400'>{$row['description']}</td>
                ...
                $totalHours   = floor($total / SECONDS_PER_HOUR);
                $totalHours   = sprintf("%'.02d\n", $totalHours);
                $totalMinutes = ($total % SECONDS_PER_HOUR) / 60;
                $totalMinutes = sprintf("%'.02d\n", $totalMinutes+1);
                ...
                $holiday               = $row['holiday'];
                $check_task_list       = $row['check_task_list'];
            }
            $total_leave_days = $personal_leave_taken + $sick_leave_taken ;
            
            if($total_leave_days > 20){
                $pay_leave_taken =  $total_leave_days - 20;
            }
            $attRec = $fn->getRecordByCondition("attendance", "record_date = '{subdate(current_date, 1)}'");

            $todays_task_comments = $row['task'];
            $yest_task_comments   = $attRec['task'];
            //print nl2br($todays_task_comments) . "<br>/";
            //print nl2br($yest_task_comments) . "<br>/";
//exit();
            $task_listing = strcmp($todays_task_comments,$yest_task_comments) ;

            if($task_listing == 0){
                $SQLUpdate1 = "
                UPDATE attendance SET productive_level = 5 
                ...
                $SQLUpdate1 = "
                UPDATE attendance SET productive_level = 10 
                WHERE staff_id = {$row['staff_id']}
                AND record_date = '{$today}'
                AND leave_time != ''
                ";
            ...
            $resultUpdateTask = substr($string, $index);  

            $string1 = $attRec['task'];
            $prefix1 = "UPDATE";
            $index1  = strpos($string1, $prefix1) + strlen($prefix1);
            $resultUpdateTask1 = substr($string1, $index1);  
            
            $update_task = strcmp($resultUpdateTask,$resultUpdateTask1) ;
            if($update_task == 0){
                $SQLUpdate1 = "
                UPDATE attendance SET late_level = 5 
                WHERE staff_id = {$row['staff_id']}
                AND record_date = '{$today}'
                AND leave_time != ''
                ";
                ...
                $SQLUpdate1 = "
                UPDATE attendance SET late_level = 10 
                WHERE staff_id = {$row['staff_id']}
                AND record_date = '{$today}'
                AND leave_time != ''
                ";
            ...
            <tr style='background-color:#DDEBF9;'>
                ...
                <td colspan='2'><b>Avg Time in: </b>{$row['avg_time_in']}</td>
                <td colspan='2'> <b>Avg Time Out: </b>{$row['avg_time_out']}</td>
            </tr>
            <tr>
                <td><b>Task Listing: </b> {$productive_level}</td> 
                <td colspan='2'><b>Check Task Level: </b> {$check_task_level}</td>
                <td colspan='2'> <b>Update Task: </b> {$late_level}</td>
            </tr>
            <tr>
                <td><b>Personal Leave:</b> {$personal_leave_taken}</td>
                <td><b>Sick Leave:</b> {$sick_leave_taken}</td>
                <td><b>Holidays:</b> {$holiday}</td>
                <td colspan='2'><b>No Pay Leave:</b> {$pay_leave_taken}</td>
            </tr>
            <tr>
                <td><b>Time In:</b> {$row['time_in']}</td>
                <td colspan='2'><b>Time Out:</b> {$row['leave_time']}</td>
                <td colspan='2'><b>Total Leave Days:</b> {$total_leave_days} </td>
            </tr>
            {$rowText}
            ";
        }
