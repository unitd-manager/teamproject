...
class CP_Admin_Modules_Tradingsg_Quote_View extends CP_Common_Lib_ModuleViewAbstract
{
   //========================================================//
    ...
    function getNew(){
        ...
        {$formObj->getDDRowBySQL('Quote Type', 'quote_type', $sqlQuoteType, 'Requirement from Client', $expVl)}
        ";
    ...
    function getRightPanelSyedSept18th($row){
        ...
        {$displayLinkData->getLinkPortalMain('tradingsg_quote', 'tradingsg_productLink',
                                             ...
                                             ' . $invoiceLink .' ' . $bulkAdd .'
                                              ', $row)}
        {$displayLinkData->getLinkPortalMain('tradingsg_quote', 'tradingsg_purchaseOrderLink', 'Purchase Order Linked', $row)}
        ";
    ...
    function getQuoteProductLink($quote_id = '') {
        ...
        $bulkGenerateUrl  = "index.php?module=tradingsg_quote&_spAction=generateBulkProduct&id={$quote_id}&showHTML=0";
        ...
        while ($row = $db->sql_fetchrow($result)) {
            ...
            if ($row['product_id'] != ''){
                ...
                if ($numRowsQP > 0 || $numRowsPP > 0){
                    $viewHistory = "
                    <a href='index.php?_topRm=order&module=tradingsg_quote&_spAction=previousOrderForClient&showHTML=0&quote_product_id={$row['quote_product_id']}'  class='productViewHistory'> View</a>
                    ";
                }
            }

            $rows .= "
            <tr recid='{$row['quote_product_id']}' class='portal-row2 row-tradingsg_quote__tradingsg_productLink'>
                ...
                <td class='client-id w100 client_id'>
        ...
        <table keyfld='quote_product_id' id='quoteProductLink'>
            <thead>
                <tr>
                    ...
                    <th class='supplier w100 client_id'>Supplier</th>
                    <th class='cp w50'>CP</th>
                    <th class='uom'>UOM</th>
                    <th class='qty w50'>Qty</th>
                    <th class='view-history w50'>History</th>
                    <th class='tcp al-right w50 totalCp'>TCP</th>
                    <th class='mark-up-type w100'>Mark Up Type</th>
                    <th class='mark-up w25 serviceCostSum'>Mark Up</th>
    ...
    function getSummaryDisplayGeneralTrading($row){
        ...
        $total_cost_price_non_haram = $rowQuoteProd['total_selling_price'];
    ...
    function getUpdateDiscountByGroupForm() {
        ...
        $formAction = "index.php?_topRm=order&module=tradingsg_quote&_spAction=updateDiscountByGroupFormSubmit&showHTML=0";
    ...
    function getUpdateDiscountForm() {
        ...
        $formAction = "index.php?_topRm=order&module=tradingsg_quote&_spAction=updateDiscountFormSubmit&showHTML=0";
    ...
    function getUpdateMarkupByCategoryForm() {
        ...
        $formAction = "index.php?_topRm=order&module=tradingsg_quote&_spAction=updateMarkupByCategoryFormSubmit&showHTML=0";
    ...
    function getPreviousOrderForClient() {
        ...
        if ($numRows == 0) {
            ...
            while ($row = $db->sql_fetchrow($result)) {
                $quote = "<a target='_blank' href='index.php?_topRm=order&module=tradingsg_quote&record_id={$row['quote_id']}&_action=edit'><u>{$row['quote_code']}</u></a>";
                $rows .= "
                <tr>
                    <td>{$quote}</td>
    ...
    function getPrintQuoteThermalPrinter() {
        ...
        echo WebClientPrint::createScript("http://trademsart.localhost/index.php?module=tradingsg_quote&_spAction=printQuoteThermalPrinter");
     }
}