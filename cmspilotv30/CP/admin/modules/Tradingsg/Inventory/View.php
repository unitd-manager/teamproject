<?
class CP_Admin_Modules_Tradingsg_Inventory_View extends CP_Common_Lib_ModuleViewAbstract
{
    /**
     *
     */
    function getList($dataArray){
        ...
        foreach ($dataArray as $row){
            ...
            */
            ...
            $stock = $rowStockSql['product_qty_purchased']- $rowStockSql['product_qty_sold_pos'] - $rowStockSql['product_qty_sold_from_quote'];

            $rows .= "
            {$listObj->getListRowHeader($row, $count)}
            {$listObj->getGoToDetailText($count, $row['product_name'])}
            {$listObj->getListDataCell($row['item_code'])}
            {$listObj->getListDataCell($row['product_model'])}
            {$listObj->getListDataCell($row['product_carton_no'])}
            {$listObj->getListDataCell($row['product_batch_no'])}
        ...
        {$listObj->getListHeaderCell('Model', 'product_model')}
        {$listObj->getListHeaderCell('Carton NO', 'product_carton_no')}
        {$listObj->getListHeaderCell('Batch NO', 'product_batch_no')}
        {$listObj->getListHeaderCell('UOM', 'i.unit')}
        {$listObj->getListHeaderCell('Stock', '' )}
    ...
    function getEdit($row){
        ...
        {$formObj->getTBRow('Total Available Qty', 'total_available_qty', $rowStockSql['product_qty_purchased'] - $rowStockSql['product_qty_sold_pos']- $rowStockSql['product_qty_sold_from_quote'], $expNoEdit)}
        {$formObj->getTBRow('Total Sold Qty', 'total_sold_qty', $rowStockSql['product_qty_sold_pos'] + $rowStockSql['product_qty_sold_from_quote'], $expNoEdit)}
        ";
