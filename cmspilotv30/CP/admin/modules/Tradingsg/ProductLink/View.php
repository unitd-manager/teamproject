<?
class CP_Admin_Modules_Tradingsg_ProductLink_View extends CP_Common_Lib_ModuleLinkViewAbstract
{
    ...
    function getList($dataArray, $linkRecType) {
        ...
        $sales_order_id = $fn->getReqParam('linkMasterTableID');
        ...
        foreach ($dataArray as $row){
            ...
            $productTitle = $fn->getRecordDetailLink('trading_product',
                                                     ...
                                                     $expProdTitle);
            $priceText = '';
            if ($srcRoom == 'trading_salesOrder') {
                $priceText = "
                {$listObj->getListDataCell($row['buy_unit_price_base'])}
                {$listObj->getListDataCell($row['sell_unit_price_base'])}
                ";
            }
        ...
        if ($rowSO['order_type'] == 'Internal SO') {
            $message = 'No records found. Possibly because there are no valid RFQs '
                     . 'for the product records.';
        }
        ...
        if ($srcRoom == 'trading_salesOrder') {
            $priceText = "
            {$listLinkObj->getListHeaderCellLink($linkRecType,
                "Buy Price - {$cpCfg['m.trading.companyCurrency']}", 'buy_unit_price_base')}
            {$listLinkObj->getListHeaderCellLink($linkRecType,
                "Sell Price - {$cpCfg['m.trading.companyCurrency']}", 'sell_unit_price_base')}
            ";
        }
        $text = "
     ...
     *
     */
    function getListNotLinkedxx($result, $linkRecType) {
        $listObj = Zend_Registry::get('listObj');
        $listLinkObj = Zend_Registry::get('listLinkObj');
