Util.createCPObject('cpm.tradingin.purchaseOrder');

cpm.tradingin.purchaseOrder = {
    init: function(){
        $('.m-trading_purchaseOrder #actBtn_apply, .m-trading_purchaseOrder #actBtn_save')
        .click(cpm.tradingin.purchaseOrder.validateCreateInventory);

        //for edit portal product
        $('#fld_quantity, #buy_unit_price')
        .live('change', cpm.tradingin.purchaseOrder.calculateValues);
    ...
    editInventoryForm: function(purchase_order_items_id) {
        ...
        var exp = {
           ...
           ,afterOpen: function() {
                ...
                $('#btnUpdateInventory').click(cpm.tradingin.purchaseOrder.saveInventoryForm);
...
$('.room-po-table input.poProductId, .room-po-table input.invoiceItemId, .room-po-table tbody tr input[id=fld_qty]').livequery('change', function (e){
    ...
    if(qty == 0 && checked){
        ...
        cpm.tradingin.purchaseOrder.updateInvoiceAmount();
    }

    Util.hideProgressInd();
});

/*$('.m-tradingin_purchaseOrder input.poProductId').livequery('click', function (e){
    Util.showProgressInd();
    po_product_id = $(this).val();
    var checked    = $(this).attr('checked') ? 'checked' : '';
    var checkedVal = checked == 'checked' ? 1 : 0;
    
    var url = 'index.php?_topRm=finance&module=tradingin_purchaseOrder&_spAction=populateReceiptAmount&showHTML=0';
...
$('.m-tradingin_purchaseOrder #raiseInvoice').livequery('click', function (e){
...
$('.m-tradingin_purchaseOrder #editInvoice').livequery('click', function (e){
