<?
class CP_Admin_Modules_Tradingin_Invoice_Model extends CP_Common_Lib_ModuleModelAbstract
{
    /**
     *
     */
    ...
    function getGenerateInvoiceFormSubmit() {
        ...
        foreach ($orderItemIds as $key=>$value){
            ...
            if ($invoice_id > 0){
                ...
                $fa['item_code']  = $orderItemRec['item_code'];
        ...
        if($cst == 0 && $vat == 1){
			$gstvalue = $row['amount'] * $vat_value / 100;
			$amount = $gstvalue + $row['amount'];
		} else if($cst == 1 && $vat == 0){
			$gstvalue = $row['amount'] * $cst_value / 100;
			$amount = $gstvalue + $row['amount'];
        } else {
			$amount = $row['amount'];
        }
        ...
        $amount = $amount + round($frieghtVal) + round($pfVal);
        $amount = round($amount);
    ...
    function getGenerateInvoiceFormValidate() {
        ...
        if($count == 0){
            $validate->errorArray['orderItemId']['name'] = "orderItemId";
            $validate->errorArray['orderItemId']['msg']  = 'Please select product';
        }
