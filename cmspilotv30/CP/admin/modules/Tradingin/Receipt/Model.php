<?
class CP_Admin_Modules_Tradingin_Receipt_Model extends CP_Common_Lib_ModuleLinkModelAbstract
{
    /**
     *
     */
    function getGenerateReceiptFormSubmit() {
        ...
        FROM invoice inv
        WHERE inv.order_id = {$order_id} AND inv.status = 'Paid'
          ) as total_invoice_amount
        ";
        $resultSubSql = $db->sql_query($subSqlForPercentSum);
        $rowSql       = $db->sql_fetchrow($resultSubSql);

        $discount_percentage_amount_sum = $rowSql['discount_sum_value'] + $rowSql['discount_sum_percent'];
        $order_amount = $rowSql['order_amount'] - $discount_percentage_amount_sum;

        //FOR AUTO UPDATING OF ORDER STATUS WHEN A RECEIPT IS PAID
        if($order_amount == $rowSql['total_invoice_amount']){
