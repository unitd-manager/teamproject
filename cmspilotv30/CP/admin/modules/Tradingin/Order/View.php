<?
class CP_Admin_Modules_Tradingin_Order_View extends CP_Common_Lib_ModuleViewAbstract
{
    /**
     *
     */
    ...
    function getRightPanel($row){
        ...
        if ($cpCfg['m.ecommerce.order.showAttachment'] == 1){
            $links .= $media->getRightPanelMediaDisplay('Attachments', 'tradingin_order', 'attachment', $row);
        }
        ...
        if ($cpCfg['m.tradingsg.order.showReceiptButton']){
            if($row['record_type'] != 'POS') {
                $formActionReceipt = "index.php?module=tradingin_order&_spAction=generateReceiptForm&order_id={$row['order_id']}&showHTML=0";
        ...
        if ($cpCfg['m.tradingsg.order.showInvoiceButton']){
            if($row['record_type'] != 'POS') {
                $formActionInvoice = "index.php?module=tradingin_order&_spAction=generateInvoiceForm&order_id={$row['order_id']}&showHTML=0";
                $actionButtons .="
                <div class='float_right button mb5'>
        ...
        if ($cpCfg['m.tradingsg.order.showOrderItemDisplay']){
            $orderItem = $displayLinkData->getLinkPortalMain('tradingin_order', 'ecommerce_orderItemLink', 'Order Items', $row);
        }
    ...
    **/
    ...
    /**
     */
    function getInvoicePortalDisplayDetailOld($row){
        $cpCfg = Zend_Registry::get('cpCfg');
        $fn = Zend_Registry::get('fn');
        $db = Zend_Registry::get('db');
        $formObj = Zend_Registry::get('formObj');
        ...
        while ($rowInvoice = $db->sql_fetchrow($result)) {
            ...
            $urlPrint 		= "index.php?_topRm=finance&module=tradingin_order&_spAction=printInvoiceRecord&invoice_code={$rowInvoice['invoice_code']}&invoice_type=normal&showHTML=0";
            $urlProforma 		= "index.php?_topRm=finance&module=tradingin_order&_spAction=printInvoiceRecord&invoice_code={$rowInvoice['invoice_code']}&invoice_type=proforma&showHTML=0";
            $urlTransporter = "index.php?_topRm=finance&module=tradingin_order&_spAction=printInvoiceRecord&invoice_code={$rowInvoice['invoice_code']}&invoice_type=transporter&showHTML=0";
            $urlExtra  		= "index.php?_topRm=finance&module=tradingin_order&_spAction=printInvoiceRecord&invoice_code={$rowInvoice['invoice_code']}&invoice_type=extra&showHTML=0";
            ...
            //if($invoice_code == '' || $invoice_code != $rowInvoice['invoice_code']){
                ...
                if ($rowInvoice['status'] == 'Due'
                 ...
                 || $rowInvoice['status'] == 'Partial Payment'
                    ...
                    $editURL = "index.php?_topRm=finance&module=tradingin_order&_spAction=editInvoiceForm&showHTML=0&invoice_id={$rowInvoice['invoice_id']}&order_id={$row['order_id']}";
                ...
                <tr>
                    ...
                    <td><a href='{$urlProforma}' target='_blank'>Proforma Invoice</a></td>
                    <td><a href='{$urlTransporter}' target='_blank'>Transporter Invoice</a></td>
                    <td><a href='{$urlExtra}' target='_blank'>Extra Invoice</a></td>
    ...
    function getInvoicePortalDisplayDetail($row){
        ...
        while ($rowInvoice = $db->sql_fetchrow($result)) {
            ...
            if($rowORder['record_type'] != 'POS') {
                $urlPrint  = "index.php?_topRm=finance&module=tradingin_order&_spAction=printInvoiceRecord&invoice_code={$rowInvoice['invoice_code']}&invoice_type=normal&showHTML=0";
            } else {
                $urlPrint  = "index.php?_topRm=pos&module=tradingsg_pos&_spAction=printBill&invoice_code={$rowInvoice['invoice_code']}&printOnly=1&orderNo={$row['order_id']}&showHTML=0";
            }
            ...
            //if($invoice_code == '' || $invoice_code != $rowInvoice['invoice_code']){
                ...
                $formActionSalesReturn = "index.php?module=tradingin_order&_spAction=generateSalesReturnForm&invoice_id={$rowInvoice['invoice_id']}&order_id={$row['order_id']}&showHTML=0";
                $salesReturn ="
    ...
    function getReceiptPortalDisplay($row){
        ...
        while ($rowReceipt = $db->sql_fetchrow($result)) {
            ...
            $urlPrint = "index.php?_topRm=finance&module=tradingin_order&_spAction=printReceipt&receipt_code={$rowReceipt['receipt_code']}&order_id={$row['order_id']}&showHTML=0";
            ...
            if ($rowReceipt['receipt_status'] != 'Cancelled') {
                ...
                '{$row['order_id']}' receipt_code='{$rowReceipt['receipt_code']}'>Cancel Receipt</a>";
            }
            if ($rowReceipt['receipt_status'] == 'Cancelled') {
