<?
class CP_Admin_Modules_Tradingin_Quote_Model extends CP_Common_Lib_ModuleModelAbstract
{
    function getSQL() {
    ...
    */
    ...
    function getAdd(){
        ...
        $paymentTerms = $cpCfg['quotePaymentTerms'];
        $deliveryTerms = $cpCfg['quoteDeliveryTerms'];
        $notes = $cpCfg['quoteNotes'];
        ...
        $fa['note']             = $notes;
        $fa['delivery_terms']   = $deliveryTerms;
        $fa['payment_terms']    = $paymentTerms;
    ...
    function getFields(){
        ...
        $fa = $fn->addToFieldsArray($fa, 'quote_by_xrate');
        $fa = $fn->addToFieldsArray($fa, 'manual_invoice');
        $fa = $fn->addToFieldsArray($fa, 'link_stock');
        $fa = $fn->addToFieldsArray($fa, 'selling_company');
        $fa = $fn->addToFieldsArray($fa, 'link_account');
     ...
     *
     */
    function getTradinginQuoteTradingsgProductLinkSQL($id) {
        $cpCfg = Zend_Registry::get('cpCfg');
        $fn = Zend_Registry::get('fn');
        $db = Zend_Registry::get('db');
     ...
     *
     */
    function getTradinginQuoteTradingsgProductLinkSQLOld12092013($id) {
        $cpCfg = Zend_Registry::get('cpCfg');
        $fn = Zend_Registry::get('fn');
        $db = Zend_Registry::get('db');
     ...
     *
     */
    function getTradinginQuoteTradingsgExpenseLinkSQL($id) {
     ...
     *
     */
    function getTradinginQuoteTradingsgPurchaseOrderLinkSQL($id) {
        $cpCfg = Zend_Registry::get('cpCfg');
        ...
        SELECT po.purchase_order_id
              ...
              ,(CONCAT_WS('', '<a href=\'index.php?module=tradingin_quote&_spAction=printPurchaseOrder&id=', po.purchase_order_id, '\' target=\'_blank\'>PDF to Supplier</a>'))
              ,(CONCAT_WS('', '<a href=\'index.php?module=tradingin_quote&_spAction=printPurchaseOrderWithPrice&id=', po.purchase_order_id, '\' target=\'_blank\'>PDF  with Price</a>'))
    ...
    function getDuplicateQuote() {
        ...
        if($linkedProduct == 1){
            ...
            SELECT qp.*
            FROM quote_product qp
            WHERE qp.quote_id = '{$quote_id_source}'
            ";
            $resultQP = $db->sql_query($SQLQP);

            while ($row = $db->sql_fetchrow($resultQP)) {
                $fa1 = array();
                ...
                $fa1['remarks'] = $row['remarks'];
                $fa1['discount_percentage'] = $row['discount_percentage'];
                $fa1['discount_type'] = $row['discount_type'];
                $fa1['vat'] = $row['vat'];
                $fa1 = $fn->addCreationDetailsToFieldsArray($fa1, 'quote_product');

                $SQL = $dbUtil->getInsertSQLStringFromArray($fa1, 'quote_product');
                $db->sql_query($SQL);
            }
        }

        $topRm = $fn->getTopRoomName('quote');
        $url = "index.php?_topRm=order&module=tradingin_quote" .
    ...
    function getUpdateMarkupByGroupFormSubmit() {
        ...
        while ($row = $db->sql_fetchrow($result)) {
            ...
            $selling_price = $row['cost_price'] + number_format($mark_up,2) ;
            $sqlUpdate = "
    ...
    function getGenerateBulkProduct() {
        ...
        $formAction = "index.php?module=tradingin_quote&_spAction=generateProductFormSubmit&showHTML=0";
    ...
    function getPrintExportAsPdf() {
        ...
        $pdf->SetWidths(array(10, 62, 35, 10, 10, 31, 31));
        ...
        while ($row = $db->sql_fetchrow($result)) {
            if ($count == 0){
                ...
                $pdf->Ln();
                $pdf->Cell(95, 12, "Kind Attn: Mr {$row['contact_name']}", 'LR', 0, 'L', 1);
                ...
                $pdf->Cell(62,8,"NAME OF THE ITEM",1,0, 'C', 1);
