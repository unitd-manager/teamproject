<?
class CP_Admin_Modules_Ecommerce_Distributor_View extends CP_Common_Modules_Ecommerce_Distributor_View
{
    /**
     *
     */
    function getList($dataArray){
        ...
        foreach ($dataArray as $row){
            ...
            {$listObj->getListSortOrderField($row, 'distributor_id')}
            {$listObj->getListDataCell($row['email'])}
            {$listObj->getListDataCell($row['division_title'])}
            {$listObj->getListDataCell($row['distributor_id'], 'center')}
            {$listObj->getListPublishedImage($row['published'], $row['distributor_id'])}
            {$listObj->getListRowEnd($row['distributor_id'])}
            ";
            $rowCounter++ ;
        }
    ...
    function getRightPanel($row) {
        ...
        $text = "
        {$media->getRightPanelMediaDisplay('Picture', 'ecommerce_distributor', 'picture', $row)}
        ";
