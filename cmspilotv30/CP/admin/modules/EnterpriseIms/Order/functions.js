$("select[name='subsidy_paid']").change(function(){
    ...
    var url = 'index.php?_topRm=reports&module=enterpriseIms_order&_spAction=updateSubsidyStatus&showHTML=0&order_item_id=' + order_item_id + '&subsidy_paid_status' + subsidy_paid_status;
...
Util.createCPObject('cpm.enterpriseIms.order');

$('.check-all').livequery('click', function (e){
    cpm.enterpriseIms.order.checkAllRow.call(this)
});

$('.uncheck-all').livequery('click', function (e){
    cpm.enterpriseIms.order.uncheckAllRow.call(this)
});
...
$('.cancelInvoice').livequery('click', function (e){
    ...
    if (!confirm(msg)){
        ...
        var url = 'index.php?_topRm=finance&module=enterpriseIms_order&_spAction=cancelInvoice&showHTML=0';
        Util.showProgressInd();
...
cpm.enterpriseIms.order = {
    init: function(){
        $('.check-all').click(cpm.enterpriseIms.order.checkAllRow);
        $('.click-all-side .uncheck-all').click(cpm.enterpriseIms.order.uncheckAllRow);
    },
    ...
    printOrder:function(){
        ...
        var url = 'index.php?module=enterpriseIms_order&_spAction=printOrder&order_id=' + recordId;
        $.get(url, function(html){
        //alert ('Status Updated Succesfully');
        //Util.hideProgressInd();
        });
...
$('#generateReceipt').livequery('click', function (e){
    ...
    if (!confirm(msg)){
        ...
        $("select[name='mode_of_payment']").livequery('change', function (e){
            cpm.enterpriseIms.order.populatePaymentMode.call(this);
        });
...
$('input.invoiceCode').livequery('click', function (e){
    ...
    var url = 'index.php?_topRm=finance&module=enterpriseIms_order&_spAction=populateReceiptAmount&showHTML=0';
    $.get(url,{invoice_code: invoice_code ,checkedVal: checkedVal}, function(html){
        $('#totalAmountPayable').html('');
        $('input[id=fld_amount]').val(html);
    });
    
    var url = 'index.php?module=enterpriseIms_order&_spAction=populateDiscountAmount&showHTML=0';
    $.get(url,{invoice_code: invoice_code ,checkedVal: checkedVal}, function(html){
...
$('input.receiptCode').livequery('click', function (e){
    ...
    var url = 'index.php?_topRm=finance&module=enterpriseIms_order&_spAction=populateRefundAmount&showHTML=0';
...
cpm.enterpriseIms.order.populatePaymentMode = function(){
...
cpm.enterpriseIms.order.populateReceiptModeOfPayment = function(){
    $("select[name='mode_of_payment']").livequery('change', function (e){
        var paymentMode = $(this).val();
        if (paymentMode == 'Cheque') {
            Util.showProgressInd();
...
$(".invoiceDisplay select[name='status']").livequery('change', function (e){
    ...
    var url = 'index.php?module=enterpriseIms_order&_spAction=invoiceRecords&showHTML=0';
...
$('.receiptForm .populateAmountPayable').livequery('click', function (e){
    ...
    var url = 'index.php?module=enterpriseIms_order&_spAction=calculateAmountPayable&showHTML=0';
    Util.showProgressInd();
