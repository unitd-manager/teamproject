Util.createCPObject('cpm.labsg.reports');
...
cpm.labsg.reports = {
    init: function(){
        ...
        cpm.labsg.reports.setSearchForm();
        ...
        $('.search select[name=report]').change(function(){
            ...
            var url = 'index.php?_topRm=reports&module=labsg_reports&_spAction=search&showHTML=0&report=' + report;
        ...
        $('.m-labsg_reports .search select[name=bill_type]').livequery('change', function(e){
            cpm.labsg.reports.loadCompanyPatientDropdown.call(this);
        });
    },
    ...
    loadCompanyPatientDropdown: function(){
        $(this).each(function(){
            bill_type = $(this).val();
            var url = $('#scopeRootAlias').val() + 'index.php?module=labsg_reports&_spAction=companyPatientSqlByBillType&showHTML=0'

            $.getJSON(url, {bill_type: bill_type}, function(data) {
                $('.m-labsg_reports .search select[name=company_patient_id]').cp_loadSelect(data);
            });
        });
    },
    ...
    setSearchForm: function(){
        $('table.search input.button').livequery('click', function(e) {
            ...
            var url = 'index.php?_topRm=reports&module=labsg_reports&_spAction=displayReport&showHTML=0' + allVars;
            $.ajax({
                type: "GET",
                url: url,
                dataType: "html",
