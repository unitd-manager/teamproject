Util.createCPObject('cpm.labsg.purchaseOrder');

cpm.labsg.purchaseOrder = {
    init: function(){
        ...
        $('#portalForm select#fld_product_group_id').livequery('change', function(){
           cpm.labsg.purchaseOrder.loadCategoryDropdown.call(this);
        });

        //for edit portal product
        $('#portalForm .costs input, #portalForm .costs select')
        .live('change', cpm.labsg.purchaseOrder.calculateProductCosting);
        ...
        $('#portalForm a.next').live('click', cpm.labsg.purchaseOrder.nextLine);
        $('#portalForm a.previous').live('click', cpm.labsg.purchaseOrder.previousLine);
        ...
        /* Edit Product */
        $('.EditPoProduct').livequery('click', function (e){
            ...
            Util.openFormInDialog.call(this, 'EditPoProductForm', title, 450, 230, expObj);
        });
        ...
        $(".addSinglePoRow").livequery('click', function (e){
            var url = 'index.php?module=labsg_purchaseOrder&_spAction=addSingleLineItem'
                    + '&showHTML=0';
