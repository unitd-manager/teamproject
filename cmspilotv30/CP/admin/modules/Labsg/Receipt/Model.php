<?
class CP_Admin_Modules_Labsg_Receipt_Model extends CP_Common_Lib_ModuleLinkModelAbstract
{
    /**
     *
     */
    function getGenerateReceiptFormSubmit() {
        ...
        foreach($invoiceCodes AS $invoice_code){
            ...
            $faInv = array();
            $recpInvAmount = 0;
            if (round($invoice_amount) <= round($receipt_amount)){
                $recpInvAmount = $invoice_amount;
                $faInv['status'] = 'Paid';
            } else if (round($invoice_amount) > round($receipt_amount)){
        ...
        if(round($order_amount) == round($total_invoice_amount)){
    ...
    function getCancelReceiptFormSubmit() {
        ...
        if (!$this->getCancelReceiptFormSubmitValidate()){
            return $validate->getErrorMessageXML();
        }
        ...
        WHERE order_id = {$receiptRec['order_id']}
        ";
        $resultUpdate = $db->sql_query($SQLUpdate);
     ...
     *
     */
    function getCancelReceiptFormSubmitValidate() {
        $validate = Zend_Registry::get('validate');
