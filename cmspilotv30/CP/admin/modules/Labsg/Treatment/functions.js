Util.createCPObject('cpm.labsg.treatment');
...
cpm.labsg.treatment = {
    init: function(){
        ...
        $('#portalForm select#fld_product_group_id').livequery('change', function(){
           cpm.labsg.treatment.loadCategoryDropdown.call(this);
        });

        //for edit portal product
        $('#portalForm .costs input, #portalForm .costs select')
        .live('change', cpm.labsg.treatment.calculateProductCosting);
        ...
        $('#portalForm a.next').live('click', cpm.labsg.treatment.nextLine);
        $('#portalForm a.previous').live('click', cpm.labsg.treatment.previousLine);
    ...
    $('.EditMedicineTemplate').livequery('click', function (e){
        ...
        $('.deleteMedicineTemplate').livequery('click', function (e){
            ...
            if (!confirm(msg)){
                ...
                var treatment_medicine_template_id = $(this).attr('treatment_medicine_template_id');
                var url = 'index.php?module=labsg_treatment&_spAction=DeleteMedicineTemplate&showHTML=0&treatment_medicine_template_id=' + treatment_medicine_template_id;
