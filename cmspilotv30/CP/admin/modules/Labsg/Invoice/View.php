<?
class CP_Admin_Modules_Labsg_Invoice_View extends CP_Common_Lib_ModuleViewAbstract
{
    /**
     *
     */
    function getList($dataArray) {
        ...
        foreach ($dataArray as $row){
            ...
            $balance_Amount = $row['invoice_amount'] - $rowSql['amount_paid'] - $row['discount'];
            ...
            <td>
                <div id='' align='center'>
                    <u><a href='index.php?_topRm=finance&module=labsg_order&order_id={$row['order_id']}&_action=edit'>{$row['order_id']}</a></u>
                </div>
            </td>            
        ...
        {$listObj->getListHeaderCell('Invoice Code', 'i.invoice_code')}
     ...
     */
     function getGenerateInvoiceForm1() {
        ...
        $formAction = "index.php?_topRm=finance&module=labsg_order&_spAction=generateInvoiceFormSubmit&showHTML=0";
     ...
     */
     function getGenerateInvoiceForm() {
        ...
        if($numRowsOrderItem > 0){
            ...
            while($rowOrderItem  = $db->sql_fetchrow($resultOrderItem)){
                ...
                if($numRowsList > 0){
                    ...
                    $order_items_Details .="<div class='order_item_type_title'>Lab Tests</div>
        ...
        $Discount     = "<input name='discount' class='order_item_type_value invoice_discount_amount' value='0.00' />";
        $sub_Total    = "<input name='sub_Total' disabled class='order_item_type_value invoice_sub_total_amount' value='{$sub_Total}' />";
        $Total_Amount = "<input name='Total_Amount' disabled class='order_item_type_value invoice_total_amount' value='{$Total_Amount}' />";
        $order_items_Details .="<div class='order_item_type_title'></div>
                                <table class='thinlist order_item_type_table'>
                                    <tr>
                                        <th width='75%'>Sub Total1</th>
                                     ...
                                     <tr>
                                        ...
                                        <th width='35%' class='txtRight invoice_total_amount'>{$Total_Amount}</th>
                                     <tr>
                                </table>
                                ";

        $formAction = "index.php?_topRm=finance&module=labsg_order&_spAction=generateIndividualInvoiceFormSubmit&showHTML=0";
        $count = $count-1;

        /* Display receipt related Data - START */
        $receiptRows = "";
        if ($receipt) {
            $receiptRows = "
            <div class='order_item_type_title'>Receipt Details</div>
            {$formObj->getDDRowByVL('Mode of Payment*', 'mode_of_payment',  'paymentType')}
            {$formObj->getTextAreaRow('Note', 'remarks')}
            ";
        }
        /* Display receipt related Data - END */
     ...
     */
     function getEditInvoiceForm() {
        ...
        $formAction = "index.php?_topRm=finance&module=labsg_order&_spAction=editInvoiceFormSubmit&showHTML=0";
    ...
    function getEdit($row) {
        ...
        $order = "<a href='index.php?_topRm=finance&module=labsg_order&order_id={$row['order_id']}&_action=edit'>{$row['order_id']}</a>";
    ...
    function getPatientVisitPortal($row){
        ...
        LEFT JOIN (order_item oi) ON (pv.patient_visit_id = oi.patient_visit_id)
        LEFT JOIN (invoice i) ON (oi.invoice_id = i.invoice_id)
        WHERE oi.invoice_id = {$row['invoice_id']}
        ";
        $resultOi = $db->sql_query($sqlOi);
        while ($rowOi = $db->sql_fetchrow($resultOi)) {
            $check_up_date = $fn->getCPDate($rowOi['check_up_date'],"d-m-Y");
            $visitUrl = "/admin/index.php?_topRm=main&module=labsg_patientVisit&_action=edit&patient_visit_id={$rowOi['patient_visit_id']}";
    ...
    function getCancelInvoiceForm() {
        ...
        if ($numRowsIrh > 0) {
            return "Cancel the related receipts and then proceed canceling the invoice";
        }

        $formAction = "index.php?_topRm=finance&module=labsg_invoice&_spAction=cancelInvoiceFormSubmit&showHTML=0";
        $text = "
        <form id='portalForm' class='yform columnar cancelInvoiceForm' method='post' action='{$formAction}'>
