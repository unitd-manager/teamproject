<?
class CP_Admin_Modules_Core_AdminTranslation_Model extends CP_Common_Lib_ModuleModelAbstract
{
    function getSQL() {
        $SQL = "
        SELECT *
        FROM admin_translation at
        ";
    ...
    function setSearchVar() {
        ...
        if ($tv['record_id'] != ''){
            $searchVar->sqlSearchVar[] = "at.admin_translation_id  = '{$tv['record_id']}'";
        } else {
            if ($group_name != '' ) {
                $searchVar->sqlSearchVar[] = "at.group_name  = '{$group_name}'";
            }


            if ($tv['keyword'] != "") {
                $searchVar->sqlSearchVar[] = "(
                    at.key_text    LIKE '%{$tv['keyword']}%'  OR
                    at.value       LIKE '%{$tv['keyword']}%'
                )";
            }
        }
    ...
    function getSave(){
        ...
        $spAction->getUpdateAdminLangFiles();
    ...
    function getUpdateValue(){
        ...
        $whereCondition = "WHERE admin_translation_id = {$rec_id}";
        $SQL            = $dbUtil->getUpdateSQLStringFromArray($fa, "admin_translation", $whereCondition);
