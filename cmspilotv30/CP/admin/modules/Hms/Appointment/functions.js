Util.createCPObject('cpm.hms.appointment');

cpm.hms.appointment = {
    init: function(){
        $("input[name='patient_name']")
        .livequery(cpm.hms.appointment.patientName);
        ...
        $('#fld_employee_id').change(function(){
            ...
            var events = {
                  url: 'index.php?module=hms_appointment&_spAction=eventDetails&showHTML=0',
                  type: 'POST',
                  data: {
                    doctor_id: doctor_id
                  }
        ...
        $('#fld_employee_id_visit').change(function(){
            ...
            cpm.hms.appointment.patientNameFilter(employee_id);
        });
        ...
        $("select[name='appointment_employee_id']").livequery('change', function (e){
            ...
            var url = "index.php?module=hms_appointment&_spAction=updateAppointmentEventDetails&showHTML=0";
            Util.showProgressInd();
        ...
        $('.appointmentNotesUpdate').livequery('click', function (e){
            ...
            var url = 'index.php?_topRm=main&module=hms_appointment&_spAction=updateAppointmentNotes&showHTML=0';
            Util.showProgressInd();

            $.get(url,{appointment_id: appointment_id, notes:notes}, function(html){
                cpm.hms.appointment.reloadNotesDetails(appointment_id, notes);
        ...
        /*$('.createVisit').livequery('click', function (e){
            ...
            if(dr_required == ''){
                cpm.hms.appointment.updateAppointmentDetails(appointment_id);
                ...
                $.get(url,{appointment_id:appointment_id}, function(html){
                    ...
                    $('#calendarAppointment').fullCalendar( 'refetchEvents' );
                    cpm.hms.appointment.reloadDoctorDetails();
                    cpm.hms.appointment.reloadAppointmentListDetails();
                    cpm.hms.appointment.reloadQueueno();
    ...
    patientName: function() {
        ...
        $(titleObj).autocomplete({
             source : 'index.php?module=hms_appointment&_spAction=searchPatientDetails&showHTML=0'
            ,minLength : 3
            ,selectFirst: true
            ,autoFocus: true
    ...
    updateAppointmentDetails: function(appointment_id){
        var title = "";
        var url   = "index.php?module=hms_appointment&_spAction=updateAppointmentDetails&appointment_id="+appointment_id+"&showHTML=0";
...
cpm.hms.appointment.run = function(exp){
    ...
    $('#' + handle).fullCalendar({
        ...
        ,dayClick: function(date, allDay, jsEvent, view) {
            ...
            if (allDay) {
                ...
                cpm.hms.appointment.addAppointmentDetails(SelectedTime, selectedDate);
            }else{
       ...
       ,eventClick: function(event, jsEvent, view) {
            ...
            if (event.url) {
                cpm.hms.appointment.showAppointmentDetails.call(this);
                return false;
            }
        }
       ...
       ,eventDrop: function(event, delta, revertFunc, jsEvent, ui, view) {
            ...
            if (!confirm("Please note for this action the appointment time will be changed")) {
                ...
                if (view.name == 'agendaDay' || view.name == 'agendaWeek') {
                    ...
                    var selectedDate = local.toJSON().slice(0, 10);
                    var url = 'index.php?_topRm=main&module=hms_appointment&_spAction=changeAppointmentByDrag&showHTML=0';
        ...
        ,eventRender: function(event, element, view){
            ...
            //if (view.name == 'agendaDay'){
                //$(element).find('.fc-event-time').append("<br/>" + event.doctor_name + "<br/>" + event.patient_name + "<br/>" + event.createVisit + event.cancelAppointment);
    ...
    cpm.hms.appointment.reloadDoctorDetails = function(){
        var url = 'index.php?module=hms_appointment&_spAction=doctorDetails&showHTML=0';
    ...
    cpm.hms.appointment.reloadNotesDetails = function(appointment_id, notes){
        var url = 'index.php?module=hms_appointment&_spAction=appointmentNotes&appointment_id='+appointment_id+'&notes='+notes+'&showHTML=0';
        $.get(url,  function(html){
    ...
    cpm.hms.appointment.reloadAppointmentListDetails = function(){
        var url = "index.php?module=hms_appointment&_spAction=appointmentListDetails&showHTML=0";
        $.get(url,  function(html){
            $('#appointmentListDetails').html(html);
            Util.hideProgressInd();
    ...
    cpm.hms.appointment.reloadAppointmentDetails = function(){
        var url = 'index.php?module=hms_appointment&_spAction=appointmentDetails&showHTML=0';
        $.get(url,  function(html){
            $('.appointmentvisitScroll').html(html);
        });
    }

    cpm.hms.appointment.addAppointmentDetails = function(SelectedTime, selectedDate){
        var title = "Add Appointment";
        var url   = "index.php?module=hms_appointment&_spAction=addAppointmentDetails&check_up_date="+selectedDate+"&check_up_time="+SelectedTime+"&showHTML=0";
        ...
        Util.openFormInDialog.call('','portalFormAppointment', title, 560, 380, exp);
    }

    cpm.hms.appointment.reloadQueueno = function(){
    ...
    cpm.hms.appointment.showAppointmentDetails = function(){
        var title = "Appointment Details";
