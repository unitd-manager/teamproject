<?
class CP_Admin_Modules_Hms_MedicalSupplier_View extends CP_Common_Lib_ModuleViewAbstract
{
    /**
     *
     */
    function getList($dataArray){
        ...
        foreach ($dataArray as $row){
            ...
            {$listObj->getListRowEnd($row['medical_supplier_id'])}
            ";
        ...
        {$listObj->getListHeaderCell('Name', 'ms.title')}
        {$listObj->getListHeaderCell('Website', 'ms.website')}
        {$listObj->getListHeaderCell('Telephone', 'ms.phone' )}
    ...
    function getEdit($row){
        ...
        <div class='linkPortalWrapper'>
            <div expanded='1' class='header'>
                <div class='floatbox'>
                    <div class='float_left'>Medical Supplier Details</div>
                ...
                <div class='linkPortalDataWrapper'>
                    <table class='thinlist'>
                        <tbody>
                            ...
                            <tr>
                                <td>{$formObj->getTBRow('Address1', 'address1', $row['address1'])}</td>
                                <td>{$formObj->getTBRow('Address2', 'address2', $row['address2'])}</td>
                            ...
                            <tr>
                                <td class= 'creModdate' colspan = '5'>{$formObj->getCreationModificationText($row)}</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
    ...
    function getRightPanel($row){
        ...
        $record_id = $fn->getIssetParam($row, 'medical_supplier_id');

        $text = "
        {$displayLinkData->getLinkPortalMain('hms_medicalSupplier', 'hms_contactLink', 'Contacts Linked', $row)}
        {$media->getRightPanelMediaDisplay('Attachments', 'hms_medicalSupplier', 'attachment', $row)}
        ";
