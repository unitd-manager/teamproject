Util.createCPObject('cpm.hms.dutyRoster');
...
cpm.hms.dutyRoster = {
    init: function(){
        ...
        $(".previous_dutyRoster_icon").livequery('click', function (e){
            ...
            cpm.hms.dutyRoster.reloadDoctorDetails(work_date);
        });
        ...
        $('#fld_employee_id').change(function(){
            ...
            var events = {
                  url: 'index.php?module=hms_dutyRoster&_spAction=eventDetails&showHTML=0',
                  type: 'POST',
                  data: {
                    doctor_id: doctor_id
                  }
        ...
        $(".addTimeinOutLine").livequery('click', function (e){
            var url = 'index.php?module=hms_dutyRoster&_spAction=addMoreWorkingTime1'
                    + '&showHTML=0';
        ...
        $(".addTimeinOutLine3").livequery('click', function (e){
            var url = 'index.php?module=hms_dutyRoster&_spAction=addMoreWorkingTime2'
                    + '&showHTML=0';
        ...
        $("a.printDutyRosterLink").livequery('click', function (e){
            ...
            var url   = "index.php?module=hms_dutyRoster&_spAction=printDutyRosterForm&showHTML=0";
            var expObj = {
                url: url
               ,validate: true
               ...
               ,callbackOnSuccess: function(){
                    ...
                    var favorite = [];
                    $.each($("input[name='site_selected[]']:checked"), function(){
                        favorite.push($(this).val());
                    });
                    var site_selected = favorite.join(", ");
                    month           = cpm.hms.dutyRoster.pad2(month);
                    var employee_id = $('select[name="employee_id_pdf"]').val();
                    var convertUrl = "index.php?_topRm=utils&module=hms_dutyRoster&_spAction=printDutyRosterPdf&year=" + year + "&month=" + month + "&employee_id=" + employee_id + "&site_selected=" +site_selected;
                    window.open(convertUrl, 'blank');
                    Util.closeAllDialogs();
                    Util.hideProgressInd();
                }
            };
            Util.openFormInDialog.call('', 'portalFormPrintDutyRoster', title, 625, 'auto', expObj);
        });
        ...
        $(".addAppointmentLinkBtn").livequery('click', function (e){
            ...
            cpm.hms.dutyRoster.addDutyRosterDetails(SelectedTime, selectedDate);
        });

        $(".editDutyRoasterDetails").livequery('click', function (e){
            var title = "Duty Roster Edit";
            var duty_roster_id = $(this).attr('duty_roster_id');
            var url   = "index.php?module=hms_dutyRoster&_spAction=dutyRosterEdit&duty_roster_id="+duty_roster_id+"&showHTML=0"; 
            var expObj = {
                url: url
               ,validate: true
               ,submitBtnText: 'Submit'
...
cpm.hms.dutyRoster.run = function(exp){
    ...
    $('#' + handle).fullCalendar({
       ...
       ,eventClick: function(event, jsEvent, view) {
            ...
            if (event.url) {
                cpm.hms.dutyRoster.dutyRosterEditForm.call(this);
                return false;
            }
        }
    ...
    cpm.hms.dutyRoster.dutyRosterEditForm = function(){
        var title = "Duty Roster Edit";
        var work_date = '';
    ...
    cpm.hms.dutyRoster.pad2 = function(number) {
        return (number < 10 ? '0' : '') + number
    },

    cpm.hms.dutyRoster.addDutyRosterDetails = function(SelectedTime, selectedDate){
        var title = "Add Duty Roster";
        var url   = "index.php?module=hms_dutyRoster&_spAction=addDutyRosterDetails&work_date="+selectedDate+"&work_from_time="+SelectedTime+"&showHTML=0"; 
        var work_date = '';
        var exp = {
            url: url
           ,validate: true
    ...
    cpm.hms.dutyRoster.reloadDoctorDetails = function(work_date){
        var url = 'index.php?module=hms_dutyRoster&_spAction=doctorDetails&showHTML=0';
