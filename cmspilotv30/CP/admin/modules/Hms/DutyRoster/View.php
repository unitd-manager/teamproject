<?
class CP_Admin_Modules_Hms_DutyRoster_View extends CP_Common_Lib_ModuleViewAbstract
{
    ...
    function getPrintDutyRosterForm() {
        ...
        $formAction = "index.php?_topRm=main&module=hms_dutyRoster&_spAction=printDutyRosterFormSubmit&showHTML=0";
        ...
        while($rowSite = $db->sql_fetchrow($resultSite)){
            array_push($siteArray, $rowSite['title']);
        }

        $text = "
        <form id='portalFormPrintDutyRoster' name='portalFormPrintDutyRoster' class='yform columnar' method='post' action='{$formAction}'>
            <table id='' class='thinlist'>
                ...
                <div class='WeekdaysSelect'>
                    {$formObj->getCheckBoxArrRowByArr('Choose Sites:', 'site_selected', $siteArray, '')}
                </div>
    ...
    function getPrintDutyRosterPdf(){
        ...
        $site_selected   = $fn->getReqParam('site_selected');
        $cpSiteIdSession = $fn->getSessionParam('cp_site_id');

        $site_selected = str_replace(", ","','", $site_selected);
        ...
        while ($rowSite = $db->sql_fetchrow($resultSite)) {
            ...
            $tbl1 = $tbl1.'<th colspan="'.$numRowsTimings.'">'.$rowSite['title'].'</th>
                    ';
        }
        ...
        while ($rowDetails = $db->sql_fetchrow($resultDetails)) {
            ...
            while ($rowSite = $db->sql_fetchrow($resultSite)) {
                ...
                while ($rowDname = $db->sql_fetchrow($resultDname)) {
                    ...
                    $SameRecCount = $fn->getRecordCount('duty_roster', "work_from_time = '{$rowDname['work_from_time']}' AND work_to_time = '{$rowDname['work_to_time']}' AND work_date = '{$rowDname['work_date']}' AND site_id = '{$rowSite['site_id']}'");
                    $endTda = '';
    ...
    function getAddDutyRosterDetails(){
        ...
        $formAction = "index.php?_topRm=main&module=hms_dutyRoster&_spAction=addDutyRosterDetailsSubmit&showHTML=0";
        ...
        $weekTypeArr     = array('Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun');
     ...
     */
     function getDutyRosterEdit(){
        ...
        $formAction = "index.php?_topRm=main&module=hms_dutyRoster&_spAction=dutyRosterEditFormSubmit&showHTML=0";
