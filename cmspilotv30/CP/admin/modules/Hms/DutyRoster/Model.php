<?
class CP_Admin_Modules_Hms_DutyRoster_Model extends CP_Common_Lib_ModuleModelAbstract
{
    /**
     *
     */
    ...
    function getEventDetails(){
       ...
       ";
        ...
        while ($row = $db->sql_fetchrow($result)) {
             ...
             $dutyRosterLink    = "index.php?module=hms_dutyRoster&_spAction=dutyRosterEdit&duty_roster_id={$row['duty_roster_id']}&showHTML=0";
    ...
    function getPrintDutyRosterFormValidate(){
        ...
        $fn = Zend_Registry::get('fn');
        $db = Zend_Registry::get('db');
        $site_selected = $fn->getPostParam('site_selected', array());

        $validate->resetErrorArray();
        $validate->validateData('duty_year', 'Please Select Year');
        $validate->validateData('duty_Month', 'Please Select Month');

        if(sizeof($site_selected) == 0){
            $validate->validateData('site_selected', 'Please Choose Site');
        }

        if(sizeof($site_selected) > 3){
            $validate->errorArray['site_selected']['name'] = "site_selected";
            $validate->errorArray['site_selected']['msg']  = "Please Choose only three sites";
        }
