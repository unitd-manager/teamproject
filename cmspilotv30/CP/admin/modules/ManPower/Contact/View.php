<?
class CP_Admin_Modules_ManPower_Contact_View extends CP_Common_Lib_ModuleViewAbstract
{
    function getList($dataArray){
        ...
        foreach ($dataArray as $row){
            ...
            if($cpCfg['m.manPower.contact.hasCompanyTable'] == 1){
                $rowsCompanyTable = "
                <td>
                    <div align='left'>
            ...
            if($cpCfg['m.manPower.contact.hasWebLogin'] == 1){
                $rowsWebLogin = "
                {$listObj->getListPublishedImage($row['published'], $row['contact_id'])}
                ";
            }

            if($cpCfg['m.manPower.contact.hasSalutation'] == 1){
                $rowsSalutation = "
            ...
            {$listObj->getListDataCell($row['member_type'])}
            {$rowsWebLogin}
        ...
        {$textSalutation}
        {$listObj->getListHeaderCell($ln->gd('m.manPower.header.contact.lbl.firstName', 'Name'), 'c.name')}
        ...
        {$listObj->getListHeaderCell('Contact Type', 'c.member_type')}
        {$textWebLogin}
    ...
    function getNew(){
        ...
        $fielset = "
        {$formObj->getTBRow($ln->gd('m.manPower.contact.lbl.firstName', 'First Name'), 'first_name')}
        {$formObj->getTBRow($ln->gd('m.manPower.contact.lbl.lastName', 'Last Name'), 'last_name')}
    ...
    function getEdit($row){
        ...
        if($cpCfg['m.manPower.contact.showUsername'] == 1){
            $username = "
        ...
        $langPref = '';
        if ($cpCfg['m.manPower.contact.showLangPrefernce']){
    ...
    function getRightPanel($row){
        ...
        if($cpCfg['m.manPower.contact.showAttachment'] == 1){
            $rows .= $media->getRightPanelMediaDisplay($ln->gd('cp.lbl.attachment', 'Attachments'), 'manPower_contact', 'attachment', $row);
        }
        
        if($cpCfg['m.manPower.contact.showInterest'] == 1){
            $rows .= $displayLinkData->getLinkPortalMain("manPower_contact", "common_interestLink", "Interests Linked", $row);
        }

        if($cpCfg['m.manPower.contact.showEvent'] == 1){
            $rows .= $displayLinkData->getLinkPortalMain("manPower_contact", "event_eventLink", $ln->gd('m.manPower.contact.lbl.EventsLinked', 'Events Linked'), $row);
        }
        
        if($cpCfg['m.manPower.contact.showOrders']){
            $rows .= $displayLinkData->getLinkPortalMain("manPower_contact", "ecommerce_orderLink", "Orders Linked", $row);
        }

        if($cpCfg['m.manPower.contact.showContentLink'] == 1){
            $links .= $displayLinkData->getLinkPortalMain("manPower_contact", "webBasic_contentLink", "Content Linked", $row);
        }

        $record_id = $fn->getIssetParam($row, 'contact_id');

        $text = "
        {$media->getRightPanelMediaDisplay($ln->gd('cp.lbl.picture', 'Picture'), 'manPower_contact', 'picture', $row)}
        {$rows}
        {$links}
        ";
    ...
    function getQuickSearch() {
        ...
        $member_type    = $fn->getReqParam('member_type');
        $interestText   = "";
        $languageText   = '';
        ...
        if ($cpCfg['m.manPower.contact.showLangPrefernce'] == 1) {
            $languageText = "
            <td>
        ...
        //==================================================================//
        $spArray = $cpCfg['m.manPower.contact.specialSearchArr'];
        ...
        <td>
            <select name='member_type'>
                ...
                {$cpUtil->getDropDown1($spArrayCon, $member_type)}
            </select>
        </td>
        <td>
    ...
    function getImportInstructions() {
        ...
        $url = 'index.php?_spAction=streamFile&showHTML=0&modname=manPower_contact&filename=contact-import-template.xls';
        $text = "
    ...
    function getAdditionalImportFields(){
        ...
        $text = '';
        if($cpCfg['m.manPower.contact.showInterestInImport']){
            $sqlInterest = $fn->getDDSql('common_interest');
