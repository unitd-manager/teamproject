<?
class CP_Admin_Modules_ManPower_Contact_Model extends CP_Common_Lib_ModuleModelAbstract
{
    function getSQL() {
    ...
    function setSearchVar($linkRecType = '') {
        ...
        if ($contact_id != "") {
            ...
            if ($member_type == 'Client' ) {
                $searchVar->sqlSearchVar[] = "c.member_type = 'Client'";
            }

            if ($member_type == 'Agent' ) {
                $searchVar->sqlSearchVar[] = "c.member_type = 'Agent'";
            }
    ...
    function getNewValidate() {
        ...
        $validate->validateData('name', 'Please enter the first name');
    ...
    function getFields(){
        ...
        if ($cpCfg['m.manPower.contact.flagInvalidEmails'] && $fa['email'] != '' && !$validate->isEmail($fa['email'])){
            $fa['invalid_email'] = 1;
        }
    ...
    function importDataRowCallback($contact_id, $fa) {
        ...
        if ($cpCfg['m.manPower.contact.flagInvalidEmails'] && !$validate->isEmail($fa['email'])){
            $fa2 = array();
            $fa2['invalid_email'] = 1;
            $fa2['subscribe'] = 0;
     ...
     *
     */
    function getmanPowerContactEventEventLinkSQL($id){
