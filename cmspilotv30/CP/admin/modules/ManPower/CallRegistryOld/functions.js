...
cpm.manPower.callRegistry.init = function(){
    $('select#fld_company_id').change(function() {
        ...
        var faxObj = $('.m-manPower_callRegistry #fld_fax');
        var mobileObj = $('.m-manPower_callRegistry #fld_mobile');
        var emailObj = $('.m-manPower_callRegistry #fld_email');
        var industryObj = $('.m-manPower_callRegistry #fld_industry');
        var companyAddressObj = $('.m-manPower_callRegistry #fld_company_address');
        
        var urlCompany = 'index.php?module=manPower_callRegistry&_spAction=companyDetailsJSON&showHTML=0';
    ...
    $('.m-manPower_callRegistry #convertToOpportunity').livequery('click', function (e){
        ...
        if (no_of_candidates != '') {
            ...
            if (!confirm(msg)){
                ...
                Util.showProgressInd();
                var call_registry_id = $(this).attr('call_registry_id');
                var url = 'index.php?module=manPower_callRegistry&_spAction=convertToOpportunity&showHTML=0&call_registry_id=' + call_registry_id;
    ...
    $(".m-manPower_callRegistry input[name='company_name']").livequery(function(){
    	...
    	$(titleObj).autocomplete({
    		...
    		,select: function(event, ui) {
                ...
                var contactObj = $('.m-manPower_callRegistry #fld_contact_name');
    ...
    $('.m-manPower_callRegistry select[name=title]').livequery('click', function (e){
        title = $(this).val();
        if(title == 'Others'){
    ...
    $('.m-manPower_callRegistry select[name=status]').livequery('change', function (e){
        ...
        if(status == 'High Win Ratio'){
            $('.m-manPower_callRegistry .noOfCandidateDisplay').show();
        } else {
            $('.m-manPower_callRegistry .noOfCandidateDisplay').hide();
        }
    });

    $('#sendProfileToClient').livequery('click', function(e){
        var title = "Send Profile To Client";
        var email = $('.m-manPower_callRegistry #fld_email').val();
        e.preventDefault();
        ...
        if (email != '') {
            ...
            msg = "Please enter email and Save the Record, email field should not be empty";
            alert(msg);
        }
    });
}