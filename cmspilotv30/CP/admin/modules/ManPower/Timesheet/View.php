<?
class CP_Admin_Modules_ManPower_Timesheet_View extends CP_Common_Lib_ModuleViewAbstract
{
    ...
    function getList($dataArray) {
        ...
        if ($cpCfg['m.manPower.timesheet.daysLbl'] == 1){
            $lbl = 'Days';
        } else {
            $lbl = 'Hours';
        }
        ...
        {$text2}
        {$listObj->getListHeaderCell('Staff Name', 'staff_name')}
        {$listObj->getListHeaderCell($cpCfg['m.manPower.staffTeamLabel'], 'staff_team')}
    ...
    function getEdit($row) {
        ...
        if ($row['project_id'] != ''){
            $sqlTask = $fn->getDDSql('manPower_task', array('condn' => "project_id = {$row['project_id']}"));
        } else if ($row['opportunity_id'] != ''){
            $sqlTask = $fn->getDDSql('manPower_task', array('condn' => "opportunity_id = {$row['opportunity_id']}"));
        }

        $sqlStaff = $fn->getDDSql('manPower_staff', array('condn' => "status = 'Current' AND staff_login_type = 'Staff'"));
