<?
class CP_Admin_Modules_LawNews_ReporterLink_View extends CP_Common_Lib_ModuleLinkViewAbstract
{
    function getList($dataArray, $linkRecType) {
        ...
        foreach ($dataArray as $row){
            ...
            {$listLinkObj->getListRowEndLink($linkRecType, $row['reporter_id'])}
            ";
            $rowCounter++ ;
        }
        ...
        {$listLinkObj->getListHeaderCellLink($linkRecType,'Title', 'r.title')}
    ...
    function getEdit(){
        ...
        $row = $fn->getRecordRowByID('reporter', 'reporter_id', $id);
        $text = "
        <form id='portalForm' class='yform columnar' method='post' action='{$formAction}'>
            <fieldset>
                {$formObj->getTBRow('Title', 'title', $row['salutation'])}
                {$formObj->getYesNoRRow('Published', 'published', $row['published'] )} 
                {$formObj->getHTMLEditor('Description', 'description', $ln->gfv($row, 'description', '0'))}  
            </fieldset>
            <input type='hidden' name='reporter_id' value='{$id}' />
        </form>
        ";
