<?
class CP_Admin_Modules_Logistics_Contact_View extends CP_Common_Lib_ModuleViewAbstract
{
    function getList($dataArray){
        ...
        foreach ($dataArray as $row){
            ...
            if($cpCfg['m.logistics.contact.hasCompanyTable'] == 1){
                $rowsCompanyTable = "
                <td>
                    <div align='left'>
            ...
            if($cpCfg['m.logistics.contact.hasWebLogin'] == 1){
                $rowsWebLogin = "
                {$listObj->getListPublishedImage($row['published'], $row['contact_id'])}
                ";
            }

            if($cpCfg['m.logistics.contact.hasSalutation'] == 1){
                $rowsSalutation = "
        ...
        {$textSalutation}
        {$listObj->getListHeaderCell($ln->gd('m.logistics.header.contact.lbl.firstName', 'First Name'), 'c.first_name')}
        {$listObj->getListHeaderCell($ln->gd('m.logistics.header.contact.lbl.lastName', 'Last Name'), 'c.last_name')}
    ...
    function getNew(){
        ...
        $fielset = "
        {$formObj->getTBRow($ln->gd('m.logistics.contact.lbl.firstName', 'First Name'), 'first_name')}
        {$formObj->getTBRow($ln->gd('m.logistics.contact.lbl.lastName', 'Last Name'), 'last_name')}
    ...
    function getEdit($row){
        ...
        if($cpCfg['m.logistics.contact.showUsername'] == 1){
            $username = "
        ...
        $langPref = '';
        if ($cpCfg['m.logistics.contact.showLangPrefernce']){
    ...
    function getRightPanel($row){
        ...
        $text = "
        {$media->getRightPanelMediaDisplay($ln->gd('cp.lbl.picture', 'Picture'), 'logistics_contact', 'picture', $row)}
        {$comment->getView(array(
             'roomName' => 'logistics_contact'
            ,'recordId' => $record_id
        ))}
        ";
    ...
    function getQuickSearch() {
        ...
        //==================================================================//
        $spArray = $cpCfg['m.logistics.contact.specialSearchArr'];
