<?
class CP_Admin_Modules_EzTrade_Contact_View extends CP_Common_Lib_ModuleViewAbstract
{
    /**
     *
     */
    function getList($dataArray){
        ...
        foreach ($dataArray as $row){
            ...
            $company = $fn->getRecordDetailLink('ezTrade_company', 'record_id', $row['company_id'], $exp);
            $rows .= "
            {$listObj->getListRowHeader($row, $count)}
            {$listObj->getListDataCell($row['salutation'])}
            {$listObj->getGoToDetailText($count, $row['contact_name'])}
    ...
    function getEdit($row){
        ...
        $sqlComp = $fn->getDDSql('ezTrade_company');
        ...
        {$formObj->getTextBoxRow('First Name', 'first_name', $row['first_name'])}
        {$formObj->getTextBoxRow('Last Name', 'last_name', $row['last_name'])}
    ...
    function getRightPanel($row){
        ...
        $text = "
        {$media->getRightPanelMediaDisplay('Attachments', 'ezTrade_contact', 'attachment', $row)}
        {$comment->getView(array(
             'roomName' => 'ezTrade_contact'
            ,'recordId' => $record_id
        ))}
        ";
    ...
    function getQuickSearch() {
        ...
        $sqlCompany = $fn->getDDSql('ezTrade_company');
