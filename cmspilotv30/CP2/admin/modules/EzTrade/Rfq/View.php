<?
class CP_Admin_Modules_EzTrade_Rfq_View extends CP_Common_Lib_ModuleViewAbstract
{
    /**
     *
     */
    function getList($dataArray){
        ...
        foreach ($dataArray as $row){
            ...
            $enquiry_code = $fn->getRecordDetailLink('ezTrade_enquiry', 'record_id', $row['enquiry_id'], $exp);
        ...
        {$listObj->getListHeaderCell('RFQ Title', 'qr.title')}
    ...
    function getEdit($row){
        ...
        $sqlSupplierName = $fn->getDDSql('ezTrade_company', array('condn' => "category = 'Supplier'"));
        ...
        $sqlDeliveryTerms = $modDeliveryTerms->model->getDeliveryTermsSupplierSQL($row['company_id_supplier']);

        $modPaymentTerms = getCPModuleObj('ezTrade_paymentTermsLink');
        $sqlPaymentTerm = $modPaymentTerms->model->getPaymentTermsSQL($row['company_id_supplier']);
        ...
        $fieldset2 = "
        {$formObj->getDDRowBySQL('Required Delivery Terms', 'required_delivery_terms', $sqlDeliveryTermsCustomer, $row['required_delivery_terms'], $expVl)}
        ...
        $fieldset3 = "
        {$formObj->getDDRowBySQL('Delivery Terms', 'delivery_terms_supplier', $sqlDeliveryTerms, $row['delivery_terms_supplier'], $expVl)}
        {$formObj->getDateRow('Quote Validity', 'valid_until', $row['valid_until'])}
        {$formObj->getDDRowBySQL('Payment Terms', 'payment_terms', $sqlPaymentTerm, $row['payment_terms'], array('sqlType' => 'OneField'))}
        {$formObj->getTextAreaRow('Notes from Supplier', 'notes_from_supplier', $row['notes_from_supplier'])}
        ";
    ...
    function getRightPanel($row){
        ...
        $links .= "
        {$displayLinkData->getLinkPortalMain("ezTrade_rfq", "ezTrade_productLink", "RFQ Line", $row)}
        ";

        $text = "
        {$links}
        {$media->getRightPanelMediaDisplay("Attachments", "ezTrade_rfq", "attachment", $row)}
        {$comment->getView(array(
             'roomName' => 'ezTrade_rfq'
            ,'recordId' => $record_id
        ))}
        ";
